[Strona główna](../../README.md) > [Dokumentacja](../README.md) > [Karty Genów](00_indeks.md) > [SCN2A](SCN2A.md)

---

### 1. Nagłówek i Nazwy
* **Główny symbol genu:** SCN2A (ang. *sodium voltage-gated channel alpha subunit 2*)
* **Pełna nazwa biochemiczna:** Podjednostka alfa-2 napięciowo-zależnego kanału sodowego typu II (Nav1.2)
* **Nazwy potoczne i medialne:** Gen rozwojowej pobudliwości, gen padaczki wczesnodziecięcej
* **Synonimy medyczne (opcjonalnie):** NAC2, NAV1.2, SCN2A1, SCN2A2; locus 2q24.3

### 2. Identyfikator (rsID) i Charakterystyka Wariantu
* **Główny rsID:** rs10174400 (intron, kognicja i schizofrenia)
* **Lokalizacja chromosomalna:** Chromosom 2, prążek 2q24.3
* **Typ wariantu:** SNP introniczne/regulacyjne; rzadkie missense patogenne
* **Zapis zmiany nukleotydowej (HGVS):** rs10174400: allel T – ryzyko; rs2121371, rs1864885 – farmakogenetyka; patogenne: np. c.788C>T (p.Ala263Val)
* **Orientacja nici i mapowanie alleli:** rs10174407 – C (referencyjny) vs T (alternatywny); weryfikuj zgodność z raportem
* **Powiązane markery / haplotyp (opcjonalnie):** rs2121371, rs1864885, rs17183814, rs2304016, rs1483179196
* **Klasyfikacja bazowa (opcjonalnie):** rs10174400 – polimorfizm powszechny; mutacje patogenne – ClinVar pathogenic (DEE, Ohtahara)

### 3. Mechanizm działania
* **Rola biologiczna genu/białka:** Nav1.2 to kanał sodowy w segmencie AIS i przewężeniach Ranviera. Kontroluje próg pobudliwości neuronów piramidowych i uwalnianie glutaminianu. Alternatywny splicing eksonu 5 (5N/5A) chroni niedojrzały mózg przed ekscytotoksycznością.
* **Wpływ wariantu na szlak:** Warianty GoF (gain-of-function) → hiperpobudliwość, padaczka wczesna. Warianty LoF (loss-of-function) → ASD, opóźnienie rozwoju, padaczka późna. rs10174400 T u zdrowych zwiększa ekspresję mRNA w dlPFC i czynnik g; u chorych na schizofrenię – spadek kognicji.
* **Efekt funkcjonalny:** Ten sam gen moduluje zarówno inteligencję (polimorfizmy), jak i ciężkie encefalopatie (mutacje patogenne).

### 4. Tabela Wariantów
**rs10174400 (kognicja, schizofrenia)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **C/C** | Prawidłowa ekspresja transkryptów w dlPFC | Standardowy fenotyp kognitywny; u chorych na schizofrenię wyższa sprawność poznawcza |
| **★ C/T** | Pośrednia ekspresja mRNA | Umiarkowana sprawność pamięci roboczej u pacjentów neuropsychiatrycznych |
| **T/T** | ↑ ekspresja dlPFC u zdrowych · ↓ transkrypty u chorych | **U zdrowych:** wyższy czynnik *g*<br><br>**U chorych:** głębokie deficyty kognitywne (10,4% wariancji) |
**rs2121371 (farmakogenetyka AED)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **C/C** | Standardowa odpowiedź na leki | Typowa tolerancja AED; słabsza odpowiedź u niektórych kohort |
| **C/T** | Pośrednia | Umiarkowane ryzyko polekowego uszkodzenia wątroby |
| **★ T/T** | Zmiana powinowactwa kanału | Słabsza odpowiedź na AED; podwyższone ALT/AST |
**rs1864885 (farmakogenetyka AED, pediatria)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ A/A** | Standardowa odpowiedź na AED | Typowa skuteczność leków przeciwpadaczkowych |
| **A/G** | Pośrednia | Obniżona efektywność AED u dzieci w niektórych kohortach |
| **G/G** | Alternatywna | Słabsza odpowiedź kliniczna na terapię przeciwpadaczkową |
**rs17183814 (Parkinson, choroba afektywna dwubiegunowa)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ G/G** | Prawidłowa ekspresja w podkorze | Brak zwiększonego ryzyka sPD/BD z tego SNP |
| **G/A** | Pośrednia | Umiarkowane ryzyko sPD (szczególnie u mężczyzn) |
| **A/A** | Obniżone mRNA SCN2A | Podwyższone ryzyko sporadycznej choroby Parkinsona i BD |
**rs2304016 (IVS7-32A>G, Parkinson)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ A/A** | Referencyjna | Standardowe ryzyko sPD |
| **A/G** | Pośrednia | Zwiększone ryzyko sPD; różnicowanie EOPD/LOPD |
| **G/G** | Alternatywna | Silniejsza asocjacja z wczesnym początkiem Parkinsona |
**Mutacje patogenne (przykłady)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **GoF (np. p.Ala263Val)** | Hiperpobudliwość kanału | Zespół Ohtahara, padaczka wczesna; karbamazepina/fenytoina skuteczne |
| **LoF (np. nonsense)** | Haploinsuficjencja Nav1.2 | ASD, opóźnienie rozwoju; blokery sodowe przeciwwskazane |

### 5. Statystyki populacyjne
* **Średnia globalna (ALL):** rs10174400 allel T MAF ~33,7–34,0%
* **Europa (NFE):** C/C ~47%, C/T ~42%, T/T ~11%
* **Afryka (AFR):** Brak precyzyjnych danych dla rs2121371/rs1864885 w kohortach europejskich
* **Azja Wschodnia (EAS):** rs2121371/rs1864885 – dane głównie z populacji chińskiej Han
* **Uwagi o zmienności populacyjnej:** SRDs (SCN2A-related disorders) ~1:21 152–1:78 608 urodzeń w zależności od regionu; patogenne missense MAF <0,01%.

### 6. Wpływ na życie (Zalecenia)
* **Diagnostyka:** Przy drgawkach u niemowląt – pełne sekwencjonowanie SCN2A; klasyfikacja GoF vs LoF decyduje o leczeniu.
* **Farmakologia:** GoF – karbamazepina, fenytoina; LoF – unikać blokerów sodowych; rozważyć ASO.
* **Dieta ketogeniczna:** Skuteczna przy lekooporności (np. p.Met136Ile); wczesne wdrożenie może dać remisję napadów.
* **Kardiologia:** Ryzyko SUDEP przy ciężkiej padaczce – monitoring EKG/Holter, urządzenia alarmowe.
* **Kognityka:** Nosiciele rs10174400 T/T (zdrowi) – stymulacja intelektualna; przy obciążeniu rodzinnym – higiena snu, redukcja stresu.
* **Ostrzeżenie kliniczne:** Materiał ma charakter informacyjny i nie zastępuje konsultacji lekarskiej.

### 7. Ciekawostki
* Międzynarodowy Dzień Świadomości SCN2A: 24 lutego (2/24) – odwołanie do locus 2q24.3.
* Jeden aminokwas (Asn→Asp w pozycji 209) między izoformą 5N a 5A to ewolucyjny mechanizm ochronny przed drgawkami u niemowląt.
* Parabeny w kosmetykach hamują ludzkie kanały Nav1.2 in vitro.

### 8. Źródła (Referencje)
* **PMID: 25961639** (Scult MA et al., 2015) – rs10174400 a inteligencja ogólna i fMRI dlPFC.
* **PMID: 24718902** (Dickinson D et al., 2014) – Różnicowy wpływ rs10174400 u schizofrenii i kontroli.
* **PMID: 30415926** (Gursoy S et al., 2019) – Dieta ketogeniczna przy mutacji SCN2A.
* **Baza referencyjna:** [SNPedia (rs10174400)](https://www.snpedia.com/index.php/Rs10174400) – SCN2A, padaczka, kognicja.
