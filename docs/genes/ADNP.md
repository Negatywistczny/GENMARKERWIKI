[Strona główna](../../README.md) > [Dokumentacja](../README.md) > [Karty Genów](00_indeks.md) > [ADNP](ADNP.md)

---

### 1. Nagłówek i Nazwy
* **Główny symbol genu:** ADNP (ang. *activity-dependent neuroprotective protein*)
* **Pełna nazwa biochemiczna:** Białko neuroprotekcyjne zależne od aktywności (activity-dependent neuroprotector homeobox)
* **Nazwy potoczne i medialne:** Gen Helsmoortel-Van der Aa, gen autyzmu, gen ADNP
* **Synonimy medyczne (opcjonalnie):** HVDAS, MRD28, SIP1, ZFHX1B

### 2. Identyfikator (rsID) i Charakterystyka Wariantu
* **Główny rsID:** rs886041116 (c.2188C>T, p.Arg730Ter)
* **Lokalizacja chromosomalna:** Chromosom 20, prążek 20q13.13
* **Typ wariantu:** Mutacje nonsense/delecje patogenne; SNP regulacyjny rs12480328
* **Zapis zmiany nukleotydowej (HGVS):** rs886041116: c.2188C>T (p.Arg730Ter); c.2491_2494delTTAA (p.Leu831Ifs*82); rs12480328 (intron)
* **Orientacja nici i mapowanie alleli:** Mutacje de novo dominują; pLI genu ~1,0
* **Powiązane markery / haplotyp (opcjonalnie):** ClinVar ID 139631 (c.2491_2494delTTAA)
* **Klasyfikacja bazowa (opcjonalnie):** Mutacje nonsense/frameshift – patogenne (HVDAS); rs12480328 – ryzyko raka prostaty

### 3. Mechanizm działania
* **Rola biologiczna genu/białka:** ADNP to czynnik transkrypcyjny z domeną homeobox i palcami cynkowymi. Tworzy kompleks ChAHP (z CHD4, HP1) i AKG (z KDM1A, GTF2I), reguluje chromatynę i blokuje transpozony. Motyw NAP stabilizuje mikrotubule (EB1/EB3).
* **Wpływ wariantu na szlak:** Haploinsuficjencja → zaburzona neurogeneza, remodelowanie chromatyny, tauopatie, dysfunkcja mitochondriów.
* **Efekt funkcjonalny:** Zespół HVDAS: opóźnienie rozwoju, ASD (~93%), przedwczesne uzębienie (~81%), hipotonia (~75%).

### 4. Tabela Wariantów
**rs886041116 (c.2188C>T, p.Arg730Ter)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ G/G** | Prawidłowa ekspresja ADNP (allel dziki C) | Normotypowy rozwój |
| **G/T** | Heterozygot de novo; nonsense (c.2188C>T) | Zespół HVDAS: opóźnienie mowy, niepełnosprawność intelektualna |
| **T/T** | Homozygot patogenny | Brak w populacji zdrowej (letalityt embrionalna) |
**Mutacje patogenne (c.2491_2494delTTAA, hotspot)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **TTAA/TTAA** | Prawidłowa | Norma |
| **TTAA/del** | Heterozygot de novo | HVDAS z nadreaktywnością sensoryczną, stereotypiami |
**rs12480328 (intron, ryzyko raka prostaty)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ T/T** | Referencyjna | Brak zwiększonego ryzyka neuro |
| **T/C** | Pośrednia | Umiarkowane ryzyko onkologiczne |
| **C/C** | Alternatywna | Asocjacja ze wzrostem ryzyka raka prostaty (RAF ~0,93) |

### 5. Statystyki populacyjne
* **Średnia globalna (ALL):** Mutacje patogenne MAF 0% w gnomAD; zespół ~1:20 000 urodzeń
* **Europa (NFE):** ~0,17% ASD przypisuje się mutacjom ADNP; incydencja HVDAS ~1:20 000
* **Afryka (AFR):** Patogenne mutacje nieobecne w kohortach zdrowych
* **Azja Wschodnia (EAS):** Podobna częstość jak w populacjach europejskich
* **Uwagi o zmienności populacyjnej:** rs12480328 RAF do 0,93; mozaicyzm somatyczny moduluje fenotyp.

### 6. Wpływ na życie (Zalecenia)
* **Wczesna diagnostyka:** Przedwczesne uzębienie mleczne (81%) – biomarker do testów genetycznych.
* **Terapia:** Fizjoterapia od niemowlęctwa; melatonina (65% zaburzeń snu); omega-3, koenzym Q10, L-karnityna.
* **Gastroenterologia:** Dieta lekkostrawna przy GERD; kontrola masy ciały (polyphagia u starszych).
* **Ostrzeżenie kliniczne:** Materiał ma charakter informacyjny i nie zastępuje konsultacji lekarskiej.

### 7. Ciekawostki
* ADNP unikalny dla strunowców — brak ortologów u bezkręgowców.
* Paradoks onkologiczny: mutacje konstytucyjne → niedorozwój mózgu; somatyczne → onkogenez.
* Dzieci z HVDAS często mają radosne usposobienie i silną potrzebę interakcji społecznych.

### 8. Źródła (Referencje)
* **PMID: 24531329** (Helsmoortel M et al., 2014) – Pierwsze powiązanie mutacji ADNP z HVDAS.
* **PMID: 27054228** (Harutyunyan A et al., 2016) – GeneReviews: standardy diagnostyczne HVDAS.
* **PMID: 25178163** (Oz S et al., 2014) – Motyw NAP i mikrotubule.
* **Baza referencyjna:** [SNPedia (Gene:ADNP)](https://www.snpedia.com/index.php/Gene:ADNP) – Mutacje i fenotypy.
