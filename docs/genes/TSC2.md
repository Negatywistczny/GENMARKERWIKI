[Strona główna](../../README.md) > [Dokumentacja](../README.md) > [Karty Genów](00_indeks.md) > [TSC2](TSC2.md)

---

### 1. Nagłówek i Nazwy
* **Główny symbol genu:** TSC2 (ang. *TSC complex subunit 2*)
* **Pełna nazwa biochemiczna:** Tuberyna (ang. *tuberin*)
* **Nazwy potoczne i medialne:** Gen TSC, gen tuberyny, gen stwardnienia guzowatego
* **Synonimy medyczne (opcjonalnie):** LAM; locus 16p13.3; TSC (tuberous sclerosis complex)

### 2. Identyfikator (rsID) i Charakterystyka Wariantu
* **Główny rsID:** rs28934872 (c.1832G>A, p.Arg611Gln)
* **Lokalizacja chromosomalna:** Chromosom 16, prążek 16p13.3
* **Typ wariantu:** SNP missense patogenny; polimorfizmy regulacyjne
* **Zapis zmiany nukleotydowej (HGVS):** rs28934872: c.1832G>A (p.Arg611Gln); rs30259, rs2074969, rs2073636 – introny
* **Orientacja nici i mapowanie alleli:** rs28934872 – G (dziki) vs A (patogenny); weryfikuj z raportem
* **Powiązane markery / haplotyp (opcjonalnie):** rs30259, rs2074969, rs2073636, rs1748
* **Klasyfikacja bazowa (opcjonalnie):** rs28934872 – pathogenic/likely pathogenic (ClinVar)

### 3. Mechanizm działania
* **Rola biologiczna genu/białka:** Tuberyna z hamartyną (TSC1) tworzy kompleks TSC — GAP dla Rheb, hamuje mTORC1. Reguluje wzrost, autophagię, synaptogenezę i plastyczność.
* **Wpływ wariantu na szlak:** Utrata funkcji TSC2 → hiperaktywacja mTOR → guzy SNC, padaczka, dysplazja korowa, LAM u kobiet.
* **Efekt funkcjonalny:** TSC2 odpowiada za ~80% przypadków TSC; mutacje de novo dominują; większa penetracja niż TSC1.

### 4. Tabela Wariantów
**rs28934872 (c.1832G>A, p.Arg611Gln, patogenny)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ G/G** | Prawidłowa tuberyna | Normotypowy rozwój |
| **G/A** | Heterozygot patogenny | TSC: padaczka, guzy, plamki, ASD |
| **A/A** | Homozygot patogenny | Ciężki fenotyp; rzadki |
**rs30259 (intron, modyfikator)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ C/C** | Referencyjna | Standardowy fenotyp |
| **C/T** | Pośrednia | Modyfikacja nasilenia padaczki |
| **T/T** | Alternatywna | Potencjalnie wpływ na ekspresję tuberyny |
**rs1748 (synonimiczny, ekspresja alleliczna TSC2)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **★ T/T** | Jednorodna ekspresja alleli | Standardowy fenotyp; prawidłowa regulacja mTOR |
| **T/C** | Pośrednia | Heterozygot; model badań AEI |
| **C/C** | Alternatywna | Zmiana równowagi ekspresji allelicznej w tkankach |
**rs2074969 / rs2073636 (haplotyp regulacyjny)**

| Genotyp | Aktywność / ekspresja | Wpływ fenotypowy (kliniczny i funkcjonalny) |
| :--- | :--- | :--- |
| **A/A** | Referencyjny haplotyp | Prawidłowa regulacja mTOR; brak zwiększonego ryzyka |
| **★ A/G** | Heterozygot | Pośrednia ekspresja; umiarkowane ryzyko w kohortach rodzinnych |
| **G/G** | Alternatywny haplotyp | Zmiana ekspresji; asocjacje z nasileniem fenotypu TSC |

### 5. Statystyki populacyjne
* **Średnia globalna (ALL):** TSC ~1:6 000–1:10 000; TSC2 ~80% przypadków
* **Europa (NFE):** rs28934872 MAF <0,01% (patogenny)
* **Afryka (AFR):** Brak wystarczających danych dla wszystkich SNP
* **Azja Wschodnia (EAS):** Patogenne mutacje rzadkie
* **Uwagi o zmienności populacyjnej:** LAM (limfangioleiomyomatoza) u kobiet z mutacją TSC2; mozaicyzm moduluje fenotyp.

### 6. Wpływ na życie (Zalecenia)
* **Neurologia:** Wczesne EEG; everolimus/sirolimus przy SEGA i padaczce opornej.
* **Pulmonologia (kobiety):** Monitoring LAM — HRCT, spirometria.
* **Nefrologia:** USG nerek (AML); kontrola funkcji nerek.
* **Dieta:** Ketogenna przy padaczce; unikanie hiperaktywacji mTOR (nadmiar białka w dyskusji).
* **Ostrzeżenie kliniczne:** Materiał ma charakter informacyjny i nie zastępuje konsultacji lekarskiej.

### 7. Ciekawostki
* TSC2 odkryty niezależnie od TSC1 — oba geny hamują ten sam szlak mTOR.
* SEGA (guz podwzgórza) wymaga pilnej interwencji — everolimus zmniejsza objętość guza.
* TSC2 mutacje częściej prowadzą do cięższej padaczki niż TSC1.

### 8. Źródła (Referencje)
* **PMID: 8269512** (The European Chromosome 16 Tuberous Sclerosis Consortium, 1993) – Klonowanie TSC2.
* **PMID: 10205261** (Dabora SL et al., 2001) – Korelacja genotyp-fenotyp TSC1 vs TSC2.
* **PMID: 17304050** (Krueger DA et al., 2007) – Everolimus w TSC.
* **Baza referencyjna:** [SNPedia (TSC2)](https://www.snpedia.com/index.php/TSC2) – Mutacje i fenotypy.
