# Zbiorowe badanie markerów

Scalony kompendium genów bez pełnych kart w wiki — Każdy gen: trzy profile genotypowe (referencyjny / pośredni / ryzyko lub patogenny).

**Łącznie genów:** 118

---

## CZĘŚĆ I. Neurorozwój syndromiczny, chromatyna i geny o wysokiej penetracji

### ANK2

* **Główny symbol genu:** ANK2
* **Pełna nazwa biochemiczna:** Ankiryna 2 (ang. *Ankyrin 2*)
* **Nazwy potoczne i medialne:** Gen rusztowania synaptycznego, strukturalny gen neurorozwojowy
* **Główny rsID / Wariant:** Mutacje punktowe *de novo*, mikrodelecje lub warianty typu utraty funkcji (LoF)
* **Lokalizacja chromosomalna:** Chromosom 4 (4q25-q27)
* **Rola biologiczna genu/białka:** ANK2 koduje ankirynę-B – białko strukturalne, które kotwiczy integralne białka błonowe (kanały jonowe, transportery, cząsteczki adhezyjne) w cytoszkieleletu aktynowo-spektrynowym. Odgrywa krytyczną rolę w organizacji synaps, morfogenezie dendrytów oraz aksonalnym transporcie pęcherzyków synaptycznych.

* **Mój genotyp (WGS):**
  `rs1939197` — **AA** (ensembl+bam, high) — ensembl; depth=20,avg=20.0; gt=0/0






**Typ Dziki (Brak mutacji patogennych)**
Prawidłowa gęstość i stabilna architektura połączeń synaptycznych. Wydajny transport pęcherzyków wzdłuż aksonów, optymalna plastyczność neuronalna i zrównoważony rozwój sieci korowych.

**Warianty populacyjne / Polimorfizmy (Łagodne)**
Subtelne zmiany w wydajności zakotwiczenia kanałów jonowych. Mogą prowadzić do nieznacznych wahań w przewodnictwie synaptycznym lub podatności na przeciążenia sensoryczne w niesprzyjających warunkach środowiskowych.

**Wariant Patogenny (Loss of Function / De novo)**
Drastyczne zaburzenie rozwoju wypustek neuronalnych. Prowadzi do patologicznej hiperłączności (hyperconnectivity) w lokalnych sieciach mózgowych, manifestując się klinicznie jako fenotyp ASD z wyraźnym opóźnieniem rozwoju mowy i dysfunkcjami integracji bodźców.

---

---
### ARID1B

* **Główny symbol genu:** ARID1B
* **Pełna nazwa biochemiczna:** Białko zawierające domenę wiążącą AT-bogaty region 1B (ang. *AT-rich interaction domain-containing protein 1B*)
* **Nazwy potoczne i medialne:** Gen kompleksu SWI/SNF, gen Zespołu Coffin-Siris
* **Główny rsID / Wariant:** Mutacje nonsensowne, przesunięcia ramki odczytu *de novo* lub mikrodelecje 6q25.3
* **Lokalizacja chromosomalna:** Chromosom 6 (6q25.3)
* **Rola biologiczna genu/białka:** ARID1B jest kluczowym komponentem zależnego od ATP kompleksu remodelowania chromatyny SWI/SNF (BAF). Działa jako regulator epigenetyczny, kontrolując dostęp czynników transkrypcyjnych do promotorów tysięcy genów zaangażowanych w neurogenezę, proliferację komórek macierzystych mózgu i powstawanie ciała modzelowatego.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Typ Dziki (Brak mutacji patogennych)**
Prawidłowy profil epigenetyczny. Harmonijna ekspresja genów odpowiedzialnych za rozwój układu nerwowego, optymalna strukturyzacja połączeń międzypółkulowych.

**Warianty Hipomorficzne / Polimorfizmy**
Niewielkie modyfikacje w tempie remodelowania chromatyny. Mogą skutkować subtelnymi odchyleniami w tempie przetwarzania informacji lub elastyczności poznawczej bez wywoływania cech syndromowych.

**Wariant Patogenny (Heterozygota LoF / Mikrodelecja)**
Spadek wydajności kompleksu SWI/SNF prowadzący do Zespołu Coffin-Siris lub niesyndromowej niepełnosprawności intelektualnej. Objawia się głębokim opóźnieniem rozwoju mowy, agenezją ciała modzelowatego, cechami dysmorficznymi i wysokim nasileniem cech autystycznych.

---

---
### CHD8

* **Główny symbol genu:** CHD8
* **Pełna nazwa biochemiczna:** Białko helikazy chromodomenowej wiążące DNA 8 (ang. *Chromodomain helicase DNA-binding protein 8*)
* **Nazwy potoczne i medialne:** Gen makrocefalii i autyzmu, główny regulator (master regulator) genów ASD
* **Główny rsID / Wariant:** Mutacje *de novo* przerywające ramkę odczytu (LoF)
* **Lokalizacja chromosomalna:** Chromosom 14 (14q11.2)
* **Rola biologiczna genu/białka:** CHD8 reguluje strukturę chromatyny poprzez ATP-zależną supresję lub aktywację szlaku sygnałowego Wnt/beta-kateniny. Jest nadrzędnym regulatorem proliferacji wczesnych progenitorów neuronów – decyduje o ostatecznej masie i objętości mózgowia.

* **Mój genotyp (WGS):**
  `rs10122956` — **GG** (ensembl+bam, high) — ensembl; depth=36,avg=36.0; gt=0/0






**Typ Dziki (Ekspresja zbalansowana)**
Prawidłowy, symetryczny podział komórkowy w fazie neurogenezy. Obwód czaszki w normie populacyjnej, prawidłowa homeostaza szlaku Wnt i harmonijny rozwój struktur korowych.

**Warianty populacyjne o niskiej penetracji**
Łagodna tendencja do wyższych percentyli obwodu głowy (w granicach normy), bez strukturalnych zaburzeń architektury mózgu i bez nasilonych cech behawioralnych.

**Mutacja Patogenna (Heterozygota LoF)**
Klasyczny podtyp kliniczny CHD8-ASD. Przełamany hamulec proliferacji skutkuje wyraźną makrocefalią (wielkogłowiem). Towarzyszą mu specyficzne dysfunkcje osi jelitowej (przewlekłe zaparcia), głębokie zaburzenia architektury snu oraz nasilony fenotyp autystyczny.

---

---
### DYRK1A

* **Główny symbol genu:** DYRK1A
* **Pełna nazwa biochemiczna:** Kinaza regulowana fosforylacją tyrozyny o podwójnej specyficzności 1A (ang. *Dual specificity tyrosine-phosphorylation-regulated kinase 1A*)
* **Nazwy potoczne i medialne:** Gen mikrocefalii, gen regionu krytycznego zespołu Downa
* **Główny rsID / Wariant:** Mutacje *de novo* utraty funkcji lub zmiany liczby kopii (CNV)
* **Lokalizacja chromosomalna:** Chromosom 21 (21q22.13)
* **Rola biologiczna genu/białka:** DYRK1A fosforyluje kluczowe czynniki transkrypcyjne oraz białka cytoszkieletu. Kontroluje krytyczne przejście neuronów z fazy proliferacji komórkowej do fazy różnicowania (neurogeneza). Zarówno nadekspresja, jak i niedobór białka drastycznie zaburzają architekturę mózgu.

* **Mój genotyp (WGS):**
  `rs8025580` — **AA** (ensembl+bam, high) — ensembl; depth=48,avg=48.0; gt=0/0; chr21 proxy




**Typ Dziki (Zbalansowana ekspresja)**
Optymalne tempo neurogenezy. Prawidłowa gęstość neuronów w korze mózgowej, obwód głowy stabilny w normie populacyjnej.

**Nadekspresja (np. Trisomia 21 / Zespół Downa)**
Przedwczesne wyjście progenitorów ze stadium podziałów, wczesne starzenie się neuronów, upośledzenie plastyczności synaptycznej i upośledzenie funkcji kognitywnych.

**Mutacja Utraty Funkcji (LoF - Heterozygota)**
Zespół niepełnosprawności intelektualnej związany z DYRK1A. Skutkuje ciężką mikrocefalią (małogłowiem), głębokim deficytem mowy, hipertonią mięśniową oraz stereotypiami ruchowymi wpisującymi się w spektrum autyzmu.

---

---
### GRIN2B

* **Główny symbol genu:** GRIN2B
* **Pełna nazwa biochemiczna:** Podjednostka 2B receptora glutaminianergicznego jonotropowego NMDA (ang. *Glutamate ionotropic receptor NMDA type subunit 2B*)
* **Nazwy potoczne i medialne:** Gen plastyczności synaptycznej, pamięci i uczenia się
* **Główny rsID / Wariant:** Mutacje *de novo* (typu utraty funkcji lub mutacje missens o charakterze dominant-negative)
* **Lokalizacja chromosomalna:** Chromosom 12 (12p13.1)
* **Rola biologiczna genu/białka:** Koduje podjednostkę GluN2B receptora NMDA, kluczowego dla postsynaptycznego napływu jonów wapnia (Ca²⁺). Odpowiada za indukcję długotrwałego wzmocnienia synaptycznego (LTP), plastyczność rozwojową mózgu oraz funkcjonowanie pamięci roboczej.

* **Mój genotyp (WGS):**
  `rs7301328` — **GC** (ensembl+bam, high) — ncbi; depth=40,avg=40.0; gt=0/1






**Typ Dziki (Optymalna przewodność receptora)**
Zbalansowany, precyzyjny napływ wapnia do neuronu postsynaptycznego. Efektywne procesy uczenia się, wysoka elastyczność poznawcza, prawidłowa ochrona przed ekscytotoksycznością.

**Warianty populacyjne Gain-of-Function (Łagodne)**
Zwiększona gęstość lub czułość receptorów NMDA. Może sprzyjać wysokiej sprawności analitycznej w optymalnych warunkach, kosztem wyższej podatności na stany lękowe i nadwrażliwość na bodźce (przebodźcowanie).

**Mutacja Patogenna de novo (LoF / Dominant-Negative)**
Encefalopatia neurorozwojowa GRIN2B. Powoduje drastyczne zaburzenie plastyczności synaptycznej, ciężką niepełnosprawność intelektualną, hiponię mięśniową, lekooporną padaczkę, brak rozwoju mowy oraz głęboki fenotyp autystyczny.

---

---
### KMT2D

* **Główny symbol genu:** KMT2D
* **Pełna nazwa biochemiczna:** Metylotransferaza lizynowa 2D (ang. *Lysine methyltransferase 2D*)
* **Nazwy potoczne i medialne:** Gen Zespołu Kabuki, epigenetyczny otwieracz chromatyny
* **Główny rsID / Wariant:** Mutacje nonsensowne, zmiany ramki odczytu *de novo* (LoF)
* **Lokalizacja chromosomalna:** Chromosom 12 (12q13.12)
* **Rola biologiczna genu/białka:** KMT2D odpowiada za mono-, di- i trimetylację lizyny 4 w histonie H3 (H3K4me1/2/3). Modyfikacja ta stanowi nadrzędny sygnał epigenetyczny do rozluźnienia struktury chromatyny (przejście w euchromatynę), co umożliwia aktywację transkrypcji genów kluczowych dla rozwoju twarzoczaszki, narządów wewnętrznych oraz neurogenezy w hipokampie.

* **Mój genotyp (WGS):**
  `rs772057441` — **AA** (ensembl+bam, high) — ncbi; depth=29,avg=29.0; gt=0/0





**Typ Dziki (Prawidłowa metylacja histonów)**
Stabilna, skoordynowana transkrypcja zależna od stanu euchromatyny. Harmonijny rozwój struktur somatycznych oraz prawidłowy przebieg neurogenezy dorosłej w hipokampie.

**Warianty populacyjne / Polimorfizmy**
Subtelne przesunięcia w profilu metylacji histonów, mogące skutkować drobnymi, bezobjawowymi cechami anatomii twarzy lub niewielkimi różnicami w dynamice zapamiętywania.

**Mutacja Patogenna (Heterozygota LoF / Zespół Kabuki typ 1)**
Zablokowanie otwierania chromatyny dla kluczowych genów rozwojowych. Skutkuje unikalną dysmorfią twarzy (długie szpary powiekowe, wywinięcie dolnej powieki, duże małżowiny uszne), wrodzonymi wadami serca, niepełnosprawnością intelektualną, znacznym opóźnieniem rozwoju mowy oraz cechami spektrum autyzmu powiązanymi z upośledzeniem plastyczności hipokampa.

---
### PTEN

* **Główny symbol genu:** PTEN
* **Pełna nazwa biochemiczna:** Homolog fosfatazy i tensyny (ang. *Phosphatase and tensin homolog*)
* **Nazwy potoczne i medialne:** Gen Zespołu Cowdena, główny hamulec wzrostu komórkowego (szlak mTOR)
* **Główny rsID / Wariant:** rs701848 (tag-SNP) / mutacje LoF PTEN (osobno, sekwencjonowanie)
* **Lokalizacja chromosomalna:** Chromosom 10 (10q23.31)
* **Rola biologiczna genu/białka:** PTEN to enzym działający jako jeden z najważniejszych supresorów nowotworowych w organizmie. Jest bezpośrednim negatywnym regulatorem szlaku PI3K/Akt/mTOR. Hamuje nadmierną proliferację, przerost somatyczny neuronów oraz patologiczne zagęszczenie wypustek synaptycznych.

* **Mój genotyp (WGS):**
  `rs701848` — **CC** (ensembl+bam, high) — ncbi; depth=32,avg=32.0; gt=1/1






**T/T (Typ referencyjny — pełna supresja mTOR)**
Precyzyjnie kontrolowany metabolizm i wzrost komórkowy. Optymalna wielkość somatyczna neuronów, prawidłowa makrostruktura kory mózgowej i obwód głowy w normie.

**T/C (Wariant heterozygoticzny)**
Subtelne przesunięcia w kinetyce szlaku mTOR. Mogą skutkować łagodną tendencją do wyższych percentyli wzrostu tkankowego.

**C/C (Homozygota alleliczna C)**
Typowy profil populacyjny tag-SNP rs701848. Nie oznacza Zespołu Cowdena ani mutacji LoF — te wymagają osobnej analizy sekwencji/CNV.

---

---
### TCF4

* **Główny symbol genu:** TCF4
* **Pełna nazwa biochemiczna:** Czynnik transkrypcyjny 4 (ang. *Transcription factor 4*)
* **Nazwy potoczne i medialne:** Gen Zespołu Pitta-Hopkinsa, nadrzędny regulator komórek progenitorowych
* **Główny rsID / Wariant:** rs17597926 (GWAS) lub mutacje typu utraty funkcji (*de novo*)
* **Lokalizacja chromosomalna:** Chromosom 18 (18q21.2)
* **Rola biologiczna genu/białka:** TCF4 koduje czynnik transkrypcyjny z rodziny bHLH (basic helix-loop-helix). Działa jako nadrzędny dimer wiążący DNA, który kontroluje wczesną neurogenezę, prawidłową migrację neuronów do odpowiednich warstw kory oraz plastyczność synaptyczną w strukturach hipokampa.

* **Mój genotyp (WGS):**
  `rs17597926` — **GG** (ensembl+bam, high) — marker_report



**Typ Dziki (Prawidłowa transkrypcja bHLH)**
Prawidłowa, warstwowa architektura kory mózgowej. Zbalansowane różnicowanie linii neuronalnych i glejowych, stabilny rozwój intelektualny.

**Warianty Introniczne populacyjne (Sygnały GWAS)**
Silnie powiązane genetycznie ze zmianami podatności na schizofrenię, manie oraz drobnymi wahaniami sprawności pamięci roboczej w populacji ogólnej.

**Mutacja Patogenna (Heterozygota LoF / Haploinsuficjencja)**
**Zespół Pitta-Hopkinsa**. Skutkuje głębokim opóźnieniem rozwoju intelektualnego, specyficznymi, naprzemiennymi epizodami hiperwentylacji i bezdechu (apnea), cechami autystycznymi, ataksją oraz charakterystyczną dysmorfią ust (szerokie usta w kształcie łuku Amora).

---

---
### UBE3A

* **Główny symbol genu:** UBE3A
* **Pełna nazwa biochemiczna:** Ligaza białkowa ubikwityny E3A (ang. *Ubiquitin protein ligase E3A*)
* **Nazwy potoczne i medialne:** Gen Zespołu Angelmana, gen piętnowania genomowego (imprinting)
* **Główny rsID / Wariant:** rs1807588 (tag-SNP) / delecje 15q11-q13, imprinting (osobno)
* **Lokalizacja chromosomalna:** Chromosom 15 (15q11.2)
* **Rola biologiczna genu/białka:** UBE3A koduje ligazę ubikwityny, która kieruje specyficzne białka (np. Arc – regulator receptorów AMPA) do degradacji w proteasomach. W neuronach ludzkiego mózgu ekspresji ulega *wyłącznie* kopia pochodząca od matki; kopia ojcowska jest całkowicie wyciszona epigenetycznie.

* **Mój genotyp (WGS):**
  `rs1807588` — **AG** (ensembl+bam, high) — ncbi; depth=33,avg=33.0; gt=0/1






**A/A (Typ referencyjny)**
Optymalna utylizacja białek synaptycznych. Prawidłowa plastyczność, właściwe wygaszanie zbędnych impulsów elektrycznych i harmonijny rozwój psychoruchowy.

**A/G (Wariant heterozygoticzny)**
Standardowy profil populacyjny tag-SNP rs1807588. Nie oznacza duplikacji 15q11-q13 (Dup15q) — CNV wymaga osobnej analizy.

**G/G (Homozygota alleliczna G)**
Homozygota alleliczna G przy tag-SNP — typowy wariant populacyjny bez cech zespołu Angelmana (Angelmana wymaga utraty ekspresji matczynej UBE3A).

---

---

## CZĘŚĆ II. Szlaki monoaminergiczne — transportery i receptory
### DRD4

* **Główny symbol genu:** DRD4
* **Pełna nazwa biochemiczna:** Receptor dopaminergiczny D4 (ang. *Dopamine receptor D4*)
* **Nazwy potoczne i medialne:** Gen poszukiwaczy przygód (ang. *Novelty seeking gene*), wariant nomadów
* **Główny rsID / Wariant:** Polimorfizm VNTR 48-bp w egzonie 3 (warianty od 2R do 11R; kluczowy 7R)
* **Lokalizacja chromosomalna:** Chromosom 11 (11p15.5)
* **Rola biologiczna genu/białka:** Koduje receptor sprzężony z białkiem G, wykazujący silną ekspresję w korze przedczołowej. Wpływa na hamowanie cyklazy adenylowej, modulując funkcje wykonawcze, elastyczność poznawczą i przetwarzanie nagrody.

* **Mój genotyp (WGS):**
  VNTR 48-bp (egzon 3, 2R–11R) — brak bezpośredniego calla w WGS; proxy SNP:
  - `rs1800955` — **TC** (ensembl, high) — ensembl; depth=6,avg=6.0; gt=0/1
  - `rs3758653` — **TT** (ensembl, high) — ensembl; depth=9,avg=9.0; gt=0/0


* **Warianty Krótkie (np. 4R/4R - Typ dziki):** Prawidłowa czułość receptora na dopaminę. Stabilna samoregulacja, mniejsza potrzeba gratyfikacji zewnętrznej, wysoka efektywność uczenia się na błędach.
* **Warianty Pośrednie (2R/4R lub 4R/7R):** Umiarkowana modyfikacja plastyczności behawioralnej. Wyraźna zależność profilu od bodźców środowiskowych (hipoteza podatności różnicowej).
* **Wariant 7R/7R (Niska czułość receptora):** Tępa odpowiedź na dopaminę. Wymaga ekstremalnych stymulantów (sporty ekstremalne, hazard, wysokie ryzyko) do osiągnięcia satysfakcji. Silnie powiązany z ADHD, ale w wysoce wspierającym środowisku koreluje z wybitną kreatywnością i zdolnościami przywódczymi.

---
### DRD5

* **Główny symbol genu:** DRD5
* **Pełna nazwa biochemiczna:** Receptor dopaminergiczny D5 (ang. *Dopamine receptor D5*)
* **Nazwy potoczne i medialne:** Gen precyzyjnego dostrajania uwagi (kora przedczołowa i hipokamp)
* **Główny rsID / Wariant:** Polimorfizmy powtórzeń mikrosatelitarnych (marker dinukleotydowy przy promotorze)
* **Lokalizacja chromosomalna:** Chromosom 4 (4q16.1)
* **Rola biologiczna genu/białka:** Koduje receptor dopaminergiczny należący do rodziny D1-like (sprzężony z białkiem Gs, aktywujący cyklazę adenylową). Cechuje się unikalną właściwością – wykazuje znacznie wyższe powinowactwo do dopaminy oraz wyższą konstytutywną (spoczynkową) aktywność niż klasyczny receptor D1. Kluczowy dla stabilizacji pamięci roboczej i eliminacji szumu informacyjnego w korze przedczołowej.

* **Mój genotyp (WGS):**
  Mikrosatelita promotorowa — brak VNTR w WGS; proxy SNP:
  - `rs1800762` — **CC** (ensembl, high) — ensembl; depth=55,avg=55.0; gt=0/0
  - `rs10033951` — **CT** (ensembl, high) — ensembl; depth=35,avg=35.0; gt=0/1



**Warianty Standardowe (Brak alelu ryzyka)**
Optymalna gęstość i wysoka spoczynkowa aktywność receptora. Umożliwia precyzyjne działanie pamięci roboczej przy niskich (spoczynkowych) stężeniach dopaminy, zapewniając stabilne skupienie na zadaniu analitycznym.

**Warianty Pośrednie (Heterozygotyczne przesunięcia)**
Umiarkowana modyfikacja odporności na dystraktory środowiskowe.

**Obecność Alelu Ryzyka (np. Wariant mikrosatelitarny 148-bp)**
Powoduje zaburzenie gęstości ekspresji receptora D5 w strukturach przedczołowo-limbicznych. Jeden z najlepiej udokumentowanych i powtarzalnych wariantów ryzyka w **etiologii ADHD**. Skutkuje gwałtownym spadkiem efektywności pamięci roboczej i niemożnością utrzymania uwagi bez silnej stymulacji zewnętrznej.

---

## BLOK 12: Kaskada Neuroimmunologiczna, Zapalna i Obwodowa (Neuroinflammation)

---
### HTR1B

* **Główny symbol genu:** HTR1B
* **Pełna nazwa biochemiczna:** Receptor serotoninowy 1B (ang. *5-hydroxytryptamine receptor 1B*)
* **Nazwy potoczne i medialne:** Gen kontroli impulsów i zachowań aspołecznych, gen autoreceptora agresji
* **Główny rsID:** rs6296
* **Lokalizacja chromosomalna:** Chromosom 6 (6q14.1)
* **Rola biologiczna genu/białka:** Koduje receptor sprzężony z białkiem Gi, który pełni funkcję hamującego autoreceptora presynaptycznego. Lokalizuje się w terminalach aksonalnych (szczególnie w prążkowiu i korze czołowej). Jego aktywacja ogranicza dalszy wyrzut serotoniny, bezpośrednio regulując hamowanie behawioralne, poziom agresji oraz tolerancję na frustrację.

* **Mój genotyp (WGS):**
  - `rs6296` — **CC** (ensembl, high) — ensembl; depth=45,avg=45.0; gt=0/0
  - `rs6297` — **TT** (ensembl, high) — ensembl; depth=53,avg=53.0; gt=1/1



**Genotyp G/G (Wariant referencyjny / Stabilny)**
Zrównoważona ekspresja autoreceptora. Optymalny tonus serotoninowy, prawidłowa kontrola odruchów, niska skłonność do zachowań ryzykownych lub autoagresywnych.

**Genotyp A/G (Umiarkowana elastyczność)**
Standardowy profil adaptacyjny; prawidłowa kontrola afektu w większości sytuacji populacyjnych.

**Genotyp A/A (Wariant ryzyka / Niska kontrola impulsów)**
Zmieniona gęstość lub czułość receptorów, skutkująca rozregulowaniem pętli zwrotnej serotoniny. W badaniach neuropsychiatrycznych wariant ten silnie koreluje z impulsywnością, skłonnością do wybuchów gniewu, zachowaniami opozycyjno-buntowniczymi (ODD) oraz podatnością na uzależnienia behawioralne.

---

---
### HTR2A

* **Główny symbol genu:** HTR2A
* **Pełna nazwa biochemiczna:** Receptor serotoninowy 2A (ang. *5-hydroxytryptamine receptor 2A*)
* **Nazwy potoczne i medialne:** Gen neuroplastyczności, cel psychodelików i atypowych neuroleptyków
* **Główny rsID:** rs6311 (-1438G/A w promotorze) lub rs6313
* **Lokalizacja chromosomalna:** Chromosom 13 (13q14.2)
* **Rola biologiczna genu/białka:** Główny pobudzeniowy receptor serotoninowy w korze mózgowej. Reguluje uwalnianie glutaminianu i dopaminy, moduluje percepcję, wyobraźnię, lęk oraz procesy głębokiej przebudowy synaptycznej (neuroplastyczności).

* **Mój genotyp (WGS):**
  - `rs6311` — **CT** (ensembl, high) — ensembl; depth=68,avg=68.0; gt=0/1
  - `rs6313` — **GA** (ensembl, high) — ensembl; depth=69,avg=69.0; gt=0/1


* **Genotyp G/G (Wariant A/A w starym nazewnictwie - Wysoka gęstość receptorów):** Zwiększona ekspresja HTR2A. Ekstremalna wrażliwość na bodźce emocjonalne i społeczne. Wysoka podatność na traumę, ale też doskonała odpowiedź na terapię i substancje modulujące ten receptor.
* **Genotyp A/G (Ekspresja umiarkowana):** Zrównoważona gęstość receptorowa. Prawidłowe przetwarzanie afektu, stabilna odporność na zmienność środowiskową.
* **Genotyp A/A (Wariant G/G w starym nazewnictwie - Niska gęstość receptorów):** Obniżona ekspresja receptora. Niższa reaktywność na stres psychologiczny, wysoka stabilność emocjonalna, lecz potencjalnie mniejsza elastyczność poznawcza i słabsza odpowiedź na klasyczne leki SSRI.

---

## BLOK 2: Układ Glutaminianergiczny i Plastyczność Synaptyczna

---
### HTR4

* **Główny symbol genu:** HTR4
* **Pełna nazwa biochemiczna:** Receptor serotoninowy 4 (ang. *5-hydroxytryptamine receptor 4*)
* **Nazwy potoczne i medialne:** Gen neurogenezy hipokampa i optymizmu poznawczego
* **Główny rsID:** rs211105
* **Lokalizacja chromosomalna:** Chromosom 5 (5q32)
* **Rola biologiczna genu/białka:** Receptor postsynaptyczny sprzężony z białkiem Gs. Jego aktywacja stymuluje cyklazę adenylową, podnosząc poziom cAMP w neuronie. Wykazuje silną ekspresję w hipokampie, gdzie promuje długotrwałe wzmocnienie synaptyczne (LTP), sekrecję czynnika BDNF oraz stymuluje powstawanie nowych neuronów (neurogenezę dorosłą).

* **Mój genotyp (WGS):**
  - `rs211105` — **TT** (ensembl+bam, high) — marker_report
  - `rs2013162` — **CC** (ensembl, high) — ensembl; depth=42,avg=42.0; gt=0/0
  - `rs6440851` — **CC** (ensembl, high) — ensembl; depth=45,avg=45.0; gt=1/1



**Genotyp Wysokiej Aktywności (Prokognitywny / Rezylientny)**
Wydajna stymulacja szlaku cAMP-BDNF. Przyspieszona konsolidacja śladów pamięciowych, wysoka plastyczność hipokampa, doskonała naturalna odpowiedź na klasyczne leki przeciwdepresyjne i odporność na anhedonię.

**Genotyp Pośredni (Typ dziki)**
Prawidłowa plastyczność struktur hipokampa i standardowy profil uczenia się.

**Genotyp Niskiej Aktywności (Wariant ryzyka / Wolna neurogeneza)**
Obniżony potencjał aktywacji cAMP pod wpływem serotoniny. Skutkuje gorszą pamięcią przestrzenną i deklaratywną, trudnościami z elastycznym gaszeniem lęku oraz podwyższonym ryzykiem depresji endogennej odpornej na standardową terapię SSRI.

---

---
### HTR6

* **Główny symbol genu:** HTR6
* **Pełna nazwa biochemiczna:** Receptor serotoninowy 6 (ang. *5-hydroxytryptamine receptor 6*)
* **Nazwy potoczne i medialne:** Gen tonusu kognitywnego, cel nowoczesnych leków prokognitywnych
* **Główny rsID:** rs1131441 / rs3813929
* **Lokalizacja chromosomalna:** Chromosom 1 (1q32.1)
* **Rola biologiczna genu/białka:** Receptor postsynaptyczny sprzężony z białkiem Gs, stymulujący produkcję cAMP. Posiada unikalną, niemal wyłączną lokalizację w OUN (kora, hipokamp, jądra podstawne). Reguluje uwalnianie acetylocholiny i glutaminianu, wpływając bezpośrednio na plastyczność synaptyczną, pamięć roboczą i konsolidację śladów pamięciowych.

* **Mój genotyp (WGS):**
  - `rs1131441` — NOT_IN_DBSNP
  - `rs3813929` — **CC** (ensembl+bam, high) — ensembl; depth=20,avg=20.0; gt=0/0






**Genotyp Referencyjny (Optymalna gęstość receptorowa)**
Sprawna synteza wewnątrzkomórkowego cAMP pod wpływem serotoniny. Prawidłowe dawkowanie acetylocholiny w korze przedczołowej, wysoka sprawność zapamiętywania i elastyczność poznawcza.

**Genotyp Pośredni (Typ dziki)**
Standardowe parametry uczenia się i przetwarzania informacji w populacji ogólnej.

**Genotyp Niskiej Aktywności (Wariant ryzyka)**
Obniżona ekspresja HTR6. Skutkuje gorszymi wynikami w testach pamięciowych, osłabieniem procesów neuroplastyczności oraz wyższą podatnością na deficyty poznawcze obserwowane w schizofrenii i depresji.

---

---
### SLC6A2

* **Główny symbol genu:** SLC6A2
* **Pełna nazwa biochemiczna:** Transporter noradrenaliny (ang. *Solute carrier family 6 member 2*)
* **Nazwy potoczne i medialne:** Gen napędu psychoruchowego i uwagi, cel atomoksetyny
* **Główny rsID:** rs5569
* **Lokalizacja chromosomalna:** Chromosom 16 (16q12.2)
* **Rola biologiczna genu/białka:** Koduje transporter NET, usuwający noradrenaline (oraz częściowo dopaminę w korze przedczołowej) ze szczeliny synaptycznej. Kluczowy dla regulacji czuwania, skupienia oraz reakcji autonomicznych.

* **Mój genotyp (WGS):**
  - `rs5569` — **AA** (ensembl, high) — ensembl; depth=48,avg=48.0; gt=1/1
  - `rs3785157` — **TT** (ensembl, high) — ensembl; depth=45,avg=45.0; gt=1/1
* **Dopasowany profil:** Genotyp A/A (Wysoka wydajność NET):


* **Genotyp A/A (Wysoka wydajność NET):** Szybki wychwyt zwrotny noradrenaliny. Stabilność emocjonalna, mniejsza reaktywność lękowa, ale w niesprzyjających warunkach ryzyko apatii lub trudności z podtrzymaniem uwagi.
* **Genotyp A/G (Wydajność pośrednia):** Prawidłowa fizjologia przekaźnictwa noradrenergicznego. Optymalny balans pomiędzy czujnością a spokojem.
* **Genotyp G/G (Niska wydajność NET):** Wolniejsze usuwanie noradrenaliny. Wyższa spoczynkowa czujność, lepsza koncentracja w warunkach wymagających presji, ale też obniżony próg lękowy i tendencja do somatyzacji stresu (kołatania serca, drżenie rąk).

---
### SLC6A3

* **Główny symbol genu:** SLC6A3
* **Pełna nazwa biochemiczna:** Transporter dopaminy 1 (ang. *Solute carrier family 6 member 3*)
* **Nazwy potoczne i medialne:** Gen tempa oczyszczania dopaminy, gen ADHD
* **Główny rsID / Wariant:** Polimorfizm VNTR 40-bp w regionie 3'UTR (warianty 9R i 10R)
* **Lokalizacja chromosomalna:** Chromosom 5 (5p15.33)
* **Rola biologiczna genu/białka:** Koduje transporter DAT, który odpowiada za wychwyt zwrotny dopaminy ze szczeliny synaptycznej z powrotem do neuronu presynaptycznego (głównie w prążkowiu). Reguluje siłę i czas trwania sygnału dopaminergicznego.

* **Mój genotyp (WGS):**
  VNTR 40-bp (3'UTR, 9R/10R) — brak bezpośredniego calla w WGS; proxy SNP:
  - `rs27072` — **CC** (ensembl+bam, high) — ncbi; depth=22,avg=22.0; gt=0/0





* **9R/9R (Niska ekspresja DAT):** Wolniejszy wychwyt zwrotny. Wyższy podstawowy poziom dopaminy w synapsach prążkowia. Lepsza koncentracja i motywacja wewnętrzna, mniejsza impulsywność.
* **9R/10R (Ekspresja umiarkowana):** Zrównoważona dynamika dopaminergiczna. Dobra elastyczność behawioralna, optymalne zarządzanie układem nagrody.
* **10R/10R (Wysoka ekspresja DAT):** Błyskawiczne "wymiatanie" dopaminy z synaps. Chronicznie niski poziom dopaminy synaptycznej. Klasyczny marker podatności na ADHD, poszukiwanie silnych bodźców (novelty seeking) i osłabioną samokontrolę.

---

## CZĘŚĆ III. Glutaminian, GABA i plastyczność synaptyczna
### GABBR2

* **Główny symbol genu:** GABBR2
* **Pełna nazwa biochemiczna:** Receptor kwasu gamma-aminomasłowego typu B, podjednostka 2 (ang. *Gamma-aminobutyric acid type B receptor subunit 2*)
* **Nazwy potoczne i medialne:** Gen metabotropowego hamowania, strażnik tonusu neuroelektrycznego
* **Główny rsID:** rs1435252
* **Lokalizacja chromosomalna:** Chromosom 9 (9q22.33)
* **Rola biologiczna genu/białka:** Koduje podjednostkę heterodimerycznego receptora GABA-B. W przeciwieństwie do jonotropowego GABA-A, receptor GABA-B działa wolniej, lecz długofalowo – poprzez białka G (Gi/o) i aktywację kanałów potasowych GIRK. Wywołuje głęboką hiperpolaryzację, stabilizując błonę postsynaptyczną i zapobiegając uogólnionym wyładowaniom drgawkowym.

* **Mój genotyp (WGS):**
  `rs1435252` — **GG** (ensembl+bam, high) — marker_report


* **Genotyp Referencyjny (Wysoka sprawność hamowania wolnego):** Stabilna kontrola pobudliwości sieciowej. Doskonała zdolność do relaksacji układu nerwowego, prawidłowa jakość snu głębokiego (wolnofalowego), wysoka odporność na lęk napadowy.
* **Genotyp Pośredni (Heterozygota):** Standardowe, populacyjne parametry kinetyki hamowania metabotropowego.
* **Genotyp Niskiej Aktywności / Mutacje Patogenne:** Niewydolność długofalowego hamowania. Układ nerwowy nie potrafi utrzymać stanu wyciszenia. Skutkuje to permanentnym napięciem psychoruchowym, ciężką bezsennością, skłonnością do mioklonii i padaczki, a także koreluje ze sztywnością poznawczą i zachowaniami obsesyjnymi w spektrum autyzmu.

---

## BLOK 9: Kanały Jonowe i Homeostaza Elektrofizjologiczna

---
### GADL1

* **Główny symbol genu:** GADL1
* **Pełna nazwa biochemiczna:** Decarboksylaza kwasu glutaminowego-podobna 1 (ang. *Glutamate decarboxylase like 1*)
* **Nazwy potoczne i medialne:** Gen syntezy tauryny, marker predykcji odpowiedzi na lit
* **Główny rsID:** rs704180
* **Lokalizacja chromosomalna:** Chromosom 3 (3p24.2)
* **Rola biologiczna genu/białka:** Koduje enzym odpowiedzialny za syntezę tauryny (kwasu 2-aminoetanosulfonowego) oraz beta-alaniny z aminokwasów kwasowych. Tauryna w mózgu działa jako nadrzędny agonista receptorów GABA-A i glicynowych, stabilizując tonus neuroelektryczny i chroniąc neurony przed ekscytotoksycznością glutaminianu.

* **Mój genotyp (WGS):**
  `rs704180` — **AG** (ensembl+bam, high) — marker_report



**Genotyp T/T (Wysoka wydajność enzymatyczna)**
Optymalna wewnątrzkomórkowa synteza tauryny. Silne naturalne hamowanie synaptyczne, wysoki próg drgawkowy, doskonała ochrona neuronów przed przestymulowaniem.

**Genotyp C/T (Wariant pośredni)**
Zrównoważona pula tauryny; standardowa sprawność hamowania populacyjnego.

**Genotyp C/C (Niska wydajność / Sygnał ryzyka ChAD)**
Chroniczny niedobór tauryny w strukturach podkorowych. Skutkuje osłabieniem hamowania GABAergicznego i podatnością na destabilizację afektu. W badaniach farmakogenomicznych wariant ten jest kluczowym **markerem braku odpowiedzi terapeutycznej na jony litu** u pacjentów z chorobą afektywną dwubiegunową.

---

---
### GRIA2

* **Główny symbol genu:** GRIA2
* **Pełna nazwa biochemiczna:** Podjednostka 2 jonotropowego receptora glutaminianergicznego AMPA (ang. *Glutamate ionotropic receptor AMPA type subunit 2*)
* **Nazwy potoczne i medialne:** Strażnik przepuszczalności wapniowej, gen ochrony przed ekscytotoksycznością
* **Główny rsID / Wariant:** Witryna redagowania RNA (modyfikacja Q/R w egzonie 11) / Rzadkie mutacje de novo
* **Lokalizacja chromosomalna:** Chromosom 4 (4q32.1)
* **Rola biologiczna genu/białka:** Koduje kluczową podjednostkę (GluA2) receptora AMPA pośredniczącego w szybkiej neurotransmisji synaptycznej. Transkrypty GRIA2 podlegają niemal w 100% procesowi posttranskrypcyjnego redagowania RNA (zamiana aminokwasu Q na R). Ta pojedyncza zmiana sprawia, że receptor AMPA staje się **całkowicie nieprzepuszczalny dla jonów wapnia (Ca²⁺)**, co jest absolutnym fundamentem ochrony neuronów przed toksycznym zalaniem wapniowym.

* **Mój genotyp (WGS):**
  `rs13182811` — **CC** (ensembl+bam, high) — ensembl; depth=37,avg=37.0; gt=0/0






**Typ Dziki (Pełne, prawidłowe redagowanie RNA)**
Receptory AMPA przepuszczają wyłącznie jony sodu i potasu. Precyzyjna kontrola nad napływem wapnia (który może wchodzić tylko przez receptory NMDA). Prawidłowa plastyczność synaptyczna bez ryzyka śmierci komórkowej.

**Warianty populacyjne / Obniżony poziom ekspresji**
Subtelne wahania w gęstości postsynaptycznej receptorów, modyfikujące próg zmęczenia synaptycznego podczas intensywnej pracy intelektualnej.

**Mutacja Patogenna (Brak redagowania / Mutacje de novo utraty funkcji)**
Katastrofalne dopuszczenie do masowego napływu jonów Ca²⁺ przez receptory AMPA. Wywołuje permanentną ekscytotoksyczność. Objawia się ciężką, lekooporną padaczką, głęboką niepełnosprawnością intelektualną, wczesnym zanikiem neuronów ruchowych oraz nasilonym fenotypem neurorozwojowym (ASD).

---

---
### GRIN2A

* **Główny symbol genu:** GRIN2A
* **Pełna nazwa biochemiczna:** Podjednostka 1A receptora glutaminianergicznego NMDA (ang. *Glutamate ionotropic receptor NMDA type subunit 1A*)
* **Nazwy potoczne i medialne:** Gen integracji synaptycznej, marker GWAS schizofrenii
* **Główny rsID / Wariant:** rs11645009 / Rzadkie mutacje utraty funkcji (LoF)
* **Lokalizacja chromosomalna:** Chromosom 16 (16p13.2)
* **Rola biologiczna genu/białka:** Koduje podjednostkę GluN2A receptora NMDA. W miarę dojrzewania mózgu zastępuje ona podjednostkę GluN2B, co przyspiesza kinetykę deaktywacji kanału wapniowego, stabilizując dojrzałe synapsy i zamykając okresy krytyczne rozwoju.

* **Mój genotyp (WGS):**
  `rs11645009` — **TT** (ensembl+bam, high) — marker_report


* **Genotyp Referencyjny (Prawidłowy przełącznik GluN2B -> GluN2A):** Precyzyjne zamykanie faz rozwoju kory mózgowej. Optymalna filtracja sensoryczna, prawidłowe funkcjonowanie pamięci długotrwałej.
* **Warianty Polimorficzne (Sygnały GWAS):** Subtelne rozkoordynowanie kinetyki receptora. Może objawiać się przejściowymi trudnościami z koncentracją lub nadwrażliwością na szum informacyjny.
* **Mutacja Patogenna (LoF / Mutacje punktowe):** Ciężkie spektrum padaczkowo-afatyczne (np. zespół Landaua-Klefnera), głębokie opóźnienie rozwoju mowy, dysfazja oraz drastycznie podwyższone ryzyko schizofrenii i zachowań psychotycznych.

---
### GRM3

* **Główny symbol genu:** GRM3
* **Pełna nazwa biochemiczna:** Metabotropowy receptor glutaminianergiczny 3 (ang. *Glutamate metabotropic receptor 3*)
* **Nazwy potoczne i medialne:** Gen kontroli ekscytotoksyczności, gen ochrony neuronów
* **Główny rsID:** rs6465084
* **Lokalizacja chromosomalna:** Chromosom 7 (7q21.11-q21.12)
* **Rola biologiczna genu/białka:** Koduje autoreceptor presynaptyczny mGluR3 sprzężony z białkiem Gi. Jego aktywacja hamuje nadmierne uwalnianie glutaminianu do szczeliny synaptycznej, pełniąc rolę nadrzędnego bezpiecznika chroniącego neurony przed toksycznym przestymulowaniem.

* **Mój genotyp (WGS):**
  `rs6465084` — **AA** (ensembl+bam, high) — ensembl; depth=60,avg=60.0; gt=0/0
* **Dopasowany profil:** Genotyp A/A (Wysoka ekspresja autoreceptora):





* **Genotyp A/A (Wysoka ekspresja autoreceptora):** Wydajna kontrola homeostazy glutaminianu. Wysoka odporność na stres neurobiologiczny, doskonała ochrona przed neurodegeneracją.
* **Genotyp A/G (Ekspresja zrównoważona):** Prawidłowa regulacja neurotransmisji pobudzającej.
* **Genotyp G/G (Niska ekspresja / Wariant ryzyka schizofrenii):** Słaba czułość "hamulca" presynaptycznego. Przewlekły wyciek glutaminianu, zaburzenia stosunku sygnału do szumu (signal-to-noise ratio) w korze przedczołowej. Skorelowany z gorszą pamięcią roboczą i podatnością na psychozy.

---

## BLOK 3: Neurorozwój, Architektura Mózgu i Sygnały GWAS

---
### GRM5

* **Główny symbol genu:** GRM5
* **Pełna nazwa biochemiczna:** Metabotropowy receptor glutaminianergiczny 5 (ang. *Glutamate metabotropic receptor 5*)
* **Nazwy potoczne i medialne:** Receptor mGluR5, gen osi pobudzenia postsynaptycznego, molekularny klucz Zespołu Łamliwego X
* **Główny rsID:** rs362990
* **Lokalizacja chromosomalna:** Chromosom 11 (11q14.2-q14.3)
* **Rola biologiczna genu/białka:** Receptor mGluR5 (sprzężony z białkiem Gq/11) jest zlokalizowany postsynaptycznie, głównie w gęstości postsynaptycznej (PSD). Jego aktywacja stymuluje fosfolipazę C, prowadząc do uwolnienia wapnia z zapasów wewnątrzkomórkowych i aktywacji lokalnej translacji białek synaptycznych. Jest nadrzędnym modulatorem długotrwałego osłabienia synaptycznego (LTD) oraz plastyczności synaps.

* **Mój genotyp (WGS):**
  - `rs362990` — **TT** (ensembl+bam, high) — ensembl; depth=52,avg=52.0; gt=1/1
  - `rs362584` — **AA** (ensembl, high) — ensembl; depth=45,avg=45.0; gt=1/1
  - `rs2129571` — NOT_IN_DBSNP (martwy tag w panelu)


**A/A (Zbalansowana odpowiedź mGluR5)**
Optymalna plastyczność synaptyczna. Prawidłowy przebieg procesów LTD i LTP, wysoka zdolność adaptacji kognitywnej i prawidłowa synteza białek strukturalnych synaps.

**A/T (Umiarkowane przesunięcia ekspresji)**
Drobne różnice w prędkości adaptacji sensorycznej i podatności na zmęczenie poznawcze.

**T/T (Homozygota alleliczna T)**
Typowy profil populacyjny mGluR5 przy homozygotii allelicznej T. Bez cech patologicznego rozprzężenia szlaku (patologiczna nadekspresja mGluR5 dotyczy głównie braku FMRP w zespole łamliwego X, nie tego tag-SNP).

---
### SLC12A5

* **Główny symbol genu:** SLC12A5
* **Pełna nazwa biochemiczna:** Kotransporter potasowo-chlorkowy 2 (ang. *Solute carrier family 12 member 5*)
* **Nazwy potoczne i medialne:** Gen dojrzałości układu GABA, "wielki przełącznik" pobudzenia
* **Główny rsID / Wariant:** Warianty eQTL / Rzadkie mutacje punktowe de novo
* **Lokalizacja chromosomalna:** Chromosom 20 (20q13.12)
* **Rola biologiczna genu/białka:** Koduje transporter KCC2, który wypompowuje jony chlorkowe ($Cl^-$) z wnętrza neuronu na zewnątrz. Jest to absolutnie fundamentalne: wysoka aktywność KCC2 w dojrzałym mózgu sprawia, że otwarcie kanału GABA-A powoduje napływ $Cl^-$ do środka i **hiperpolaryzację (hamowanie neuronu)**. W niedojrzałym mózgu lub przy dysfunkcji KCC2, GABA działa paradoksalnie – pobudzająco!

* **Mój genotyp (WGS):**
  `rs2885267` — **AG** (ensembl+bam, high) — ncbi; depth=53,avg=53.0; gt=0/1





* **Typ Dziki (Wysoka ekspresja KCC2):** Prawidłowy, niski poziom chlorków wewnątrzkomórkowych. Układ GABA działa doskonale hamująco – zapewnia wyciszenie, redukcję lęku, relaksację mięśniową i stabilność neuroelektryczną.
* **Warianty Polimorficzne (Suboptymalna ekspresja):** Lekkie osłabienie potencjału hamującego GABA. Objawia się gorszą tolerancją na stres sensoryczny i podatnością na stany lękowe przy zmęczeniu.
* **Obniżona Ekspresja / Mutacja Patogenna (Niski eQTL / LoF):** Katastrofalny wzrost stężenia chlorków w neuronach. Układ GABA traci funkcję hamującą (lub zaczyna pobudzać). Skutkuje to ciężką, lekooporną padaczką dziecięcą, permanentnym przebodźcowaniem sensorycznym, brakiem możliwości filtracji informacji i drastycznym nasileniem cech autystycznych (ASD).

---

## CZĘŚĆ IV. Receptory cholinergiczne, kanabinoidowe i histaminowe
### CHRNA2

* **Główny symbol genu:** CHRNA2
* **Pełna nazwa biochemiczna:** Podjednostka alfa-2 nikotynowego receptora acetylocholiny (ang. *Cholinergic receptor nicotinic alpha 2 subunit*)
* **Nazwy potoczne i medialne:** Gen podatności na ciężkie uzależnienia stymulujące
* **Główny rsID:** rs144754751 / rs6498305
* **Lokalizacja chromosomalna:** Chromosom 8 (8p21.2)
* **Rola biologiczna genu/białka:** Koduje podjednostkę receptorów nikotynowych $a2$, obecnych m.in. w interneuronach hamujących kory i prążkowia. Moduluje uwalnianie somatostatyny, GABA i dopaminy, pełniąc funkcję stabilizatora pętli sprzężenia zwrotnego w obwodach nagrody i kontroli impulsów.

* **Mój genotyp (WGS):**
  `rs144754751` — **CC** (ensembl+bam, high) — marker_report



**Genotyp Referencyjny (Standardowa ekspresja)**
Zrównoważona gęstość receptorowa. Prawidłowe hamowanie i pobudzanie w układzie nagrody, niska skłonność do zachowań nałogowych i kompulsywnego poszukiwania chemicznych stymulantów.

**Genotyp Pośredni (Wariant populacyjny)**
Normalna wrażliwość na stymulację cholinergiczną; standardowy próg satysfakcji dopaminergicznej.

**Genotyp Ryzyka (Drastycznie obniżona ekspresja receptora)**
Osłabienie "hamulca" cholinergicznego w obwodach motywacyjnych. W badaniach neuropsychiatrycznych wariant ten wykazuje potężną asocjację z wysokim ryzykiem ciężkiego uzależnienia od nikotyny, konopi indyjskich oraz z ogólną podatnością na polilekomanię i zachowania impulsywne.

---

---
### CHRNA4

* **Główny symbol genu:** CHRNA4
* **Pełna nazwa biochemiczna:** Podjednostka alfa-4 nikotynowego receptora acetylocholiny (ang. *Cholinergic receptor nicotinic alpha 4 subunit*)
* **Nazwy potoczne i medialne:** Gen nocnej padaczki czołowej, modulator tonusu uwagi i czujności kory
* **Główny rsID / Wariant:** rs1044396 / Mutacje missens w pętli kanałowej
* **Lokalizacja chromosomalna:** Chromosom 20 (20q13.33)
* **Rola biologiczna genu/białka:** Koduje podjednostkę alfa-4, która wspólnie z podjednostką beta-2 tworzy najpowszechniejszy i wysoce czuły receptor nikotynowy (α4β2) w ośrodkowym układzie nerwowym. Receptor ten reguluje presynaptyczny wyrzut dopaminy, GABA i acetylocholiny, decydując o wzmocnieniu uwagi, pamięci roboczej i stabilności snu.

* **Mój genotyp (WGS):**
  - `rs1044396` — **GG** (ensembl, high) — ensembl; depth=33,avg=33.0; gt=0/0
  - `rs2273502` — **TT** (ensembl, high) — ensembl; depth=45,avg=45.0; gt=1/1



**Genotyp T/T (Wysoka wydajność receptorowa)**
Prawidłowa, silna ekspresja receptorów α4β2. Wysoki poziom skupienia uwagi, świetna pamięć robocza, dobra stabilizacja faz snu i wysoka efektywność stymulacji kognitywnej.

**Genotyp C/T (Wariant pośredni)**
Zrównoważona gęstość receptorów cholinergicznych; standardowy profil uwagi populacyjnej.

**Genotyp C/C lub Mutacje Patogenne (Niska ekspresja / ADNFLE)**
Znaczne obniżenie tonusu acetylocholinergicznego. Powoduje deficyty uwagi (korelacja z ADHD) oraz tendencję do wybudzeń. Specyficzne mutacje punktowe w tym genie są bezpośrednią przyczyną **Autosomalnej Dominującej Nocnej Padaczki z Napadami z Kory Czołowej (ADNFLE)**, charakteryzującej się gwałtownymi, hiperkinetycznymi napadami ruchowymi wyłącznie w trakcie snu.

---

---
### CHRNA7

* **Główny symbol genu:** CHRNA7
* **Pełna nazwa biochemiczna:** Podjednostka alfa-7 nikotynowego receptora acetylocholiny (ang. *Cholinergic receptor nicotinic alpha 7 subunit*)
* **Nazwy potoczne i medialne:** Gen bramkowania sensorycznego, gen krytycznego regionu 15q13.3
* **Główny rsID / Wariant:** rs2337980 / Mikrodelecje regionu 15q13.3
* **Lokalizacja chromosomalna:** Chromosom 15 (15q13.3)
* **Rola biologiczna genu/białka:** Koduje homopentameryczny receptor nikotynowy ($a7$ nAChR), cechujący się wysoką przepuszczalnością dla jonów wapnia ($Ca^{2+}$). Odpowiada za mechanizm bramkowania sensorycznego (ang. *sensory gating*) – zdolność mózgu do odfiltrowywania powtarzalnych, nieistotnych bodźców z tła, chroniąc korę przed przeciążeniem.

* **Mój genotyp (WGS):**
  - `rs2337980` — **CC** (ensembl+bam, high) — marker_report
  - `rs2064070` — **TT** (ensembl, high) — ensembl; depth=24,avg=24.0; gt=1/1
  - `rs6494223` — **CC** (ensembl, high) — ensembl; depth=14,avg=14.0; gt=0/0


* **Genotyp Referencyjny (Wysoka ekspresja receptorów):** Sprawne bramkowanie sensoryczne (prawidłowy test tłumienia P50). Doskonała koncentracja selektywna, odporność na szum informacyjny i wysoka wydajność pamięci roboczej.
* **Genotyp Pośredni / Polimorfizmy eQTL:** Umiarkowane fluktuacje w gęstości receptorowej, dające okresową podatność na dystrakcję pod wpływem zmęczenia lub przebodźcowania.
* **Niska Ekspresja / Mikrodelecja 15q13.3 (Patologia):** Całkowite załamanie filtracji sensorycznej. Mózg zalewany jest chaotycznymi bodźcami. Skutkuje to deficytami uwagi, trudnościami integracji sensorycznej w ASD oraz drastycznym wzrostem podatności na schizofrenię i epizody psychotyczne.

---
### CNR1

* **Główny symbol genu:** CNR1
* **Pełna nazwa biochemiczna:** Receptor kanabinoidowy 1 (ang. *Cannabinoid receptor 1*)
* **Nazwy potoczne i medialne:** Gen układu endokanabinoidowego, receptor CB1, modulator wygaszania lęku
* **Główny rsID:** rs2020720 / rs6454674
* **Lokalizacja chromosomalna:** Chromosom 6 (6q15)
* **Rola biologiczna genu/białka:** CB1 to jeden z najliczniejszych receptorów sprzężonych z białkiem G (Gi/o) w mózgu. Działa presynaptycznie – jego aktywacja przez endokanabinoidy (anandamid, 2-AG) hamuje wstecznie uwalnianie innych neurotransmiterów (GABA lub glutaminianu). Reguluje plastyczność synaptyczną, odczuwanie bólu, apetyt oraz wygaszanie negatywnych wspomnień lękowych.

* **Mój genotyp (WGS):**
  - `rs2020720` — NOT_IN_DBSNP
  - `rs2023239` — **TT** (ensembl, high) — ensembl; depth=53,avg=53.0; gt=0/0
  - `rs806368` — **TC** (ensembl, high) — ensembl; depth=43,avg=43.0; gt=0/1


* **Genotyp Standardowy (Optymalna gęstość CB1):** Prawidłowa dynamika retrogradacji synaptycznej. Sprawne wygaszanie reakcji lękowych na bodźce, które przestały stanowić zagrożenie; zrównoważony próg odczuwania przyjemności (hedonizm).
* **Warianty Populacyjne (Umiarkowane przesunięcia ekspresji):** Niewielkie zmiany w zapotrzebowaniu na stymulację układu nagrody; zmienna podatność na stany lękowe w warunkach stresu.
* **Genotyp Ryzyka (Niska ekspresja / Obniżone powinowactwo):** Utrata elastyczności endokanabinoidowej. Niemożność biochemicznego "wygaszenia" utrwalonych śladów pamięci lękowej (skłonność do PTSD, lęku uogólnionego), podatność na anhedonię oraz wysokie genetyczne ryzyko rozwoju uzależnień (od konopi, alkoholu, jedzenia).

---
### HRH3

* **Główny symbol genu:** HRH3
* **Pełna nazwa biochemiczna:** Receptor histaminowy H3 (ang. *Histamine receptor H3*)
* **Nazwy potoczne i medialne:** Gen kontroli snu, czuwania i neurotransmisji kognitywnej
* **Główny rsID:** rs3787429
* **Lokalizacja chromosomalna:** Chromosom 20 (20q13.33)
* **Rola biologiczna genu/białka:** Działa jako presynaptyczny autoreceptor (ograniczający uwalnianie histaminy) oraz heteroreceptor (hamujący wydzielanie acetylocholiny, dopaminy, serotoniny i noradrenaliny). Pełni funkcję nadrzędnego regulatora tonusu kognitywnego i stabilności cyklu sen-czuwanie.

* **Mój genotyp (WGS):**
  - `rs3787429` — NOT_IN_DBSNP
  - `rs3743419` — **GG** (ensembl, high) — ensembl; depth=49,avg=49.0; gt=1/1
  - `rs16840066` — **AA** (ensembl, high) — ensembl; depth=74,avg=74.0; gt=0/0


* **Genotyp Referencyjny (Zbalansowane sprzężenie zwrotne):** Prawidłowy rytm dobowy. Wysoka ostrość umysłu w fazie czuwania, głęboka regeneracja w fazie snu, stabilna gospodarka neurotransmiterowa.
* **Genotyp Pośredni (Wariant populacyjny):** Standardowa elastyczność przystosowawcza do zmian faz dobowych (np. tolerancja na jet-lag).
* **Genotyp Ryzyka (Niezrównoważona nadekspresja H3):** Permanentne, nadmierne blokowanie uwalniania acetylocholiny i dopaminy w korze mózgowej. Skutkuje chroniczną sennością, deficytami pamięci roboczej, zaburzeniami snu REM i silną korelacją z zaburzeniami nastroju oraz deficytami poznawczymi w ADHD.

---

## BLOK 8: Sygnalizacja Wewnątrzkomórkowa i Kaskady Przeżywalności Neuronów

---
### MCHR1

* **Główny symbol genu:** MCHR1
* **Pełna nazwa biochemiczna:** Receptor hormonu koncentrującego melaninę 1 (ang. *Melanin concentrating hormone receptor 1*)
* **Nazwy potoczne i medialne:** Gen osi snu REM i lęku metabolicznego
* **Główny rsID:** rs13306893
* **Lokalizacja chromosomalna:** Chromosom 22 (22q13.2)
* **Rola biologiczna genu/białka:** Receptor sprzężony z białkiem G (Gi/o lub Gq), wykazujący silną ekspresję w podwzgórzu, ciele migdałowatym i jądrze półleżącym. Układ MCH promuje przyjmowanie pokarmów (oreksygenny) oraz inicjację i podtrzymanie snu REM. Moduluje zachowania lękowe, łącząc homeostazę energetyczną z przetwarzaniem emocjonalnym.

* **Mój genotyp (WGS):**
  `rs13306893` — **TC** (ensembl+bam, high) — marker_report



**Typ Dziki (Zbalansowana sygnalizacja MCH)**
Prawidłowa struktura snu z regularnymi fazami REM. Stabilny bilans energetyczny i prawidłowa adaptacja emocjonalna po odpoczynku.

**Warianty Populacyjne (Umiarkowane przesunięcia)**
Drobne wahania w długości trwania snu marzeń sennych; zmienna tendencja do jedzenia afektywnego.

**Wariant Ryzyka / Obniżona Ekspresja**
Rozregulowanie fazy REM (skrócenie lub chaotyczna fragmentacja snu). Skutkuje nasilonym lękiem wolnopłynącym, przewlekłym zmęczeniem psychicznym oraz tendencją do kompulsywnego podjadania w stanach deprywacji snu.

---

---

## CZĘŚĆ V. Neurorozwój poligenowy — GWAS, TWAS i architektura mózgu
### ADGRL3

* **Główny symbol genu:** ADGRL3
* **Pełna nazwa biochemiczna:** Receptor adhezyjny sprzężony z białkiem G L3 (ang. *Adhesion G protein-coupled receptor L3*)
* **Nazwy potoczne i medialne:** Główny gen podatności na ADHD
* **Główny rsID:** rs6551665
* **Lokalizacja chromosomalna:** Chromosom 4 (4q13.1)
* **Rola biologiczna genu/białka:** Koduje latrofilinę-3, receptor adhezyjny niezbędny do tworzenia, naprowadzania i strukturalnego dopasowania synaps w układzie dopaminergicznym (szczególnie na osi kora-prążkowie).

* **Mój genotyp (WGS):**
  - `rs6551665` — **GG** (ensembl, high) — ensembl; depth=43,avg=43.0; gt=0/0
  - `rs1397547` — **CC** (ensembl, high) — ensembl; depth=40,avg=40.0; gt=1/1


* **Genotyp T/T (Typ dziki):** Stabilna architektura połączeń synaptycznych w prążkowiu. Prawidłowa kontrola impulsów i wysoka zdolność do podtrzymywania uwagi.
* **Genotyp C/T (Ryzyko umiarkowane):** Drobne wahania w gęstości synaptycznej; profil wykazuje wyższą zależność od stymulacji środowiskowej.
* **Genotyp C/C (Wariant wysokiego ryzyka):** Wyraźne zmniejszenie gęstości połączeń synaptycznych w rejonach przedczołowych. W badaniach genomicznych potężnie podwyższa ryzyko ciężkiego fenotypu ADHD, zachowań opozycyjno-buntowniczych (ODD) oraz skłonności do uzależnień.

---
### AKAP11

* **Główny symbol genu:** AKAP11
* **Pełna nazwa biochemiczna:** Białko kotwiczące kinazę A 11 (ang. *A-kinase anchoring protein 11*)
* **Nazwy potoczne i medialne:** Gen stabilizacji nastroju, nowo odkryty "master gene" ChAD
* **Główny rsID / Wariant:** rs10936545 (tag-SNP) / mutacje LoF (osobno, sekwencjonowanie)
* **Lokalizacja chromosomalna:** Chromosom 13 (13q14.11)
* **Rola biologiczna genu/białka:** AKAP11 działa jako rusztowanie molekularne, które fizycznie wiąże kinazę białkową A (PKA) z kinazą GSK3B (głównym celem terapeutycznym litu). Reguluje wewnątrzkomórkowe szlaki sygnałowe odpowiedzialne za przeżywalność neuronów i plastyczność synaptyczną.

* **Mój genotyp (WGS):**
  `rs10936545` — **TT** (ensembl+bam, high) — ncbi; depth=69,avg=69.0; gt=1/1


**C/C (Typ referencyjny)**
Prawidłowa interakcja PKA-GSK3B. Stabilna neuroprotekcja, wysoka odporność na cykliczne zaburzenia afektywne.

**C/T (Wariant heterozygoticzny)**
Subtelne zmiany w dynamice przewodnictwa sygnałów, mogące korelować z okresową podatnością na wahania napędu psychoruchowego.

**T/T (Homozygota alleliczna T)**
Typowy profil populacyjny tag-SNP rs10936545. Nie oznacza mutacji LoF ani klinicznego ChAD — patogenne LoF wymagają osobnej analizy sekwencji/CNV.

---
### AKT3

* **Główny symbol genu:** AKT3
* **Pełna nazwa biochemiczna:** Kinaza białkowa B, składowa 3 (ang. *AKT serine/threonine kinase 3*)
* **Nazwy potoczne i medialne:** Gen objętości mózgu, regulator szlaku mTOR
* **Główny rsID / Wariant:** Warianty CNV (duplikacje/delecje) / rs1441334
* **Lokalizacja chromosomalna:** Chromosom 1 (1q43-q44)
* **Rola biologiczna genu/białka:** Jeden z trzech kluczowych homologów kinazy AKT, wykazujący dominującą ekspresję w tkance mózgowej. Koordynuje wzrost komórkowy, przeżywalność neuronów, synaptogenezę oraz architektonikę korową poprzez bezpośrednią aktywację kompleksu mTORC1.

* **Mój genotyp (WGS):**
  - `rs1441334` — NOT_IN_DBSNP
  - `rs4599042` — **CC** (ensembl+bam, high) — ncbi; depth=34,avg=34.0; gt=0/0





* **Typ Dziki (Zbalansowany sygnał wzrostowy):** Prawidłowa objętość istoty szarej, symetryczny wzrost półkul mózgowych, stabilne i precyzyjne formowanie sieci synaptycznych.
* **Warianty Polimorficzne (Sygnały GWAS):** Subtelne różnice w tempie proliferacji, korelujące z ogólną sprawnością przetwarzania informacji bez anomalii strukturalnych.
* **Mutacja Patogenna / CNV (Delecja lub Duplikacja):** Drastyczne zaburzenie rozmiaru mózgu. Utrata funkcji (LoF/delecja) skutkuje ciężką mikrocefalią (małogłowiem) i niedorozwojem psychoruchowym. Z kolei nadekspresja (duplikacja) prowadzi do megalencefalii (wielkogłowia), lekoopornej padaczkowej encefalopatii oraz głębokiego podtypu ASD.

---
### ARTN

* **Główny symbol genu:** ARTN
* **Pełna nazwa biochemiczna:** Artemina (ang. *Artemin*)
* **Nazwy potoczne i medialne:** Czynnik przetrwania neuronów czuciowych, gen rezyliencji na ból obwodowy
* **Główny rsID:** rs2516480
* **Lokalizacja chromosomalna:** Chromosom 1 (1p34.1)
* **Rola biologiczna genu/białka:** Artemina należy do rodziny czynników neurotroficznych GDNF (GFRa3/RET). Jest nadrzędnym czynnikiem warunkującym przeżywalność, wzrost i naprowadzanie aksonów neuronów czuciowych (nocyceptywnych) w zwojach korzeni grzbietowych oraz neuronów współczulnych, precyzyjnie kalibrując obwodowy próg bólu.

* **Mój genotyp (WGS):**
  `rs2516480` — **CC** (ensembl+bam, low) — ensembl; depth=5,avg=5.0; gt=0/0






**Typ Dziki (Prawidłowa trofika czuciowa)**
Optymalna gęstość i prawidłowa struktura włókien nocyceptywnych. Zrównoważony, fizjologiczny próg odczuwania bólu i bodźców termicznych.

**Warianty Populacyjne (Umiarkowane)**
Standardowe różnice osobnicze w tolerancji na ból fizyczny i bodźce dotykowe.

**Wariant Ryzyka (Niska ekspresja Arteminy)**
Przedwczesna degradacja lub nieprawidłowe naprowadzanie włókien czuciowych. Skutkuje patologicznym obniżeniem progu bólu (allodynia, hiperalgezja) oraz ekstremalną tkliwością na dotyk (nadwrażliwość taktylna), będącą powszechnym komponentem dysfunkcji sensorycznych w ASD.

---

---
### ASTN2

* **Główny symbol genu:** ASTN2
* **Pełna nazwa biochemiczna:** Astrotaktyna 2 (ang. *Astrotactin 2*)
* **Nazwy potoczne i medialne:** Gen migracji neuronów i rozwoju móżdżku
* **Główny rsID / Wariant:** rs17631335 / Delecja eksonowa (wariant strukturalny CNV)
* **Lokalizacja chromosomalna:** Chromosom 9 (9q21.13)
* **Rola biologiczna genu/białka:** Białko zaangażowane w endocytozę i recykling cząsteczek adhezji komórkowej (takich jak astrotaktyna 1 i integryny). Reguluje fizyczne przemieszczanie się (migrację) młodych neuronów wzdłuż gleju promienistego do docelowych warstw kory mózgowej oraz móżdżku w trakcie rozwoju embrionalnego.

* **Mój genotyp (WGS):**
  - `rs17631335` — **CC** (ensembl+bam, high) — marker_report
  - `rs6470054` — **GA** (ensembl, high) — ensembl; depth=49,avg=49.0; gt=0/1
  - `rs1857050` — **TT** (ensembl, high) — ensembl; depth=41,avg=41.0; gt=0/0


* **Typ Dziki (Prawidłowa migracja warstwowa):** Precyzyjna, warstwowa struktura kory mózgowej i móżdżku. Bezbłędne uformowanie makrosieci neuronalnych, prawidłowa integracja sensoryczno-motoryczna.
* **Warianty Polimorficzne (Modyfikujące):** Subtelne przesunięcia w tempie migracji komórkowej, mogące nieznacznie modyfikować profil przetwarzania bodźców motorycznych i równowagi.
* **Rearanżacja Patogenna (CNV / Delecja ASTN2):** Uszkodzenie strukturalne genu prowadzi do zatrzymania lub zaburzenia topografii migracji neuronów. Skutkuje to dysplazją korową lub hipoplazją móżdżku. Klinicznie manifestuje się jako silny fenotyp ADHD, globalne opóźnienie rozwoju, zaburzenia mowy, dyspraksja oraz cechy ASD.

---
### ATP2A2

* **Główny symbol genu:** ATP2A2
* **Pełna nazwa biochemiczna:** Magnezowo-zależna ATP-aza transportująca wapń 2 (ang. *ATPase sarcoplasmic/endoplasmic reticulum ca2+ transporting 2*)
* **Nazwy potoczne i medialne:** Pompa SERCA2, gen homeostazy wapniowej retikulum, gen choroby Dariera
* **Główny rsID / Wariant:** Mutacje patogenne utraty funkcji / rs2070366
* **Lokalizacja chromosomalna:** Chromosom 12 (12q24.11)
* **Rola biologiczna genu/białka:** Koduje pompę SERCA2, która aktywnie (zużywając ATP) przepompowuje jony wapnia ($Ca^{2+}$) z cytoplazmy z powrotem do wnętrza siateczki śródplazmatycznej (retikulum endoplazmatycznego). Precyzyjnie wygasza w ten sposób sygnały wapniowe po pobudzeniu neuronu i zapobiega wewnątrzkomórkowej toksyczności wapniowej.

* **Mój genotyp (WGS):**
  `rs2070366` — **CC** (ensembl+bam, high) — ensembl; depth=62,avg=62.0; gt=0/0





* **Typ Dziki (Wysoka wydajność SERCA2):** Błyskawiczny powrót do spoczynkowych poziomów wapnia w cytoplazmie. Prawidłowe funkcjonowanie neurosekrecji, stabilność mitochondrialna, wysoka ochrona przed stresem komórkowym.
* **Warianty Polimorficzne (Średnia wydajność):** Standardowy profil populacyjny; drobne modyfikacje w komórkowej odporności na przeciążenia metaboliczne.
* **Mutacja Patogenna (Heterozygota LoF):** Wywołuje **chorobę Dariera** (keratoza grudkowa), której bardzo często (u ok. 30-40% pacjentów) towarzyszy specyficzny fenotyp neuropsychiatryczny: skrajnie wysoka podatność na chorobę afektywną dwubiegunową, epizody ciężkiej psychozy, zaburzenia lękowe oraz niepełnosprawność intelektualną spowodowaną przewlekłym stresem retikulum w komórkach mózgowych.

---

## BLOK 10: Adhezja Komórkowa, Migracja i Architektura Macierzy Zewnątrzkomórkowej

---
### BCL11B

* **Główny symbol genu:** BCL11B
* **Pełna nazwa biochemiczna:** Czynnik transkrypcyjny BCL11B (ang. *B-cell lymphoma/leukemia 11B*)
* **Nazwy potoczne i medialne:** Gen tożsamości neuronów projekcyjnych kory, gen zespołu CANDLE / marokańskiego
* **Główny rsID / Wariant:** Mutacje punktowe *de novo* typu utraty funkcji (LoF) lub warianty strukturalne
* **Lokalizacja chromosomalna:** Chromosom 14 (14q32.2)
* **Rola biologiczna genu/białka:** BCL11B to nadrzędny regulator epigenetyczny i czynnik transkrypcyjny kontrolujący różnicowanie, rozwój aksonów i podtrzymanie tożsamości neuronów projekcyjnych głębokich warstw kory mózgowej (warstwa V – wysyłająca połączenia korowo-rdzeniowe) oraz jądra półleżącego. Jest również absolutnie niezbędny do prawidłowego rozwoju limfocytów T w układzie odpornościowym.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Typ Dziki (Prawidłowa ekspresja)**
Precyzyjne, warstwowe uformowanie głębokich struktur kory nowej oraz dróg eferentnych. Prawidłowy, zbalansowany rozwój neurobehawioralny i pełna dojrzałość immunologiczna.

**Warianty Populacyjne (Łagodne przesunięcia)**
Subtelne modyfikacje w gęstości połączeń prążkowiowych i korowych, przekładające się na indywidualne cechy motoryki małej lub profilu odpornościowego bez cech chorobowych.

**Mutacja Patogenna (Heterozygota LoF / Zespół CANDLE)**
Zablokowanie różnicowania neuronów warstwy V i niedorozwój limfocytów. Skutkuje ciężkim zespołem neurorozwojowym: głębokim opóźnieniem rozwoju mowy, hiponią mięśniową, niepełnosprawnością intelektualną, anomaliami zębowo-twarzowymi oraz wysokim nasileniem cech ze spektrum autyzmu (ASD).

---

---
### DUSP6

* **Główny symbol genu:** DUSP6
* **Pełna nazwa biochemiczna:** Fosfataza o podwójnej specyficzności 6 (ang. *Dual specificity phosphatase 6*)
* **Nazwy potoczne i medialne:** Główny hamulec szlaku ERK/MAPK, regulator lęku i neurogenezy
* **Główny rsID:** rs6583347
* **Lokalizacja chromosomalna:** Chromosom 12 (12q21.33)
* **Rola biologiczna genu/białka:** DUSP6 to negatywny regulator (fosfataza) szlaku sygnałowego ERK/MAPK, który jest aktywowany przez czynniki wzrostu (BDNF, FGF). Kontroluje proliferację i różnicowanie neuronów w niszach neurogennych oraz odgrywa kluczową rolę w wygaszaniu reakcji lękowych w strukturach limbicznych.

* **Mój genotyp (WGS):**
  `rs6583347` — **GG** (ensembl+bam, medium) — ncbi; depth=8,avg=8.0; gt=0/0






**Genotyp Wysokiej Aktywności (Silne tłumienie szlaku ERK)**
Rygorystyczne hamowanie kaskady MAPK. Może nieznacznie ograniczać podstawową neuroplastyczność, lecz skutecznie chroni sieć neuronalną przed patologiczną nadreaktywnością i lękiem.

**Genotyp Umiarkowany (Typ dziki)**
Zrównoważony, elastyczny tonus neuroplastyczny szlaku ERK w normie populacyjnej.

**Genotyp Niskiej Aktywności (Wariant ryzyka afektywnego)**
Osłabienie hamowania szlaku MAPK. Prowadzi do niekontrolowanej, chaotycznej aktywacji plastyczności w ciele migdałowatym i strukturach limbicznych. Silnie skorelowany z podatnością na przewlekłe stany lękowe, niestabilnością nastroju w ChAD oraz zachowaniami impulsywnymi.

---

---
### FOXP2

* **Główny symbol genu:** FOXP2
* **Pełna nazwa biochemiczna:** Białko Forkhead-box P2 (ang. *Forkhead box P2*)
* **Nazwy potoczne i medialne:** "Gen mowy i języka", gen ewolucji ludzkiej
* **Główny rsID / Wariant:** rs121912959 / Mutacje punktowe lub delecje 7q31
* **Lokalizacja chromosomalna:** Chromosom 7 (7q31.1)
* **Rola biologiczna genu/białka:** Nadrzędny czynnik transkrypcyjny koordynujący rozwój obwodów neuronalnych w prążkowiu, móżdżku i korze ruchowej. Odpowiada za plastyczność synaptyczną niezbędną do uczenia się sekwencji motorycznych (koordynacja ruchów aparatu mowy niezbędna do artykulacji).

* **Mój genotyp (WGS):**
  - `rs121912959` — NOT_IN_DBSNP
  - `rs2396753` — **CA** (ensembl, high) — ensembl; depth=48,avg=48.0; gt=0/1
  - `rs7782412` — **TC** (ensembl, high) — ensembl; depth=57,avg=57.0; gt=0/1


* **Typ Dziki (Ekspresja prawidłowa):** Prawidłowe formowanie struktur prążkowia. Wysoka płynność mowy, bezproblemowe przyswajanie gramatyki, wysoka koordynacja ruchowa.
* **Warianty Polimorficzne (Modyfikujące):** Subtelne różnice w tempie przetwarzania fonologicznego lub predyspozycje do dysleksji bez cech dysmorfii.
* **Mutacja Patogenna (Haploinsuficjencja / LoF):** Wywołuje Werbalną Dyspraksję Rozwojową (orofacjalną). Objawia się całkowitą niemożnością skoordynowania ruchów języka i warg w celu wypowiedzenia złożonych słów, upośledzeniem rozumienia gramatyki oraz głębokim regresorem językowym wpisującym się w ASD.

---
### LINC01795

* **Główny symbol genu:** LINC01795
* **Pełna nazwa biochemiczna:** Długie intergeniczne niekodujące RNA 1795 (ang. *Long intergenic non-protein coding RNA 1795*)
* **Nazwy potoczne i medialne:** Epigenetyczny modyfikator ekspresji tkankowej kory, sygnał GWAS neuroatypowości
* **Główny rsID:** rs12048493
* **Lokalizacja chromosomalna:** Chromosom 1 (1p36.12)
* **Rola biologiczna genu/białka:** Cząsteczka lncRNA (długiego niekodującego RNA), która nie koduje fizycznego białka, lecz działa jako regulator epigenetyczny (rusztowanie dla kompleksów modyfikujących chromatynę lub molekularna gąbka dla miRNA). Kontroluje poziom ekspresji sąsiadujących genów strukturalnych odpowiedzialnych za wczesną polaryzację kory czołowej.

* **Mój genotyp (WGS):**
  `rs12048493` — **AC** (ensembl+bam, high) — marker_report



**Genotyp Referencyjny (Prawidłowe dostrojenie eQTL)**
Zrównoważona i precyzyjna kontrola nad ekspresją genów sąsiednich, optymalny rozwój kognitywny kory przedczołowej.

**Genotyp Pośredni (Wariant populacyjny)**
Standardowa populacyjna zmienność epigenetyczna niekodującego RNA.

**Genotyp Ryzyka (Homozygota wariantu GWAS / Zmieniona ekspresja lncRNA)**
Wywołuje patologiczne przesunięcia w poziomie transkrypcji genów osi neurorozwojowej w korze mózgowej (efekt cis-eQTL). W międzynarodowych analizach wielkoskalowych wariant ten stanowi powtarzalny, statystyczny sygnał ryzyka zwiększający predyspozycje do ADHD, schizofrenii oraz trudności w uczeniu się.

---
### MACROD2

* **Główny symbol genu:** MACROD2
* **Pełna nazwa biochemiczna:** Domena MACRO zawierająca białko 2 (ang. *MACRO domain-containing protein 2*)
* **Nazwy potoczne i medialne:** Gen rozwoju mowy i integracji poznawczej
* **Główny rsID / Wariant:** Mikrodelecje introniczne, warianty strukturalne (CNV) lub rs6116204
* **Lokalizacja chromosomalna:** Chromosom 20 (20p12.1)
* **Rola biologiczna genu/białka:** Koduje hydrolazę usuwającą modyfikacje ADP-rybozylowania z białek (kluczowa regulacja posttranslacyjna). Odgrywa fundamentalną rolę w utrzymaniu stabilności chromosomalnej i precyzyjnej regulacji genów zaangażowanych w rozwój struktur przodomózgowia odpowiedzialnych za ekspresję mowy.

* **Mój genotyp (WGS):**
  `rs6116204` — **AT** (ensembl+bam, high) — ensembl; depth=39,avg=39.0; gt=0/1






**Typ Dziki (Brak rearanżacji strukturalnych)**
Prawidłowa dynamika formowania korowych ośrodków mowy (w tym obszaru Broki i Wernickego). Harmonijny rozwój kompetencji językowych i płynności poznawczej.

**Warianty populacyjne / Polimorfizmy introniczne**
Częste w populacji modyfikatory tempa rozwoju mowy u dzieci. Mogą wywoływać przejściowe opóźnienia ekspresji językowej (SLI) przy całkowicie zachowanym potencjale intelektualnym.

**Rearanżacja Patogenna (Delecja eksonów / LoF)**
Strukturalne uszkodzenia genu MACROD2 są bezpośrednio powiązane z agenezją ciała modzelowatego, głębokimi, trwałymi zaburzeniami rozwoju mowy, mutyzmem wybiórczym oraz wysokim ryzykiem klinicznego fenotypu autystycznego.

---

---
### MDFIC

* **Główny symbol genu:** MDFIC
* **Pełna nazwa biochemiczna:** Białko zawierające domenę inhibitora rodziny MyoD (ang. *MyoD family inhibitor domain-containing protein*)
* **Nazwy potoczne i medialne:** Modulator szlaku Wnt i różnicowania komórkowego
* **Główny rsID:** rs2280031
* **Lokalizacja chromosomalna:** Chromosom 7 (7q31.1-q31.2)
* **Rola biologiczna genu/białka:** Koduje białko, które moduluje aktywność czynników transkrypcyjnych w jądrze komórkowym. Działa jako inhibitor szlaku sygnałowego Wnt/beta-kateniny (kluczowego dla wczesnej polaryzacji mózgowia, wyznaczania osi przód-tył oraz proliferacji komórek glejowych).

* **Mój genotyp (WGS):**
  - `rs2280031` — **CC** (ensembl+bam, high) — marker_report
  - `rs10246939` — **CC** (ensembl, high) — ensembl; depth=49,avg=49.0; gt=1/1



**Typ Dziki (Prawidłowa regulacja Wnt)**
Symetryczny i precyzyjnie kontrolowany podział komórkowy w fazie embriogenezy. Prawidłowy rozwój struktur podkorowych.

**Warianty Populacyjne (Umiarkowane)**
Standardowe populacyjne przesunięcia w dynamice transkrypcyjnej.

**Homozygota alleliczna C/C (tag-SNP GWAS)**
Profil populacyjny rs2280031 (C/C). W GWAS skorelowany ze statystyczną podatnością na ASD i dyspraksję — nie oznacza zaburzeń ze spektrum autyzmu ani defektu genu MDFIC.

---

---
### MED8

* **Główny symbol genu:** MED8
* **Pełna nazwa biochemiczna:** Podjednostka kompleksu mediatora 8 (ang. *Mediator complex subunit 8*)
* **Nazwy potoczne i medialne:** Gen globalnego przesyłu transkrypcyjnego
* **Główny rsID:** rs2273574
* **Lokalizacja chromosomalna:** Chromosom 19 (19p13.2)
* **Rola biologiczna genu/białka:** Wchodzi w skład kompleksu Mediator – gigantycznego mostka wielobiałkowego, który fizycznie łączy czynniki transkrypcyjne związane z enhancerami z Polimerazą RNA II. Jest niezbędny do uruchomienia ekspresji jakiegokolwiek genu strukturalnego pod wpływem bodźców zewnętrznych (synaptycznych).

* **Mój genotyp (WGS):**
  - `rs2273574` — NOT_IN_DBSNP
  - `rs2797285` — **GG** (ensembl, high) — ensembl; depth=39,avg=39.0; gt=1/1



**Typ Dziki (Wydajna transkrypcja dynamiczna)**
Błyskawiczna i adekwatna odpowiedź genomowa neuronu na bodźce (uczenie się, stres, plastyczność).

**Warianty Populacyjne (Standardowe)**
Prawidłowa, stabilna homeostaza aparatu transkrypcyjnego w normie populacyjnej.

**Wariant Ryzyka (Suboptymalna stabilność kompleksu)**
Niewielkie spowolnienie dynamiki transkrypcyjnej genów zależnych od aktywności (IEGs), co może objawiać się subtelnym obniżeniem elastyczności poznawczej i gorszą adaptacją w nowych sytuacjach.

---

---
### MEIS1

* **Główny symbol genu:** MEIS1
* **Pełna nazwa biochemiczna:** Homeoboks MEIS1 (ang. *Meis homeobox 1*)
* **Nazwy potoczne i medialne:** Główny gen Zespołu Niespokojnych Nóg
* **Główny rsID:** rs12469063 / rs2300478
* **Lokalizacja chromosomalna:** Chromosom 2 (2q14.2)
* **Rola biologiczna genu/białka:** Koduje czynnik transkrypcyjny z rodziny homeodomen TALE, zaangażowany we wczesną specyfikację przestrzenną struktur mózgowia, rozwój hematopoetyczny oraz homeostazę dopaminergiczną prążkowia. Reguluje gęstość receptorów dopaminy oraz metabolizm i wiązanie żelaza w strukturach podkorowych.

* **Mój genotyp (WGS):**
  `rs12469063` — **AA** (ensembl+bam, high) — marker_report



**Genotyp Referencyjny (Wariant ochronny)**
Prawidłowa gęstość i dystrybucja neuronów dopaminergicznych w istocie czarnej i prążkowiu. Stabilna gospodarka żelazem w OUN, wysoka jakość i ciągłość snu.

**Genotyp Pośredni (Heterozygota)**
Standardowa populacyjna podatność na przejściowy niepokój ruchowy kończyn dolnych w stanach głębokiego zmęczenia lub przy niedoborach ferrytyny we krwi.

**Genotyp Ryzyka (Homozygota wariantu GWAS)**
Najsilniejszy populacyjny marker genetyczny dla **Zespołu Niespokojnych Nóg (RLS)** oraz okresowych ruchów kończyn podczas snu (PLMD). Powoduje lokalne zaburzenia neurochemii dopaminy i mikro-niedobory żelaza w strukturach podkorowych, drastycznie niszcząc architekturę snu wolnofalowego.

---

---
### MPL

* **Główny symbol genu:** MPL
* **Pełna nazwa biochemiczna:** Protoonkogen MPL, receptor trombopoetyny (ang. *MPL proto-oncogene, thrombopoietin receptor*)
* **Nazwy potoczne i medialne:** Gen receptora TPO, regulator nisz komórek macierzystych mózgu
* **Główny rsID / Wariant:** rzadkie mutacje utraty funkcji / rs2228511
* **Lokalizacja chromosomalna:** Chromosom 1 (1p34.2)
* **Rola biologiczna genu/białka:** Klasycznie znany jako receptor kontrolujący produkcję płytek krwi. Jednak najnowsze badania wykazują silną ekspresję MPL w komórkach macierzystych układu nerwowego (NSCs). Sygnalizacja TPO/MPL utrzymuje komórki progenitorowe mózgu w stanie spoczynku (quiescence), chroniąc nisze neurogenne przed przedwczesnym wyczerpaniem.

* **Mój genotyp (WGS):**
  - `rs2228511` — NOT_IN_DBSNP
  - `rs139606423` — **CC** (ensembl, high) — ensembl; depth=59,avg=59.0; gt=0/0



**Typ Dziki (Prawidłowa ochrona nisz neurogennych)**
Zbalansowana, długofalowa neurogeneza dorosła (w hipokampie). Zachowanie pełnego potencjału regeneracyjnego i plastycznego mózgu przez całe życie.

**Warianty Populacyjne (Średnia ekspresja)**
Standardowa populacyjna dynamika odnawiania komórek progenitorowych.

**Mutacja Patogenna / Silny Wariant Ryzyka**
Przedwczesne, masowe wyjście komórek macierzystych ze stanu spoczynku i ich gwałtowna proliferacja, prowadząca do wczesnego wyczerpania puli neurogennej. Skutkuje postępującym spadkiem plastyczności kognitywnej, zaburzeniami pamięci oraz wysoką korelacją z ciężkimi zespołami neuroatypowymi.

---

---
### MSRA

* **Główny symbol genu:** MSRA
* **Pełna nazwa biochemiczna:** Reduktaza sulfotlenku metioniny A (ang. *Methionine sulfoxide reductase A*)
* **Nazwy potoczne i medialne:** Gen tarczy antyoksydacyjnej mózgu
* **Główny rsID:** rs10957973 (częsty wariant markerowy w badaniach psychiatrycznych)
* **Lokalizacja chromosomalna:** Chromosom 8 (8p23.1)
* **Rola biologiczna genu/białka:** MSRA to kluczowy enzym naprawczy, który redukuje utleniony sulfotlenek metioniny w białkach z powrotem do formy funkcjonalnej (metioniny). Chroni neurony przed uszkodzeniami strukturalnymi wywołanymi przez permanentny stres oksydacyjny (ROS).

* **Mój genotyp (WGS):**
  `rs10957973` — **TT** (ensembl+bam, high) — ensembl; depth=33,avg=33.0; gt=1/1






**Genotyp Wysokiej Aktywności (Homozygota referencyjna)**
Maksymalna wydajność naprawy utlenionych białek. Wysoka rezyliencja neuronów wobec stresorów toksycznych i metabolicznych, stabilna ochrona synaps przed degradacją wolnorodnikową.

**Genotyp Średniej Aktywności (Heterozygota)**
Standardowa ochrona antyoksydacyjna. Wydolna w warunkach fizjologicznych; przy skrajnym stresie komórkowym może dochodzić do przejściowego zmęczenia kognitywnego.

**Genotyp alleliczny T/T (tag-SNP GWAS)**
Typowy profil populacyjny rs10957973 (T/T). W metaanalizach GWAS ten wariant koreluje ze statystyczną podatnością na schizofrenię, autyzm i zaburzenia afektywne — nie oznacza to patogennej mutacji MSRA ani klinicznej diagnozy.

---

---
### NEK4

* **Główny symbol genu:** NEK4
* **Pełna nazwa biochemiczna:** Kinaza powiązana z NIMA 4 (ang. *NIMA related kinase 4*)
* **Nazwy potoczne i medialne:** Regulator stabilności mikrotubul i rzęsek komórkowych
* **Główny rsID:** rs1051543
* **Lokalizacja chromosomalna:** Chromosom 3 (3p21.1)
* **Rola biologiczna genu/białka:** Kinaza serynowo-treoninowa kontrolująca dynamikę polimeryzacji mikrotubul w cytoszkielecie aksonalnym oraz uczestnicząca w formowaniu rzęsek pierwotnych neuronów. Odgrywa kluczową rolę w stabilizacji strukturalnej stożka wzrostu aksonu.

* **Mój genotyp (WGS):**
  `rs1051543` — **GG** (ensembl+bam, high) — marker_report



**Typ Dziki (Stabilny cytoszkielet aksonalny)**
Prawidłowy, niezakłócony wzrost i kierowanie aksonów do celów synaptycznych, wysoka integralność strukturalna sieci nerwowej.

**Warianty Populacyjne (Umiarkowane)**
Standardowa dynamika cytoszkieletu w normie populacyjnej.

**Wariant Ryzyka (Niska ekspresja NEK4)**
Lokalna niestabilność mikrotubul w rosnących wypustkach, mogąca prowadzić do opóźnień w tworzeniu długodystansowych połączeń międzypółkulowych. Skorelowany w GWAS z trudnościami w koordynacji ruchowej (dyspraksja) i deficytami uwagi.

---

---
### NKX2-2

* **Główny symbol genu:** NKX2-2
* **Pełna nazwa biochemiczna:** Homeoboks NK2, czynnik 2 (ang. *NK2 homeobox 2*)
* **Nazwy potoczne i medialne:** Architekt oligodendrocytów i mielinizacji kory
* **Główny rsID / Wariant:** Warianty eQTL o obniżonej ekspresji / Rzadkie mutacje punktowe
* **Lokalizacja chromosomalna:** Chromosom 2 (2q31.1)
* **Rola biologiczna genu/białka:** Czynnik transkrypcyjny kluczowy dla końcowego różnicowania komórek progenitorowych oligodendrocytów (OPC) do dojrzałych, funkcjonalnych oligodendrocytów. Odpowiada za proces powstawania osłonki mielinowej w OUN, warunkując prędkość, precyzję i synchronizację przewodzenia impulsów nerwowych.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Typ Dziki (Maksymalna wydajność mielinizacji)**
Prawidłowy, dynamiczny rozwój i regeneracja osłonek mielinowych. Wysoka prędkość przewodzenia sygnałów nerwowych, doskonała integracja sensoryczno-motoryczna i płynność poznawcza.

**Warianty Polimorficzne (Średnia ekspresja)**
Standardowa, populacyjna dynamika remielinizacji; tkanka nerwowa wykazuje prawidłowe parametry przewodnictwa w warunkach fizjologicznych.

**Niska Ekspresja / Wariant Ryzyka (Niski eQTL)**
Upośledzenie dojrzewania strukturalnego oligodendrocytów. Powoduje subtelne deficyty w architekturze istoty białej (dysmyelinację). W badaniach wielkoskalowych GWAS skorelowany z trudnościami w koordynacji ruchowej (dyspraksja), zaburzeniami uczenia się oraz podwyższoną podatnością na schizofrenię i ASD.

---

---
### NKX2-4

* **Główny symbol genu:** NKX2-4
* **Pełna nazwa biochemiczna:** Homeoboks NK2, czynnik 4 (ang. *NK2 homeobox 4*)
* **Nazwy potoczne i medialne:** Gen różnicowania międzymózgowia, sygnał GWAS neurorozwoju
* **Główny rsID:** Powiązany z blokiem sprzężeń GWAS na 20p11.22 (w bliskim sąsiedztwie XRN2)
* **Lokalizacja chromosomalna:** Chromosom 20 (20p11.22)
* **Rola biologiczna genu/białka:** Koduje homeodomenowy czynnik transkrypcyjny zaangażowany we wczesne różnicowanie neuronów oraz strukturyzację przestrzenną międzymózgowia i podwzgórza. Decyduje o integralności sieci łączących jądra podstawne z korą.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Genotyp Referencyjny (Wariant standardowy)**
Prawidłowa gęstość neuronalna w strukturach głębokich mózgowia, harmonijny rozwój osi podwzgórzowo-przysadkowej i prawidłowe przekaźnictwo korowo-podkorowe.

**Heterozygota (Ekspresja umiarkowana)**
Drobne przesunięcia w gęstości receptorowej lub dystrybucji neuronów w jądrach podstawnych. Dobra elastyczność adaptacyjna bez wyraźnych deficytów.

**Genotyp Ryzyka (Homozygota wariantu GWAS / Niski eQTL)**
Znaczne obniżenie ekspresji tkankowej genu w mózgowiu (szczególnie w móżdżku i jądrach podstawnych). Koreluje z klinicznym fenotypem podwyższonej impulsywności, zaburzeniami koncentracji uwagi (ADHD) oraz podatnością na cechy ze spektrum autyzmu.

---

---
### POU3F2

* **Główny symbol genu:** POU3F2
* **Pełna nazwa biochemiczna:** Czynnik transkrypcyjny klasy POU 3, składowa 2 (ang. *POU class 3 homeobox 2*)
* **Nazwy potoczne i medialne:** Gen tożsamości neuronów kory nowej, top-hit GWAS neurorozwoju
* **Główny rsID:** rs1015362
* **Lokalizacja chromosomalna:** Chromosom 6 (6q16.1)
* **Rola biologiczna genu/białka:** Koduje nadrzędny czynnik transkrypcyjny (BRN2), który decyduje o tożsamości komórkowej, różnicowaniu i migracji neuronów projekcyjnych wyższych warstw kory nowej (warstwy II-IV). Kontroluje również prawidłowy rozwój jąder neurosekrecyjnych podwzgórza, regulując ekspresję setek genów odpowiedzialnych za formowanie synaps.

* **Mój genotyp (WGS):**
  `rs1015362` — **CT** (ensembl+bam, high) — marker_report



**Genotyp Referencyjny (Prawidłowa cytoarchitektonika)**
Precyzyjne wykształcenie wyższych warstw kory mózgowej odpowiedzialnych za myślenie abstrakcyjne, integrację bodźców i funkcje wykonawcze. Prawidłowa gęstość i dystrybucja neuronów.

**Genotyp Pośredni (Wariant populacyjny)**
Standardowa plastyczność i strukturyzacja sieci korowych.

**Genotyp Ryzyka (Niski eQTL / Sygnał GWAS)**
Wyraźne obniżenie ekspresji tkankowej genu w trakcie embrionalnego rozwoju mózgu. Prowadzi do mikrostrukturalnych zaburzeń w ułożeniu neuronów kory nowej. W międzynarodowych megabadaniach wielkoskalowych (GWAS) wariant ten jest jednym z najsilniejszych punktowych sygnałów predysponujących do jednoczesnego rozwoju schizofrenii, choroby afektywnej dwubiegunowej oraz cech z kręgu spektrum autyzmu.

---
### RORB

* **Główny symbol genu:** RORB
* **Pełna nazwa biochemiczna:** Sierocy receptor jądrowy powiązany z receptorem kwasu retinowego beta (ang. *RAR-related orphan receptor beta*)
* **Nazwy potoczne i medialne:** Gen integracji sensorycznej warstwy IV kory, regulator chronotypu
* **Główny rsID / Wariant:** rs2730573 / Mutacje *de novo* utraty lub zmiany funkcji
* **Lokalizacja chromosomalna:** Chromosom 9 (9q21.13)
* **Rola biologiczna genu/białka:** Receptor jądrowy działający jako ligand-zależny czynnik transkrypcyjny. Wykazuje silną, specyficzną ekspresję w warstwie IV kory nowej (główna warstwa odbierająca informacje sensoryczne ze wzgórza) oraz w siatkówce i jądrach nadskrzyżowaniowych podwzgórza. Koordynuje przetwarzanie bodźców czuciowych oraz synchronizację zegara biologicznego.

* **Mój genotyp (WGS):**
  `rs2730573` — **GG** (ensembl+bam, high) — ensembl; depth=53,avg=53.0; gt=0/0






**Typ Dziki (Prawidłowa integracja wzgórzowo-korowa)**
Precyzyjna dystrybucja impulsów czuciowych, słuchowych i wzrokowych w korze nowej. Stabilny, prawidłowo zsynchronizowany z zewnętrznym światłem rytm okołodobowy.

**Warianty Populacyjne (Umiarkowane)**
Standardowe różnice osobnicze w chronotypie (poranny ptaszek vs nocny marek) oraz drobne wahania w indywidualnej tolerancji na silne bodźce zmysłowe.

**Mutacja Patogenna / Silny Wariant Ryzyka**
Wywołuje uogólnione padaczki dziecięce (w tym ciężką padaczkę miokloniczno-atoniczną) oraz złożone zaburzenia neurorozwojowe charakteryzujące się głęboką niepełnosprawnością intelektualną, stereotypiami ruchowymi i ekstremalną nadwrażliwością sensoryczną w ASD.

---

## BLOK 14: Modulatory Kinetyki Synaptycznej i Organizacji Transmisji

---
### SESTD1

* **Główny symbol genu:** SESTD1
* **Pełna nazwa biochemiczna:** Białko 1 zawierające domeny SEC14 i spektrynowe (ang. *SEC14 and spectrin domains 1*)
* **Nazwy potoczne i medialne:** Regulator kanałów TRPC, gen synaptycznego transportu fosfolipidów
* **Główny rsID:** rs10811234
* **Lokalizacja chromosomalna:** Chromosom 2 (2q31.2)
* **Rola biologiczna genu/białka:** Działa jako sensor i transporter fosfolipidów, który bezpośrednio reguluje wbudowywanie i aktywność receptorowych kanałów wapniowych TRPC (głównie TRPC4 i TRPC5) w błonie postsynaptycznej. Kontroluje nie-napięciowy, receptorowy napływ jonów wapnia uczestniczący w naprowadzaniu aksonów i plastyczności dendrytów.

* **Mój genotyp (WGS):**
  `rs10811234` — **CT** (ensembl+bam, high) — marker_report



**Typ Dziki (Zbalansowana regulacja TRPC)**
Precyzyjny, kontrolowany prąd wapniowy przez kanały TRPC, prawidłowa plastyczność i brak przeciążeń komórkowych.

**Warianty Populacyjne (Umiarkowane)**
Standardowa dynamika transportu błonowego fosfolipidów.

**Wariant Ryzyka (Niezrównoważona ekspresja SESTD1)**
Zaburzenie gęstości kanałów TRPC w synapsach, prowadzące do destabilizacji spoczynkowego tonusu wapniowego neuronu. Skorelowany w GWAS z cechami neurorozwojowymi i nadwrażliwością sensoryczną.

---

---
### SORCS3

* **Główny symbol genu:** SORCS3
* **Pełna nazwa biochemiczna:** Receptor sortyliny 3 (ang. *Sortilin-related VPS10 domain containing receptor 3*)
* **Nazwy potoczne i medialne:** Gen handlu receptorami, top-hit GWAS depresji, ADHD i ChAD
* **Główny rsID:** rs10881512 / rs2051515
* **Lokalizacja chromosomalna:** Chromosom 10 (10q25.1)
* **Rola biologiczna genu/białka:** Koduje receptor transportu wewnątrzkomórkowego (sortylinę), wykazujący potężną ekspresję w hipokampie i korze nowej. Odpowiada za endocytozę, prawidłowe kierowanie (trafficking) i recykling postsynaptycznych receptorów AMPA i NMDA oraz neurotrofiny BDNF. Jest krytycznym moderatorem wygaszania synaptycznego (LTD).

* **Mój genotyp (WGS):**
  - `rs10881512` — **CT** (ensembl+bam, high) — marker_report
  - `rs139885610` — **GG** (ensembl, high) — ensembl; depth=35,avg=35.0; gt=0/0
  - `rs56163402` — **GG** (ensembl, high) — ensembl; depth=28,avg=28.0; gt=1/1



**Genotyp Referencyjny (Prawidłowy recykling receptorów)**
Stabilne zarządzanie gęstością postsynaptyczną. Sprawny transport BDNF, wysoka neuroprotekcja komórkowa i doskonała zdolność adaptacji poznawczej (cognitive flexibility).

**Genotyp Pośredni (Wariant standardowy)**
Prawidłowy, populacyjny profil transportu pęcherzykowego w neuronach.

**Genotyp Ryzyka (Niska ekspresja SORCS3)**
Upośledzenie recyklingu receptorów AMPA oraz deficyty w dystrybucji BDNF. W międzynarodowych megabadaniach genomicznych wariant ten stanowi jeden z najsilniejszych, powtarzalnych sygnałów ryzyka dla **ciężkiej depresji (MDD), ADHD, choroby afektywnej dwubiegunowej** oraz przyspieszonego starzenia poznawczego.

---

## BLOK 15: Regulatory Ścieżek Wzrostu i Przekształcania Neurotrofin

---
### SOX7

* **Główny symbol genu:** SOX7
* **Pełna nazwa biochemiczna:** Czynnik transkrypcyjny SRY-box 7 (ang. *SRY-box transcription factor 7*)
* **Nazwy potoczne i medialne:** Architekt bariery krew-mózg i neurogenezy
* **Główny rsID / Wariant:** Polimorfizmy regulatorowe lub mikrodelecje regionu 8p23.1
* **Lokalizacja chromosomalna:** Chromosom 8 (8p23.1)
* **Rola biologiczna genu/białka:** SOX7 moduluje szlaki sygnałowe Wnt/beta-kateniny oraz VEGF. Jest kluczowy dla prawidłowej angiogenezy w rozwijającym się przodomózgowiu, formowania szczelnej bariery krew-mózg oraz regulacji podziałów komórek macierzystych w niszach neurogennych.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Typ Dziki (Ekspresja prawidłowa)**
Prawidłowo wykształcone mikrokrążenie mózgowe, wysoka integralność bariery krew-mózg. Zrównoważony proces różnicowania progenitorów neuronalnych.

**Warianty Polimorficzne (Średnia ekspresja)**
Drobne, bezobjawowe fluktuacje w gęstości unaczynienia specyficznych obszarów podkorowych; bez bezpośredniego przełożenia na patologię.

**Zmniejszona Ekspresja / Mikrodelecja 8p23.1**
Zaburzenie wczesnej neurogenezy i stabilności naczyniowej mózgu. Generuje złożony fenotyp neurorozwojowy: wady sercowo-naczyniowe, specyficzną dysmorfię oraz wysokie ryzyko niepełnosprawności intelektualnej z cechami spektrum autyzmu.

---

---
### ST3GAL3

* **Główny symbol genu:** ST3GAL3
* **Pełna nazwa biochemiczna:** Alfa-2,3-sialylotransferaza 3 (ang. *ST3 beta-galactoside alpha-2,3-sialyltransferase 3*)
* **Nazwy potoczne i medialne:** Gen sialilacji receptorów, gen niesyndromowej niepełnosprawności intelektualnej
* **Główny rsID / Wariant:** Mutacje typu utraty funkcji (LoF) / rs1138844
* **Lokalizacja chromosomalna:** Chromosom 1 (1p34.1)
* **Rola biologiczna genu/białka:** Koduje enzym rezydujący w aparacie Golgiego, który dołącza kwas sialowy do końców łańcuchów oligosacharydowych na glikoproteinach i glikolipidach błonowych. Sialilacja decyduje o prawidłowym ustrukturyzowaniu i działaniu cząsteczek adhezyjnych (NCAM1) oraz receptorów neuroprzekaźników, warunkując ich stabilność w synapsie.

* **Mój genotyp (WGS):**
  `rs1138844` — **TT** (ensembl+bam, high) — marker_report



**Typ Dziki (Prawidłowa sialilacja synaptyczna)**
W pełni funkcjonalne, prawidłowo "oglikozylowane" receptory i cząsteczki adhezyjne. Optymalna plastyczność synaptyczna i wysoka sprawność kognitywna.

**Warianty Populacyjne (Średnia wydajność)**
Standardowa bezobjawowa zmienność w strukturze oligosacharydowej błon komórkowych.

**Mutacja Patogenna (Heterozygota/Homozygota LoF)**
Ciężka, autosomalna recesywna niepełnosprawność intelektualna (MRT12). Brak prawidłowej sialilacji uniemożliwia fizyczną stabilizację synaps w hipokampie i korze mózgowej. Objawia się głębokim opóźnieniem rozwoju mowy, regressem poznawczym, dyspraksją oraz ciężkim fenotypem autystycznym (ASD).

---

---
### STX16-NPEPL1

* **Główny symbol genu:** STX16 / NPEPL1 (Region intergeniczny / blok sprzężeń)
* **Pełna nazwa biochemiczna:** Syntaksyna 16 / Aminopeptidase-like 1 locus (ang. *Syntaxin 16 / Aminopeptidase like 1 region*)
* **Nazwy potoczne i medialne:** Gen pętli pęcherzyków Golgiego i metabolizmu kognitywnego
* **Główny rsID:** rs6033475
* **Lokalizacja chromosomalna:** Chromosom 20 (20q13.32)
* **Rola biologiczna genu/białka:** STX16 koduje białko t-SNARE zaangażowane w transport pęcherzykowy między endosomami a aparatem Golgiego. NPEPL1 koduje metaloproteazę aminopeptydową. Ten specyficzny region genomu kontroluje wewnątrzkomórkowy transport transporterów glukozy (GLUT4) oraz precyzyjną obróbkę neuropeptydów w neuronach.

* **Mój genotyp (WGS):**
  `rs6033475` — **TT** (ensembl+bam, high) — ensembl; depth=41,avg=41.0; gt=0/0






**Genotyp Referencyjny (Wydajny transport Golgiego)**
Prawidłowy recykling białek transportowych, optymalny metabolizm glukozy w tkance nerwowej, stabilna gospodarka neuropeptydowa.

**Genotyp Pośredni (Umiarkowany)**
Standardowa populacyjna dynamika transportu wewnątrzkomórkowego.

**Genotyp Ryzyka (Sygnał GWAS / Obniżona ekspresja bloku)**
Zaburzenia w wewnątrzkomórkowym transporcie pęcherzyków, skutkujące suboptymalnym zaopatrzeniem neuronów w energię lub błędami w obróbce peptydów sygnałowych. W badaniach genomicznych powiązany z podatnością na zaburzenia metaboliczne, ADHD oraz zaburzenia nastroju.

---

---
### TRANK1

* **Główny symbol genu:** TRANK1
* **Pełna nazwa biochemiczna:** Białko zawierające domeny tetratrikopeptydowe powtarzalne i kinazowe 1 (ang. *Tetratricopeptide repeat and kinase domain containing 1*)
* **Nazwy potoczne i medialne:** Gen odpowiedzi na lit, marker stabilizacji afektywnej
* **Główny rsID:** rs9834970
* **Lokalizacja chromosomalna:** Chromosom 3 (3p22.2)
* **Rola biologiczna genu/białka:** TRANK1 wykazuje silną ekspresję w OUN pod wpływem leków normotymicznych (litu i walproinianów). Choć dokładny mechanizm białka jest wciąż badany, jest ono kluczowym modulatorem ekspresji genów zapalnych oraz stabilizatorem cytoszkieletu w stanach stresu komórkowego.

* **Mój genotyp (WGS):**
  `rs9834970` — **TT** (ensembl+bam, high) — ensembl; depth=58,avg=58.0; gt=0/0
* **Dopasowany profil:** Genotyp T/T (Wariant referencyjny):





* **Genotyp T/T (Wariant referencyjny):** Standardowa ekspresja genu. Prawidłowa odporność komórkowa na bodźce destabilizujące neurochemię.
* **Genotyp C/T (Pośredni):** Średnia wrażliwość na czynniki normotymiczne; standardowy profil odpowiedzi populacyjnej.
* **Genotyp C/C (Najsilniejszy sygnał GWAS dla ChAD):** Wyraźne zaburzenie ekspresji genu. Powoduje potężną podatność na rozwój choroby afektywnej dwubiegunowej. Co krytyczne: obecność tego wariantu ryzyka paradoxalnie koreluje z **doskonałą, natychmiastową odpowiedzią terapeutyczną na lit** (pacjenci typu *excellent lithium responders*).

---

## BLOK 4: Neuroimmunologia i Szlak Kynureninowy (Neuroinflammation)

---
### XRN2

* **Główny symbol genu:** XRN2
* **Pełna nazwa biochemiczna:** 5'-3' egzorybonukleaza 2 (ang. *5'-3' exoribonuclease 2*)
* **Nazwy potoczne i medialne:** Główny sygnał GWAS neurorozwoju, gen terminacji transkrypcji
* **Główny rsID:** rs1465223 (lub powiązane silne sygnały GWAS w locus 20p11.22)
* **Lokalizacja chromosomalna:** Chromosom 20 (20p11.22)
* **Rola biologiczna genu/białka:** XRN2 odpowiada za degradację RNA i prawidłową terminację transkrypcji realizowanej przez polimerazę RNA II (model "torpedy"). Odgrywa kluczową rolę w kontroli jakości RNA oraz usuwaniu wadliwych, niedokończonych transkryptów w komórkach progenitorowych mózgu.

* **Mój genotyp (WGS):**
  `rs1465223` — **GG** (ensembl+bam, high) — marker_report



**Genotyp Referencyjny (Wariant ochronny)**
Maksymalna wydajność terminacji transkrypcji. Stabilna homeostaza RNA w strukturach korowych i móżdżku, prawidłowy rozwój i różnicowanie neuronów.

**Heterozygota (Wariant pośredniego ryzyka)**
Subtelne przesunięcia w dynamice transkrypcji genów sąsiednich (efekty cis-eQTL). Może obniżać próg tolerancji na stresory środowiskowe, wykazując umiarkowaną korelację z cechami neurorozwojowymi.

**Genotyp Ryzyka (Homozygota wariantu GWAS)**
Najsilniejszy statystycznie sygnał w wielkoskalowych badaniach asocjacyjnych dla cech neurorozwojowych. Skorelowany ze zmienioną ekspresją genu w korze przedczołowej i móżdżku, co bezpośrednio podwyższa ryzyko deficytów uwagi (ADHD) oraz zachowań z kręgu ASD.

---

---
### ZSWIM6

* **Główny symbol genu:** ZSWIM6
* **Pełna nazwa biochemiczna:** Białko zawierające domenę cynkowo-palcową SWIM 6 (ang. *Zinc finger SWIM-type containing 6*)
* **Nazwy potoczne i medialne:** Gen połączeń prążkowia, gen dysmorfii twarzowo-czaszowej i neuroatypowości
* **Główny rsID / Wariant:** Mutacje typu missens *de novo* lub mutacje typu utraty funkcji
* **Lokalizacja chromosomalna:** Chromosom 16 (16p12.1)
* **Rola biologiczna genu/białka:** Białko ZSWIM6 odgrywa fundamentalną rolę we wczesnym rozwoju układu nerwowego, w szczególności kontrolując morfogenezę aksonów, naprowadzanie neurytów (axon guidance) oraz prawidłowe formowanie struktur prążkowia (striatum).

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Typ Dziki (Brak mutacji patogennych)**
Prawidłowy wzrost i precyzyjne kierunkowanie aksonów. Symetryczny rozwój połączeń korowo-prążkowiowych, odpowiedzialnych za integrację sensoryczno-motoryczną.

**Warianty Polimorficzne (Łagodne)**
Subtelne różnice w mikrostrukturze połączeń podkorowych, mogące wpływać na indywidualne cechy profilu sensorycznego lub motoryki małej.

**Mutacja Patogenna (np. de novo missense)**
Wywołuje zespół dysmorfii twarzowo-czaszowej (Acromelic Frontonasal Dysostosis - AFND) lub specyficzny fenotyp neurorozwojowy: ciężkie zaburzenia mowy, głębokie upośledzenie formowania synaps i koneksji neuronalnych oraz cechy ASD.

---

---

## CZĘŚĆ VI. Neuroimmunologia, HLA i zapalenie OUN
### C1R

* **Główny symbol genu:** C1R
* **Pełna nazwa biochemiczna:** Składnik dopełniacza 1, podskładnik r (ang. *Complement component 1, r subcomponent*)
* **Nazwy potoczne i medialne:** Inicjator kaskady klasycznej, gen zespołu Ehlersa-Danlosa (typ przyzębiowy)
* **Główny rsID / Wariant:** Mutacje patogenne Gain-of-Function / rs11545624
* **Lokalizacja chromosomalna:** Chromosom 12 (12q12)
* **Rola biologiczna genu/białka:** C1r to proteaza serynowa tworząca kompleks C1 (kluczowy inicjator klasycznej drogi aktywacji dopełniacza). Oprócz niszczenia patogenów, bierze udział w remodelowaniu macierzy zewnątrzkomórkowej oraz – wspólnie z opisywanym wcześniej C4A – uczestniczy w znakowaniu synaps do przycinania.

* **Mój genotyp (WGS):**
  - `rs11545624` — NOT_IN_DBSNP
  - `rs1806698` — **GG** (ensembl+bam, medium) — ensembl; depth=15,avg=15.0; gt=0/0






**Typ Dziki (Zbalansowana aktywacja dopełniacza)**
Prawidłowy profil odpornościowy, optymalna gęstość naczyń krwionośnych i zbalansowane usuwanie słabych połączeń synaptycznych.

**Warianty Populacyjne (Średnia aktywność)**
Standardowa populacyjna reaktywność immunologiczna.

**Mutacja Patogenna (GoF / Niezrównoważona hiperaktywacja)**
Wywołuje **Przyzębiowy Zespół Ehlersa-Danlosa (pEDS)**, charakteryzujący się ekstremalną wiotkością tkanki łącznej, niszczeniem przyzębia oraz wiotkością naczyń krwionośnych w mózgu. Towarzyszy mu wysokie ryzyko leukoencefalopatii, wad strukturalnych istoty białej oraz zaburzeń neurorozwojowych z powodu mikrostanów zapalnych naczyń.

---

---
### C4A

* **Główny symbol genu:** C4A
* **Pełna nazwa biochemiczna:** Składnik dopełniacza 4A (ang. *Complement component 4A*)
* **Nazwy potoczne i medialne:** Gen synaptycznego przycinania, główny winowajca schizofrenii
* **Główny wariant:** Zmienność liczby kopii (CNV) - strukturalne warianty AL (mózgowy nadmiar kopii)
* **Lokalizacja chromosomalna:** Chromosom 6 (6p21.3) w regionie MHC klasy III
* **Rola biologiczna genu/białka:** Białko dopełniacza C4A "znakuje" słabe lub mało aktywne synapsy w okresie dojrzewania, dając sygnał mikroglejowi do ich fizycznego pożarcia i eliminacji (synaptic pruning). Proces ten zachodzi intensywnie w korze przedczołowej w wieku nastoletnim.

* **Mój genotyp (WGS):**
  `rs1137101` — **AG** (ensembl+bam, high) — ncbi; depth=48,avg=48.0; gt=0/1




* **Niska liczba kopii C4A (Zbalansowany pruning):** Selektywne, precyzyjne usunięcie niepotrzebnych synaps. Zachowanie wysokiej gęstości połączeń korowych, optymalne zdolności analityczne, minimalne ryzyko psychozy.
* **Umiarkowana liczba kopii:** Standardowa populacyjna przebudowa sieci neuronalnej.
* **Wysoka liczba kopii (Nadekspresja strukturalna C4A):** Patologiczna hiperaktywność mikrogleju. Dochodzi do masowego, bezlitosnego niszczenia zdrowych połączeń synaptycznych w korze przedczołowej. Skutkuje to drastycznym "ocienieniem" istoty szarej, zaburzeniami poznawczymi i stanowi **główny molekularny mechanizm wyzwalający schizofrenię**.

---
### CRP

* **Główny symbol genu:** CRP
* **Pełna nazwa biochemiczna:** Białko C-reaktywne (ang. *C-reactive protein*)
* **Nazwy potoczne i medialne:** Gen systemowego markeru neuro-stresu i depresji opornej
* **Główny rsID:** rs1205 / rs1800947
* **Lokalizacja chromosomalna:** Chromosom 1 (1q23.2)
* **Rola biologiczna genu/białka:** CRP to klasyczne białko ostrej fazy, syntezowane w hepatocytach pod wpływem stymulacji przez IL-6. Choć jego produkcja zachodzi głównie obwodowo, przewlekle podwyższone poziomy CRP uszkadzają komórki śródbłonka bariery krew-mózg (BBB), zwiększając jej przepuszczalność dla toksyn oraz upośledzając transport kluczowych aminokwasów do mózgu.

* **Mój genotyp (WGS):**
  `rs1205` — **CT** (ensembl+bam, high) — marker_report



**Warianty Niskiej Ekspresji (Niskie bazowe CRP)**
Wysoka szczelność bariery krew-mózg, minimalne systemowe obciążenie zapalne, prawidłowa farmakokinetyka leków w OUN.

**Warianty Pośrednie (Populacyjne)**
Standardowa zmienność poziomu CRP zależna głównie od stylu życia, diety i aktywności fizycznej.

**Warianty Wysokiej Spoczynkowej Ekspresji (Wysokie bazowe CRP)**
Genetycznie zaprogramowane, stale podwyższone stężenie CRP we krwi (nawet przy braku jawnej infekcji). Skutkuje przewlekłym drażnieniem struktur naczyniowych mózgu. W psychiatrii genomicznej to kluczowy marker tzw. **depresji zapalnej**, charakteryzującej się całkowitym brakiem odpowiedzi na standardowe leki serotoninergiczne (wymaga terapii celowanej na redukcję stanu zapalnego).

---

---
### HLA-B

* **Główny symbol genu:** HLA-B
* **Pełna nazwa biochemiczna:** Główny kompleks zgodności tkankowej, klasa I, B (ang. *Major histocompatibility complex, class I, B*)
* **Nazwy potoczne i medialne:** Gen prezentacji antygenów, marker neuroimmunologiczny ASD
* **Główny rsID / Allel:** Profil wieloalleliczny (serotypy z grupy HLA-B, np. HLA-B*44)
* **Lokalizacja chromosomalna:** Chromosom 6 (6p21.33)
* **Rola biologiczna genu/białka:** Prezentuje peptydy pochodzenia wewnątrzkomórkowego limfocytom T cytotoksycznym (CD8+). W ośrodkowym układzie nerwowym cząsteczki MHC klasy I pełnią kluczową rolę nieimmunologiczną: regulują proces "pruningu" (synaptycznego przycinania) realizowanego przez mikroglej w trakcie rozwoju kory mózgowej.

* **Mój genotyp (WGS):**
  - `rs3135388` — **AA** (ensembl+bam, low) — proxy (OptiType niedostepny); ncbi; depth=3,avg=3.0; gt=0/0
  - (OptiType niedostepny; proxy tag-SNP — nie pelne typowanie alleli HLA)






**Allele Referencyjne (Zbalansowany pruning)**
Prawidłowa, selektywna eliminacja zbędnych lub słabych połączeń synaptycznych w dzieciństwie. Stabilna tolerancja immunologiczna, optymalna gęstość sieci neuronowych.

**Allele Predysponujące (Występujące u ok. 44% pacjentów w grupach badawczych ASD)**
Zmienione powinowactwo prezentacji antygenowej na komórkach glejowych. Skutkuje anomaliami w tempie i precyzji eliminacji synaps, co prowadzi do lokalnego "przebodźcowania" sieci neuronalnych i predyspozycji do cech neurorozwojowych.

**Dysfunkcja / Profil Autoimmunologiczny HLA**
Przewlekła, patologiczna aktywacja immunologiczna w obrębie OUN. Autoreaktywność limfocytów, niszczenie zdrowych połączeń synaptycznych, permanentny stan zapalny (neuroinflammation) i głęboki behawioralny fenotyp ASD.

---

---
### HLA-DQB1

* **Główny symbol genu:** HLA-DQB1
* **Pełna nazwa biochemiczna:** Główny kompleks zgodności tkankowej, klasa II, DQ beta 1 (ang. *Major histocompatibility complex, class II, DQ beta 1*)
* **Nazwy potoczne i medialne:** Gen osi jelitowo-mózgowej, gen podatności na celiakię i dysbiozę
* **Główny rsID / Allel:** Haplotypy wysokiego ryzyka (np. HLA-DQB1*02, HLA-DQB1*03:02)
* **Lokalizacja chromosomalna:** Chromosom 6 (6p21.32)
* **Rola biologiczna genu/białka:** Koduje podjednostkę receptora MHC klasy II na komórkach prezentujących antygen (APC). Odpowiada za inicjację odpowiedzi odpornościowej na antygeny zewnątrzkomórkowe – w tym białka pokarmowe (gluten, kazeina) oraz patogeny w świetle jelita.

* **Mój genotyp (WGS):**
  - `rs7775228` — **TT** (ensembl+bam, low) — proxy (OptiType niedostepny); ncbi; depth=5,avg=5.0; gt=0/0
  - (OptiType niedostepny; proxy tag-SNP — nie pelne typowanie alleli HLA)






**Allele Neutralne (Populacyjne)**
Prawidłowa tolerancja immunologiczna na białka pokarmowe. Stabilna bariera jelitowa, homeostaza śluzówkowa, stan eubiozy (prawidłowy profil mikrobiomu jelitowego).

**Allele Pośredniego Ryzyka (Heterozygota)**
Zwiększona reaktywność immunologiczna śluzówki jelit na określone frakcje białkowe. Może powodować subkliniczną eozynofilię jelitową i okresowe przesunięcia w profilu mikrobioty pod wpływem błędów dietetycznych.

**Haplotypy Wysokiego Ryzyka (np. celiakia / autoimmunizacja)**
Agresywna odpowiedź immunologiczna na antygeny pokarmowe. Prowadzi do enteropatii, rozszczelnienia bariery jelitowej (leaky gut) i głębokiej dysbiozy. Przenikające cytokiny zapalne oraz peptydy o charakterze egzorfin (np. kazomorfina) przedostają się przez barierę krew-mózg, zaostrzając dysfunkcje sensoryczne i objawy behawioralne w ASD.

---

---
### HLA-DRB1

* **Główny symbol genu:** HLA-DRB1
* **Pełna nazwa biochemiczna:** Główny kompleks zgodności tkankowej, klasa II, DR beta 1 (ang. *Major histocompatibility complex, class II, DR beta 1*)
* **Nazwy potoczne i medialne:** Gen podtypu MAR-ASD (Maternal Autoantibody-Related Autism)
* **Główny rsID / Allel:** Specyficzne konstelacje alleli matczynych (np. z grupy HLA-DRB1*04)
* **Lokalizacja chromosomalna:** Chromosom 6 (6p21.32)
* **Rola biologiczna genu/białka:** Kluczowy receptor MHC klasy II odpowiedzialny za prezentację antygenów limfocytom CD4+. U kobiet w ciąży obecność specyficznych wariantów HLA-DRB1 determinuje patologiczną skłonność do produkcji specyficznych autoprzeciwciał IgG skierowanych przeciwko białkom mózgowym płodu.

* **Mój genotyp (WGS):**
  - `rs9268839` — **AA** (ensembl+bam, medium) — proxy (OptiType niedostepny); ncbi; depth=16,avg=16.0; gt=0/0
  - (OptiType niedostepny; proxy tag-SNP — nie pelne typowanie alleli HLA)






**Genotyp Matczyny Standardowy**
Prawidłowa tolerancja immunologiczna ciąży. Brak krążących przeciwciał przeciwmózgowych, w pełni bezpieczny, niezakłócony rozwój neurobiologiczny płodu.

**Obecność Alleli Ryzyka (Heterozygota u matki)**
Podwyższona skłonność do subklinicznej autoimmunizacji. Zwiększone ryzyko silnej aktywacji immunologicznej i reakcji krzyżowej limfocytów B pod wpływem infekcji wirusowych w trakcie ciąży.

**Matczyny Genotyp MAR-ASD (Homozygota ryzyka / Konstelacja patogenna)**
Wymuszona produkcja matczynych autoprzeciwciał IgG (skierowanych m.in. przeciwko CRMP1, CRMP2, GDA). Przechodzą one przez łożysko do krążenia płodu, penetrują barierę krew-mózg i nieodwracalnie niszczą architekturę rozwijającej się kory mózgowej, indukując u dziecka podtyp MAR-ASD (ciężki autyzm z makrocefalią i regresją rozwojową).

---

---
### IDO1

* **Główny symbol genu:** IDO1
* **Pełna nazwa biochemiczna:** 2,3-dioksygenaza indoloaminowa 1 (ang. *Indoleamine 2,3-dioxygenase 1*)
* **Nazwy potoczne i medialne:** Gen kradzieży tryptofanu, przełącznik depresji zapalnej
* **Główny rsID:** rs9657182
* **Lokalizacja chromosomalna:** Chromosom 8 (8p11.21)
* **Rola biologiczna genu/białka:** IDO1 to enzym aktywowany przez cytokiny zapalne (głównie IFN-gamma, IL-6, TNF-alpha). Przekierowuje on metabolizm tryptofanu: zamiast do serotoniny i melatoniny, tryptofan jest masowo zużywany do produkcji neurotoksycznych metabolitów szlaku kynureninowego.

* **Mój genotyp (WGS):**
  `rs9657182` — **TT** (ensembl+bam, high) — marker_report


* **Genotyp Niskiej Aktywności (Odporność na depresję zapalną):** Mimo obecności obwodowego stanu zapalnego, enzym nie ulega gwałtownej nadekspresji. Tryptofan bezpiecznie zasilają syntezę serotoniny. Wysoka rezyliencja psychiczna w trakcie infekcji.
* **Genotyp Umiarkowany (Typ dziki):** Standardowa fizjologiczna odpowiedź immunologiczno-Metaboliczna.
* **Genotyp Wysokiej Aktywności (Homozygota ryzyka / Nadreaktywność):** Błyskawiczna indukcja IDO1 przy minimalnym wzroście cytokin. Dochodzi do "podkradania" tryptofanu – nagły spadek serotoniny wywołuje ciężką anhedonię, stany lękowe oraz tzw. "brain fog" i zachowania chorobowe (sickness behavior).

---
### IL6

* **Główny symbol genu:** IL6
* **Pełna nazwa biochemiczna:** Interleukina 6 (ang. *Interleukin 6*)
* **Nazwy potoczne i medialne:** Gen "mgły mózgowej" i zapalnego drenażu nastroju
* **Główny rsID:** rs1800795 (słynny polimorfizm -174G/C w regionie promotorowym)
* **Lokalizacja chromosomalna:** Chromosom 7 (7p15.3)
* **Rola biologiczna genu/białka:** Koduje plejotropową cytokinę, która w warunkach stresu psychofizycznego lub infekcji przenika przez barierę krew-mózg. W OUN aktywuje mikroglej, zaburza neurogenezę w hipokampie, obniża ekspresję BDNF i nasila aktywność enzymu IDO1, drastycznie ograniczając syntezę serotoniny.

* **Mój genotyp (WGS):**
  `rs1800795` — **GG** (ensembl+bam, high) — marker_report
* **Dopasowany profil:** Genotyp G/G (Wysoka podstawowa i reaktywna produkcja IL-6)



**Genotyp C/C (Niska podstawowa produkcja IL-6)**
"Wariant odporny na zapalenie". Bardzo niska spoczynkowa oraz stymulowana sekrecja interleukiny. Doskonała ochrona przed obwodowym stresem zapalnym, wysoka rezyliencja psychiczna w trakcie infekcji (brak tendencji do "mgły mózgowej").

**Genotyp C/G (Pośrednia ekspresja)**
Zrównoważona odpowiedź immunologiczna; typowy profil populacyjny.

**Genotyp G/G (Wysoka podstawowa i reaktywna produkcja IL-6)**
"Wariant hiper-zapalny". Błyskawiczna, potężna sekrecja IL-6 w odpowiedzi na najmniejszy stresor środowiskowy lub błąd dietetyczny. Generuje przewlekły mikrostan zapalny w mózgu (neuroinflammation). Silnie powiązany z patofizjologią ciężkiej depresji, stanów lękowych, chronicznego zmęczenia (CFS) oraz zaostrzeniem deficytów sensorycznych w ASD.

---

---
### KMO

* **Główny symbol genu:** KMO
* **Pełna nazwa biochemiczna:** Monooksygenaza kynureninowa (ang. *Kynurenine 3-monooxygenase*)
* **Nazwy potoczne i medialne:** Gen neurotoksyczności, generator kwasu chinolinowego
* **Główny rsID:** rs2275163 / rs10532
* **Lokalizacja chromosomalna:** Chromosom 1 (1q43.3)
* **Rola biologiczna genu/białka:** KMO przekształca kynureninę w 3-hydroksykynureninę (3-HK), co prowadzi bezpośrednio do powstawania kwasu chinolinowego (QUIN) – silnego agonisty receptorów NMDA, który w wysokich stężeniach indukuje napływ wapnia i śmierć ekscytotoksyczną neuronów.

* **Mój genotyp (WGS):**
  `rs2275163` — **CT** (ensembl+bam, high) — marker_report


* **Genotyp Niskiej Aktywności (Wariant neuroprotekcyjny):** Kynurenina jest chętniej przekształcana przez inny enzym (KAT) do kwasu kynurenionowego (KYNA), który blokuje receptory NMDA i chroni mózg. Niższe ryzyko stanów lękowych i psychotycznych.
* **Genotyp Pośredni:** Zrównoważony metabolizm kynurenin w OUN.
* **Genotyp Wysokiej Aktywności (Nadekspresja KMO):** Drastyczne przesunięcie metabolizmu w stronę neurotoksycznego kwasu chinolinowego (QUIN). Powoduje chroniczny mikrostan zapalny, niszczenie astrocytów, silny lęk wolnopłynący i jest mocno powiązany z patofizjologią epizodów maniakalnych oraz depresji endogennej odpornej na leczenie.

---

## BLOK 5: Oś Stresu (HPA), Kanały Jonowe i Homeostaza GABA

---
### MICA

* **Główny symbol genu:** MICA
* **Pełna nazwa biochemiczna:** Sekwencja A powiązana z polipeptydem MHC klasy I (ang. *MHC class I polypeptide-related sequence A*)
* **Nazwy potoczne i medialne:** Gen komórkowego sygnału alarmowego, aktywator komórek NK w mózgu
* **Główny rsID / Polimorfizm:** Polimorfizmy liczby powtórzeń mikrosatelitarnych (MICA-STR, np. allel A5.1)
* **Lokalizacja chromosomalna:** Chromosom 6 (6p21.33)
* **Rola biologiczna genu/białka:** MICA koduje białko stresu komórkowego, które nie prezentuje antygenów, lecz działa jako bezpośredni ligand dla aktywującego receptora NKG2D na komórkach NK (Natural Killer) oraz limfocytach T. Jest wskaźnikiem uszkodzenia komórki, infekcji lub przewlekłego stresu metabolicznego.

* **Mój genotyp (WGS):**
  `rs2596542` — **CC** (ensembl+bam, low) — ncbi; depth=1,avg=1.0; gt=0/0






**Typ Dziki (Alerty zbalansowane)**
Niski spoczynkowy poziom ekspresji. Białko MICA pojawia się na błonach komórkowych tylko w sytuacjach realnego, ostrego zagrożenia litycznego, po czym sygnał jest szybko wygaszany.

**Warianty o wydłużonej ekspresji (STR)**
Opóźnione wygaszanie sygnału alarmowego po przebytych infekcjach wirusowych lub bakteryjnych. Sprzyja przedłużaniu lokalnych odczynów zapalnych w tkankach obwodowych.

**Warianty Wysokiego Ryzyka (Niezrównoważona nadekspresja w OUN)**
Chroniczna, patologiczna ekspresja MICA na powierzchni astrocytów i mikrogleju w mózgu. Generuje ciągłą, bezpodstawną aktywację wrodzonej odpowiedzi odpornościowej. Komórki NK zaczynają niszczyć zdrowe neurony i nowo powstałe synapsy, napędzając permanentny neurostan zapalny (neuroinflammation) i indukcję cech ASD.

---

---
### TDO2

* **Główny symbol genu:** TDO2
* **Pełna nazwa biochemiczna:** 2,3-dioksygenaza tryptofanowa wątrobowa (ang. *Tryptophan 2,3-dioxygenase*)
* **Nazwy potoczne i medialne:** Gen stresowego drenażu tryptofanu, metaboliczny partner IDO1
* **Główny rsID:** rs1010816
* **Lokalizacja chromosomalna:** Chromosom 4 (4q32.1)
* **Rola biologiczna genu/białka:** TDO2 to drugi (obok IDO1) nadrzędny enzym inicjujący szlak kynureninowy. Kluczowa różnica polega na regulacji: podczas gdy IDO1 jest aktywowany przez cytokiny zapalne, **TDO2 reaguje bezpośrednio na glukokortykoidy (kortyzol)**. Odpowiada za masową degradację systemowego tryptofanu pod wpływem przewlekłego, psychologicznego stresu, uniemożliwiając jego wykorzystanie do produkcji serotoniny i melatoniny.

* **Mój genotyp (WGS):**
  `rs1010816` — **TT** (ensembl+bam, high) — marker_report



**Genotyp Niskiej Aktywności (Odporność na stresowy drenaż)**
Enzym wykazuje słabą reaktywność na kortyzol. Nawet przy chronicznym stresie pula tryptofanu pozostaje nienaruszona, zabezpieczając ciągłość syntezy neurotransmiterów. Wysoka rezyliencja psychiczna pod presją.

**Genotyp Pośredni (Typ dziki)**
Standardowa, fizjologiczna degradacja aminokwasów w wątrobie.

**Genotyp Wysokiej Aktywności (Hiper-reaktywność na kortyzol)**
Błyskawiczne "wypalanie" zapasów tryptofanu przy najmniejszym wzroście poziomu stresu. Dochodzi do natychmiastowego deficytu serotoniny i melatoniny (gwałtowny spadek nastroju, lęki, stany bezsenności), przy jednoczesnym zalaniu mózgu neurotoksycznymi produktami szlaku kynureniny (kwas chinolinowy).

---

---
### TNFA

* **Główny symbol genu:** TNF (TNFA)
* **Pełna nazwa biochemiczna:** Czynnik martwicy nowotworu alfa (ang. *Tumor necrosis factor alpha*)
* **Nazwy potoczne i medialne:** Główny molekularny przełącznik zapalny, gen degradacji synaps
* **Główny rsID:** rs1800629 (polimorfizm -308G/A w promotorze)
* **Lokalizacja chromosomalna:** Chromosom 6 (6p21.33) w obrębie locus HLA klasy III
* **Rola biologiczna genu/białka:** TNF-alpha to nadrzędna cytokina prozapalna kaskady immunologicznej. W mózgu jest wydzielana przez aktywowany mikroglej. Stymuluje astrocyty do masowego, patologicznego uwalniania glutaminianu (indukcja ekscytotoksyczności) przy jednoczesnym wymuszaniu endocytozy (fagocytozy) postsynaptycznych receptorów AMPA, co prowadzi do fizycznego demontażu i zaniku połączeń synaptycznych.

* **Mój genotyp (WGS):**
  `rs1800629` — **GG** (ensembl+bam, medium) — ncbi; depth=8,avg=8.0; gt=0/0
* **Dopasowany profil:** Genotyp G/G (Standardowa, niska produkcja TNF)






**Genotyp G/G (Standardowa, niska produkcja TNF)**
Prawidłowa kontrola nad procesami immunologicznymi. Brak bezpodstawnej aktywacji kaskady litycznej, stabilna gęstość synaptyczna.

**Genotyp G/A (Umiarkowanie podwyższona ekspresja)**
Zwiększona podatność na odczyny zapalne tkanek obwodowych i przejściowe zmęczenie kognitywne pod wpływem stresu.

**Genotyp A/A (Ekstremalna, permanentna nadekspresja TNF-alpha)**
Potężna, destrukcyjna siła zapalna. Przewlekły stan wysokiej ekspresji wywołuje masową utratę kolców dendrytycznych i uszkodzenie mitochondriów w neuronach. W badaniach klinicznych wariant ten wykazuje potężną korelację z autyzmem regresywnym (nagła utrata mowy i kompetencji u dzieci), chorobą Alzheimera oraz ciężkim, destrukcyjnym przebiegiem faz maniakalnych i depresyjnych w ChAD.

---

---

## CZĘŚĆ VII. Oś stresu (HPA), hormony i modulatory behawioralne
### CRHR1

* **Główny symbol genu:** CRHR1
* **Pełna nazwa biochemiczna:** Receptor czynnika uwalniającego kortykotropinę 1 (ang. *Corticotropin-releasing hormone receptor 1*)
* **Nazwy potoczne i medialne:** Gen osi stresu, receptor CRF-1
* **Główny rsID / Haplotyp:** rs11078013 / rs2425512 (silnie sprzężone z megablokiem inwersyjnym H1/H2 na 17q21.31)
* **Lokalizacja chromosomalna:** Chromosom 17 (17q21.31)
* **Rola biologiczna genu/białka:** CRHR1 to główny receptor pośredniczący w odpowiedzi na stres w ośrodkowym układzie nerwowym (oś podwzgórze-przysadka-nadnercza - HPA). Aktywacja tego receptora przez CRH reguluje stany lękowe, czujność, reakcje emocjonalne oraz wchodzi w bezpośrednią interakcję funkcjonalną z cytoszkieletem aksonalnym sąsiadującego białka tau.

* **Mój genotyp (WGS):**
  `rs11078013` — **GG** (ensembl+bam, high) — marker_report



**Diplotyp H1/H1 (Wysoka reaktywność osi stresu)**
Zwiększona gęstość i ekspresja receptorów CRHR1. Nadreaktywność na bodźce stresowe, wysoka czujność środowiskowa, ale też silna podatność na traumę wczesnodziecięcą, rozwój zaburzeń lękowych i depresyjnych.

**Diplotyp H1/H2 (Umiarkowana reaktywność)**
Optymalna homeostaza glukokortykoidowa. Zrównoważona odpowiedź kortyzolowa na bodźce ostre, wysoka rezyliencja psychiczna i sprawna samoregulacja emocjonalna pod presją.

**Diplotyp H2/H2 (Niska reaktywność / Ryzyko strukturalne)**
Tłumiona odpowiedź osi HPA, skłonność do specyficznych, nietypowych reakcji behawioralnych na stres lub tendencja do apatii. Ze względu na lokalizację w bloku H2, współwystępuje z podwyższonym ryzykiem mikrostrukturalnych zaburzeń kory mózgowej i fenotypu autystycznego.

---

---
### GPR151

* **Główny symbol genu:** GPR151
* **Pełna nazwa biochemiczna:** Receptor 151 sprzężony z białkiem G (ang. *G-protein coupled receptor 151*)
* **Nazwy potoczne i medialne:** Gen bramkowania uzależnień w uzdeczce (habenula)
* **Główny rsID:** rs11652495
* **Lokalizacja chromosomalna:** Chromosom 5 (5q32)
* **Rola biologiczna genu/białka:** Koduje sierocy receptor sprzężony z białkiem G, wykazujący unikalną, wysoce specyficzną ekspresję w uzdeczce przyśrodkowej (medial habenula) – strukturze pełniącej funkcję "węzła rozczarowania", która hamuje układ nagrody po negatywnych doświadczeniach. GPR151 moduluje przekaźnictwo nikotynowe i dopaminergiczne w tej osi.

* **Mój genotyp (WGS):**
  `rs11652495` — **CC** (ensembl+bam, high) — marker_report



**Genotyp Referencyjny (Wysoka ekspresja w uzdeczce)**
Sprawne działanie układów awersyjnych. Prawidłowe uczenie się na błędach, niska skłonność do wpadania w chroniczne pętle nałogowe, wysoka kontrola impulsów.

**Genotyp Pośredni (Wariant populacyjny)**
Standardowa czułość struktur uzdeczki na bodźce awersyjne.

**Genotyp Ryzyka (Niska ekspresja / Obniżone powinowactwo)**
Osłabienie sygnalizacji w "węźle rozczarowania". Mózg słabiej rejestruje negatywne skutki zachowań ryzykownych. W badaniach GWAS wariant ten silnie predysponuje do ciężkiego uzależnienia od nikotyny, podatności na kompulsywne zachowania poszukiwania nagrody (novelty seeking) oraz trudności w gaszeniu wyuczonych lęków.

---

## BLOK 18: Adhezja, Migracja i Neurogeneza

---
### NR3C1

* **Główny symbol genu:** NR3C1
* **Pełna nazwa biochemiczna:** Receptor glukokortykoidowy (ang. *Nuclear receptor subfamily 3 group C member 1*)
* **Nazwy potoczne i medialne:** Gen podatności na traumę, receptor kortyzolu
* **Główny rsID / Polimorfizm:** rs41423247 (polimorfizm *BclI* - wariant intronowy)
* **Lokalizacja chromosomalna:** Chromosom 5 (5q31.3)
* **Rola biologiczna genu/białka:** Koduje receptor GR, który wiąże kortyzol i działa jako czynnik transkrypcyjny wygaszający oś stresu HPA poprzez ujemne sprzężenie zwrotne. Odpowiada za biochemiczne "zresetowanie" organizmu po ustąpieniu zagrożenia.

* **Mój genotyp (WGS):**
  `rs41423247` — **GG** (ensembl+bam, high) — ensembl; depth=35,avg=35.0; gt=0/0
* **Dopasowany profil:** Wariant G/G (Wysoka czułość na kortyzol / Podatność na stres):





* **Wariant G/G (Wysoka czułość na kortyzol / Podatność na stres):** Nadreaktywność na glukokortykoidy. Błyskawiczne wygaszanie osi HPA, ale kosztem skłonności do chronicznie niskiego poziomu spoczynkowego kortyzolu (hipokortyzolemia). Silna korelacja z zespołem stresu pourazowego (PTSD), wypaleniem zawodowym i chronicznym zmęczeniem.
* **Wariant C/G (Pośredni):** Prawidłowa, plastyczna odpowiedź receptorowa na stres środowiskowy.
* **Wariant C/C (Niska czułość na kortyzol / Rezyliencja):** Zmniejszona wrażliwość receptora GR. Oś HPA potrzebuje więcej czasu na wygaszenie (dłuższy wyrzut kortyzolu), co daje wysoką wrodzoną odporność na traumę psychologiczną i brak ulegania paraliżowi w kryzysie.

---
### NR3C2

* **Główny symbol genu:** NR3C2
* **Pełna nazwa biochemiczna:** Receptor mineralokortykoidowy (ang. *Nuclear receptor subfamily 3 group C member 2*)
* **Nazwy potoczne i medialne:** Gen spoczynkowego tonusu osi stresu, receptor MR
* **Główny rsID:** rs5522 (polimorfizm I180V)
* **Lokalizacja chromosomalna:** Chromosom 4 (4q31.23)
* **Rola biologiczna genu/białka:** Receptor MR wykazuje ekstremalnie wysokie powinowactwo do glukokortykoidów – wiąże kortyzol przy jego niskich, spoczynkowych stężeniach (w przeciwieństwie do opisywanego wcześniej NR3C1/GR, który aktywuje się przy ostrym stresie). Odpowiada za dobowy rytm wydzielania kortyzolu, utrzymanie podstawowego nastroju oraz plastyczność synaptyczną hipokampu w warunkach relaksu.

* **Mój genotyp (WGS):**
  `rs5522` — **TT** (ensembl+bam, high) — marker_report



**Genotyp A/A (Wariant referencyjny / Optymalna czułość MR)**
Stabilny spoczynkowy tonus osi HPA. Prawidłowy dobowy profil kortyzolu, wysoka sprawność pamięci przestrzennej i stabilny, bazowy nastrój w ciągu dnia.

**Genotyp A/G (Umiarkowana elastyczność)**
Standardowa, prawidłowa reaktywność spoczynkowa; typowy profil populacyjny.

**Genotyp G/G (Wariant V180 / Obniżona czułość MR)**
Słabsze wiązanie spoczynkowego kortyzolu. Powoduje chroniczne, kompensacyjne podwyższenie bazowego poziomu kortyzolu i przyspieszone wybudzenia nocne. Silnie koreluje z podatnością na depresję melancholijną, lękiem uogólnionym oraz gorszą plastycznością hipokampa pod wpływem rutynowych wyzwań.

---

---

## CZĘŚĆ VIII. Kanały jonowe i homeostaza elektrofizjologiczna
### CACNB2

* **Główny symbol genu:** CACNB2
* **Pełna nazwa biochemiczna:** Podjednostka beta-2 napięciowo-zależnego kanału wapniowego (ang. *Calcium voltage-gated channel subunit beta 2*)
* **Nazwy potoczne i medialne:** "Cross-disorder gene" (wspólny gen zaburzeń psychicznych)
* **Główny rsID:** rs1051375
* **Lokalizacja chromosomalna:** Chromosom 10 (10p12.33)
* **Rola biologiczna genu/białka:** Reguluje gęstość ekspresji i kinetykę otwierania kanałów wapniowych typu L w błonie neuronów. Napływ wapnia przez te kanały decyduje o sile wyładowania elektrycznego, uwalnianiu neurotransmiterów oraz aktywacji transkrypcji zależnej od aktywności (CREB).

* **Mój genotyp (WGS):**
  `rs1051375` — **GA** (query_rsid, high)


* **Genotyp Referencyjny (Stabilny prąd wapniowy):** Precyzyjne dozowanie jonów wapnia. Zrównoważona odpowiedź synaptyczna, stabilna transmisja sygnałów bez generowania szumu elektrofizjologicznego.
* **Genotyp Pośredni:** Prawidłowe populacyjne funkcjonowanie kanałów napięciowych.
* **Genotyp Ryzyka (Homozygota GWAS):** Patologiczne wydłużenie czasu otwarcia kanału wapniowego. Powoduje przewlekły nadmiar intracelularnego wapnia w neuronach kory przedczołowej i struktur limbicznych. W międzynarodowych megabadaniach genetycznych PGC wariant ten okazał się **wspólnym mianownikiem plejotropowym**, podnoszącym u tego samego pacjenta ryzyko jednoczesnego wystąpienia: ADHD, ChAD, schizofrenii oraz ciężkiej depresji nawracającej.

---

## BLOK 6: Farmakogenetyka i Bariery Krew-Mózg

---
### HCN1

* **Główny symbol genu:** HCN1
* **Pełna nazwa biochemiczna:** Kanał potasowo-sodowy aktywowany hiperpolaryzacją i modulowany cyklicznymi nukleotydami 1 (ang. *Hyperpolarization activated cyclic nucleotide gated potassium channel 1*)
* **Nazwy potoczne i medialne:** Gen prądu rozrusznikowego mózgu (prąd Ih), gen rytmu neuronalnego
* **Główny rsID / Wariant:** Warianty eQTL / Mutacje de novo LoF lub Gain-of-Function
* **Lokalizacja chromosomalna:** Chromosom 5 (5p12)
* **Rola biologiczna genu/białka:** Koduje kanał generujący prąd rozrusznikowy $I_h$. Kanał ten otwiera się paradoksalnie przy *hiperpolaryzacji* błony, generując dokomórkowy prąd depolaryzacyjny. Odpowiada za generowanie rytmicznej aktywności oscylacyjnej sieci neuronalnych (szczególnie w hipokampie i korze) oraz kontroluje spoczynkowy potencjał błonowy.

* **Mój genotyp (WGS):**
  `rs789088` — **CC** (ensembl+bam, high) — ncbi; depth=62,avg=62.0; gt=0/0





* **Typ Dziki (Prawidłowa kinetyka Ih):** Stabilne oscylacje sieciowe (rytm theta i gamma), niezbędne do prawidłowego kodowania informacji przestrzennych i pamięciowych. Ochrona neuronów przed nadmiernym wyciszeniem lub hiperreaktywnością.
* **Warianty Populacyjne (Łagodne przesunięcia napięciowe):** Drobne modyfikacje w amplitudzie oscylacji, wpływające na indywidualne predyspozycje do głębokości faz snu.
* **Mutacja Patogenna (De novo GoF lub LoF):** Katastrofalna destabilizacja elektryczna mózgu. Mutacje zwiększające funkcję (GoF) wywołują ciężką, wczesnodziecięcą encefalopatię padaczkową (zespół o charakterze zbliżonym do zespołu Dravet) z głęboką regresją rozwoju intelektualnego, ataksją ruchową i utratą kompetencji komunikacyjnych (ASD).

---
### KCNN2

* **Główny symbol genu:** KCNN2
* **Pełna nazwa biochemiczna:** Kanał potasowy aktywowany wapniem o małej przewodności, podrodzina N, składowa 2 (ang. *Potassium calcium-activated channel subfamily N member 2*)
* **Nazwy potoczne i medialne:** Gen SK2, strażnik pobudliwości synaptycznej
* **Główny rsID / Wariant:** Polimorfizmy eQTL lub rzadkie mutacje punktowe
* **Lokalizacja chromosomalna:** Chromosom 5 (5q22.3)
* **Rola biologiczna genu/białka:** Koduje kanał potasowy SK2, który otwiera się w odpowiedzi na wzrost stężenia intracelularnego wapnia. Generuje prąd hiperpolaryzacji pościegłowej (AHP), pełniąc funkcję "hamulca bezpieczeństwa" – ogranicza częstotliwość wyładowań neuronu i chroni go przed przestymulowaniem. Kluczowy dla plastyczności synaptycznej w hipokampie.

* **Mój genotyp (WGS):**
  `rs6480535` — **TT** (ensembl+bam, high) — ncbi; depth=45,avg=45.0; gt=0/0






**Genotyp Wysokiej Aktywności (Silne hamowanie)**
Błyskawiczna repolaryzacja i wysoka ochrona przed ekscytotoksycznością. Może podnosić próg aktywacji kognitywnej (potrzeba silniejszych bodźców środowiskowych do indukcji procesów zapamiętywania).

**Genotyp Umiarkowany (Typ dziki)**
Zbalansowana pobudliwość synaptyczna. Idealne okno elektrofizjologiczne dla indukcji długotrwałego wzmocnienia synaptycznego (LTP) bez ryzyka generowania wyładowań drgawkowych.

**Genotyp Niskiej Aktywności (Wariant ryzyka / LoF)**
Utrata kontroli nad wygaszaniem pobudzenia. Przewlekła hiperaktywność sieci neuronalnych, drastyczne zaburzenia integracji sensorycznej (brak filtrowania bodźców), podatność na padaczkę oraz korelacja z zachowaniami stereotypowymi w spektrum autyzmu.

---

---
### KCNQ3

* **Główny symbol genu:** KCNQ3
* **Pełna nazwa biochemiczna:** Napięciowo-zależny kanał potasowy, podrodzina Q, składowa 3 (ang. *Potassium voltage-gated channel subfamily Q member 3*)
* **Nazwy potoczne i medialne:** Gen prądu M (M-current), gen stabilizacji progu drgawkowego
* **Główny rsID / Wariant:** rs13271739 / Mutacje punktowe de novo
* **Lokalizacja chromosomalna:** Chromosom 8 (8q24.22)
* **Rola biologiczna genu/białka:** Wspólnie z podjednostką KCNQ2 tworzy funkcjonalny kanał potasowy generujący tzw. prąd M. Jest to prąd potasowy aktywowany powolną depolaryzacją, który nie ulega inaktywacji. Pełni rolę nadrzędnego stabilizatora potencjału spoczynkowego neuronu – zapobiega generowaniu powtarzalnych, seryjnych wyładowań iglicowych (bursting).

* **Mój genotyp (WGS):**
  `rs13271739` — **CG** (ensembl+bam, high) — marker_report


* **Typ Dziki (Maksymalna stabilizacja błonowa):** Wydajne tłumienie seryjnych wyładowań. Wysoki próg drgawkowy, doskonała ochrona przed przestymulowaniem sensorycznym, stabilna praca sieci korowych.
* **Warianty Populacyjne (Subtelne wahania ekspresji):** Niewielkie różnice w podatności na zmęczenie neuroelektryczne; sporadyczne bóle głowy typu migrenowego przy przeciążeniach bodźcami.
* **Mutacja Patogenna (LoF - Utrata funkcji):** Łagodne rodzinne drgawki noworodków (BFNC) lub ciężkie encefalopatie neurorozwojowe. Brak prądu M powoduje, że neurony po pojedynczym bodźcu wpadają w niekontrolowane, permanentne serie wyładowań. Efekt kliniczny: ciężka hiperreaktywność sensoryczna, zaburzenia uwagi, niepełnosprawność intelektualna i autyzm.

---

## CZĘŚĆ IX. Adhezja, migracja neuronów i architektura synaptyczna
### CSMD1

* **Główny symbol genu:** CSMD1
* **Pełna nazwa biochemiczna:** Białko 1 zawierające domeny CUB i powtórzenia typu Sushi (ang. *CUB and Sushi multiple domains 1*)
* **Nazwy potoczne i medialne:** Gen ochrony przed nadmiernym przycinaniem synaps, top-hit GWAS schizofrenii
* **Główny rsID:** rs10503253 / rs2023306
* **Lokalizacja chromosomalna:** Chromosom 8 (8p23.2)
* **Rola biologiczna genu/białka:** Olbrzymie białko błonowe wykazujące silną ekspresję w stożkach wzrostu aksonów i w synapsach. CSMD1 działa jako inhibitor kaskady dopełniacza – hamuje odkładanie się białek C3b i C4A na zdrowych połączeniach synaptycznych, chroniąc je przed niesłusznym "pożarciem" przez mikroglej.

* **Mój genotyp (WGS):**
  `rs10503253` — **CC** (ensembl+bam, high) — marker_report


* **Genotyp Referencyjny (Wysoka ochrona synaps):** Precyzyjne blokowanie dopełniacza. Wysoka gęstość połączeń korowych, doskonała pamięć, prawidłowa plastyczność i pełna odporność na patologiczny pruning (przycinanie).
* **Genotyp Pośredni (Heterozygota):** Zrównoważona dynamika ochrony i eliminacji połączeń synaptycznych.
* **Genotyp Ryzyka (Niska ekspresja / Sygnał GWAS):** Drastyczny spadek ochrony synaptycznej. Białko C4A bez przeszkód znakuje zdrowe, aktywne połączenia neuronalne. Mikroglej masowo niszczy sieć synaptyczną w korze przedczołowej. Wariant ten jest jednym z najbardziej powtarzalnych sygnałów w genomicznych badaniach nad schizofrenią, spektrum autyzmu oraz zaburzeniami poznawczymi.

---
### DCC

* **Główny symbol genu:** DCC
* **Pełna nazwa biochemiczna:** Receptor netryny-1 (ang. *DCC netrin 1 receptor*)
* **Nazwy potoczne i medialne:** Gen przekraczania linii pośrodkowej, architekt spoidła wielkiego
* **Główny rsID / Wariant:** rs715 / Mutacje patogenne typu utraty funkcji
* **Lokalizacja chromosomalna:** Chromosom 18 (18q21.2)
* **Rola biologiczna genu/białka:** Receptor dla netryny-1, kluczowy dla mechanizmu naprowadzania aksonów (axon guidance). Odpowiada za przyciąganie rosnących aksonów w kierunku linii pośrodkowej ciała, decydując o prawidłowym skrzyżowaniu dróg ruchowych (piramidowych) oraz uformowaniu spoidła wielkiego (ciała modzelowatego).

* **Mój genotyp (WGS):**
  `rs715` — **TC** (ensembl+bam, high) — marker_report


* **Typ Dziki (Prawidłowa lateralizacja i spoidła):** Perfekcyjne uformowanie połączeń międzypółkulowych. Prawidłowe skrzyżowanie dróg korowo-rdzeniowych, wysoka koordynacja ruchów obustronnych (biliterealnych), prawidłowa lateralizacja funkcji mózgowych.
* **Warianty Populacyjne (Łagodne):** Subtelne różnice w gęstości włókien spoidłowych, wpływające na indywidualny profil asymetrii półkulowej.
* **Mutacja Patogenna (Heterozygota LoF):** Wywołuje **wrodzone ruchy lustrzane** (ang. *congenital mirror movements*) – patologię, w której ruch jednej ręki jest bezwiednie i symetrycznie kopiowany przez drugą rękę z powodu braku skrzyżowania dróg nerwowych. Ponadto mutacje w tym genie silnie korelują z agenezją ciała modzelowatego, głęboką niepełnosprawnością intelektualną oraz fenotypem autystycznym.

---
### FURIN

* **Główny symbol genu:** FURIN
* **Pełna nazwa biochemiczna:** Furyna, konwertaza proproteinowa (ang. *Furin*)
* **Nazwy potoczne i medialne:** Gen dojrzałości BDNF, kluczowy sygnał GWAS schizofrenii
* **Główny rsID:** rs4702
* **Lokalizacja chromosomalna:** Chromosom 15 (15q26.1)
* **Rola biologiczna genu/białka:** Proteaza serynowa, która w mózgu odpowiada za enzymatyczne cięcie prekursora neurotrofiny (proBDNF) do jej dojrzałej, funkcjonalnej formy (**mBDNF**). Podczas gdy proBDNF promuje apoptozę i usuwanie synaps (poprzez receptor p75NTR), mBDNF stymuluje wzrost neuronów i LTP (poprzez receptor TrkB). Furin decyduje o tym krytycznym dla mózgu balansie.

* **Mój genotyp (WGS):**
  `rs4702` — **GA** (ensembl+bam, high) — marker_report
* **Dopasowany profil:** Genotyp A/G (Umiarkowany)



**Genotyp A/A (Wysoka wydajność Furin / Pro-kognitywny)**
Maksymalne, wydajne przekształcanie do dojrzałego mBDNF. Wysoki potencjał neuroplastyczny, stabilna ochrona synaps przed niszczeniem, doskonałe procesy uczenia się i wysoka rezyliencja psychiczna.

**Genotyp A/G (Umiarkowany)**
Zbalansowany stosunek proBDNF do mBDNF; standardowy profil populacyjny.

**Genotyp G/G (Niska wydajność / Sygnał ryzyka schizofrenii)**
Znaczny spadek ekspresji tkankowej Furin. Nadmiar proBDNF kosztem niedoboru funkcjonalnego mBDNF. Skutkuje to przyspieszonym, patologicznym zanikiem synaps, zaburzeniami rozwoju kory mózgowej w okresie dorastania i stanowi jeden z wiodących genetycznych markerów ryzyka w etiologii schizofrenii.

---

## BLOK 16: Farmakogenetyka, Detoksykacja i Homeostaza Metaboliczna

---
### NCAM1

* **Główny symbol genu:** NCAM1
* **Pełna nazwa biochemiczna:** Cząsteczka adhezji komórek nerwowych 1 (ang. *Neural cell adhesion molecule 1*)
* **Nazwy potoczne i medialne:** Gen molekularnego rusztowania synaps, marker GWAS ChAD i schizofrenii
* **Główny rsID:** rs1943620 / rs5835
* **Lokalizacja chromosomalna:** Chromosom 11 (11q23.2)
* **Rola biologiczna genu/białka:** Glikoproteina błonowa zaangażowana w fizyczną adhezję komórka-komórka, wzrost neurytów, synaptogenezę oraz indukcję długotrwałego wzmocnienia synaptycznego (LTP) w hipokampie. Poprzez bezpośrednią interakcję z receptorami FGFR aktywuje wewnątrzkomórkowe kaskady sygnałowe decydujące o przeżywalności neuronów.

* **Mój genotyp (WGS):**
  `rs1943620` — **GA** (ensembl+bam, high) — marker_report



**Typ Dziki (Prawidłowa adhezja i stabilizacja synaps)**
Wysoka integralność strukturalna połączeń synaptycznych przy zachowaniu pełnego potencjału neuroplastycznego. Sprawne procesy uczenia się i reorganizacji sieci neuronalnych.

**Warianty Polimorficzne (Średnia ekspresja)**
Standardowa populacyjna plastyczność strukturalna tkanki nerwowej.

**Genotyp Ryzyka (Niska ekspresja / Sygnał GWAS)**
Zaburzenia mechanicznej stabilności synaps i osłabienie sygnalizacji neurotroficznej. Stanowi powtarzalny sygnał ryzyka w międzynarodowych badaniach genomicznych dla schizofrenii, choroby afektywnej dwubiegunowej oraz ciężkiej depresji nawracającej.

---

---
### NCAN

* **Główny symbol genu:** NCAN
* **Pełna nazwa biochemiczna:** Neurokan (ang. *Neurocan*)
* **Nazwy potoczne i medialne:** Składnik cementu międzykomórkowego mózgu, sygnał GWAS ChAD
* **Główny rsID:** rs1064395
* **Lokalizacja chromosomalna:** Chromosom 19 (19p13.11)
* **Rola biologiczna genu/białka:** Koduje neurokan – proteoglikan siarczanu chondroityny, będący fundamentalnym składnikiem macierzy zewnątrzkomórkowej (ECM) w mózgu. Bierze udział w adhezji komórek, hamuje wzrost neurytów w dojrzałym mózgu (stabilizuje sieć), tworzy tzw. sieci perineuronalne (PNN) owijające neurony hamujące GABA.

* **Mój genotyp (WGS):**
  `rs1064395` — **GG** (ensembl+bam, high) — marker_report
* **Dopasowany profil:** Genotyp G/G (Wariant ryzyka afektywnego):


* **Genotyp A/A (Wariant standardowy):** Stabilna, prawidłowo uwodniona macierz zewnątrzkomórkowa. Dobrze wykształcone sieci perineuronalne chroniące neurony GABAergiczne, wysoka stabilność neurochemiczna.
* **Genotyp A/G (Pośredni):** Standardowa elastyczność i gęstość macierzy tkanki nerwowej.
* **Genotyp G/G (Wariant ryzyka afektywnego):** Zmieniona struktura neurokanu upośledza formowanie sieci perineuronalnych wokół neuronów hamujących. Skutkuje to utratą osłony komórkowej i podatnością na destabilizację elektrofizjologiczną. W badaniach genomicznych wariant ten wykazuje potężną, powtarzalną asocjację z **chorobą afektywną dwubiegunową (ChAD)**, manią oraz epizodami psychotycznymi.

---
### NEGR1

* **Główny symbol genu:** NEGR1
* **Pełna nazwa biochemiczna:** Molekuła adhezyjna wzrostu neuronalnego 1 (ang. *Neuronal growth regulator 1*)
* **Nazwy potoczne i medialne:** Gen top-hit GWAS otyłości, inteligencji i gęstości synaptycznej
* **Główny rsID:** rs3101336 / rs2843289
* **Lokalizacja chromosomalna:** Chromosom 1 (1q31.1)
* **Rola biologiczna genu/białka:** Białko zakotwiczone w błonie przez GPI, należące do rodziny IgLON. Kontroluje wzrost neurytów, powstawanie wypustek synaptycznych oraz fizyczne dopasowanie struktur komórkowych w hipokampie. Ponadto wykazuje silną ekspresję w podwzgórzu, kontrolując anatomię obwodów łaknienia.

* **Mój genotyp (WGS):**
  `rs3101336` — **CC** (ensembl+bam, high) — ensembl; depth=56,avg=56.0; gt=1/1






**Genotyp Referencyjny (Wariant prokognitywny / Ochronny przed otyłością)**
Wysoka ekspresja NEGR1. Precyzyjne formowanie kolców dendrytycznych w hipokampie (wysokie IQ, doskonała pamięć) oraz prawidłowo zaprogramowany próg sytości w podwzgórzu.

**Genotyp Pośredni (Heterozygota)**
Standardowe populacyjne parametry plastyczności synaptycznej i stabilny metabolizm.

**Genotyp alleliczny C/C (tag-SNP GWAS)**
Profil populacyjny rs3101336 (C/C). W międzynarodowych badaniach GWAS koreluje ze statystyczną podatnością na otyłość, ADHD i schizofrenię — nie oznacza klinicznej niepełnosprawności intelektualnej ani defektu genu NEGR1.

---

---
### PCDHG

* **Główny symbol genu:** PCDHG (locus obejmujący geny od A1 do C5)
* **Pełna nazwa biochemiczna:** Protokadheryny gamma (ang. *Protocadherin gamma gene cluster*)
* **Nazwy potoczne i medialne:** Geny unikalnego kodu kreskowego neuronu, architekci samo-unikania synaptycznego
* **Główny wariant:** Zmienność strukturalna / Metylacja promotorów w klastrze 5q31
* **Lokalizacja chromosomalna:** Chromosom 5 (5q31.3)
* **Rola biologiczna genu/białka:** Gigantyczny klaster kodujący homofilowe cząsteczki adhezji komórkowej. Protokadheryny gamma tworzą na powierzchni każdego neuronu unikalny "kod kreskowy". Umożliwia to proces samo-unikania (ang. *self-avoidance*): dendryty tego samego neuronu odpychają się, co wymusza ich prawidłowe, szerokie rozprzestrzenienie przestrzenne bez tworzenia pętli autozapętleń. Klaster decyduje też o selektywnym tworzeniu synaps z innymi komórkami.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Prawidłowa Ekspresja Klastra (Maksymalna różnorodność kodów)**
Idealna, rozłożysta architektura drzewek dendrytycznych. Sprawne tworzenie sieci synaptycznych, wysoka plastyczność i prawidłowe mapowanie sensoryczne w korze.

**Modyfikacje Epigenetyczne (Hipermetylacja promotorów)**
Subtelne zawężenie kombinatoryki białkowej. Może skutkować lokalnym zagęszczeniem lub skrzyżowaniem wypustek, objawiającym się drobnymi trudnościami w integracji sensorycznej.

**Mutacja Patogenna / Zablokowanie Ekspresji Klastra**
Katastrofalne zaburzenie samo-unikania. Dendryty gubią drogę, tworząc patologiczne zwoje i autopsynapsy. Skutkuje to masową apoptozą neuronów w fazie rozwoju, głęboką mikrocefalią, lekooporną padaczką oraz drastycznym fenotypem ASD z mutyzmem.

---

---
### PTPRE

* **Główny symbol genu:** PTPRE
* **Pełna nazwa biochemiczna:** Receptorowa fosfataza tyrozynowa typu E (ang. *Protein tyrosine phosphatase receptor type E*)
* **Nazwy potoczne i medialne:** Modulator sygnału receptorów NMDA i stabilizacji aksonów
* **Główny rsID:** rs708453
* **Lokalizacja chromosomalna:** Chromosom 10 (10q26.2)
* **Rola biologiczna genu/białka:** Usuwa grupy fosforanowe z kluczowych białek sygnałowych. W mózgu działa jako negatywny regulator aktywności kinaz z rodziny Src, bezpośrednio tonizując i wygaszając nadmierną fosforylację postsynaptycznych receptorów NMDA oraz uczestnicząc w naprowadzaniu aksonów i mielinizacji.

* **Mój genotyp (WGS):**
  `rs708453` — **GG** (ensembl+bam, high) — marker_report



**Typ Dziki (Zrównoważona defosforylacja)**
Precyzyjne wygaszanie prądów wapniowych w receptorach NMDA. Stabilna ochrona przed ekscytotoksycznością, prawidłowy profil plastyczności synaptycznej.

**Warianty Polimorficzne (Średnia aktywność)**
Standardowe parametry kinetyki fosfatazowej w populacji.

**Wariant Ryzyka (Niska ekspresja PTPRE)**
Utrata kontroli nad wygaszaniem kinaz Src. Skutkuje chroniczną nadreaktywnością receptorów NMDA (nadmierny napływ wapnia po pobudzeniu). Koreluje z hiperreaktywnością sensoryczną, podatnością na stany lękowe oraz podwyższonym ryzykiem zaburzeń afektywnych.

---

---
### PTPRF

* **Główny symbol genu:** PTPRF
* **Pełna nazwa biochemiczna:** Receptorowa fosfataza tyrozynowa typu F (ang. *Protein tyrosine phosphatase receptor type F / LAR phosphatase*)
* **Nazwy potoczne i medialne:** Gen budowniczy strefy aktywnej synaps
* **Główny rsID:** rs1049581
* **Lokalizacja chromosomalna:** Chromosom 1 (1p34.2)
* **Rola biologiczna genu/białka:** Koduje fosfatazę LAR, która lokalizuje się presynaptycznie i wchodzi w bezpośrednie interakcje strukturalne z białkami rusztowania synaptycznego (np. liprynami-alpha). Jest niezbędna do fizycznego zakotwiczenia pęcherzyków synaptycznych w strefie aktywnej oraz kontrolowania morfogenezy kolców dendrytycznych.

* **Mój genotyp (WGS):**
  - `rs1049581` — **AG** (ensembl+bam, high) — marker_report
  - `rs10996110` — **GG** (ensembl, high) — ensembl; depth=59,avg=59.0; gt=0/0



**Typ Dziki (Prawidłowa gęstość synaptyczna)**
Stabilne, precyzyjnie uformowane synapsy w korze mózgowej i hipokampie. Wysoka wierność transmisji sygnałów bez utraty pęcherzyków.

**Warianty Populacyjne (Łagodne)**
Niewielkie wahania w tempie remodelowania synaps, bez wyraźnych deficytów poznawczych.

**Obniżona Ekspresja / Wariant Ryzyka LoF**
Osłabienie strukturalnego wiązania synaptycznego. Prowadzi do przedwczesnej destabilizacji kolców dendrytycznych i utraty połączeń korowych. W badaniach neuropsychiatrycznych powiązany z deficytami funkcji wykonawczych, podatnością na schizofrenię oraz zaburzeniami uwagi.

---

## BLOK 19: Modyfikacje Epigenetyczne RNA i Translacja

---
### RET

* **Główny symbol genu:** RET
* **Pełna nazwa biochemiczna:** Protoonkogen RET, receptorowa kinaza tyrozynowa (ang. *RET proto-oncogene*)
* **Nazwy potoczne i medialne:** Gen osi jelitowo-mózgowej, receptor czynników GDNF, gen choroby Hirschsprunga
* **Główny rsID / Wariant:** rs1800858 / Mutacje utraty funkcji (LoF)
* **Lokalizacja chromosomalna:** Chromosom 10 (10q11.21)
* **Rola biologiczna genu/białka:** Koduje receptorową kinazę tyrozynową dla ligandów z rodziny GDNF (glejowych czynników neurotroficznych). RET jest absolutnie krytyczny dla migracji i różnicowania komórek grzebienia nerwowego, które formują Autonomiczny (Jelitowy) Układ Nerwowy (ENS) oraz jądra dopaminergiczne w śródmózgowiu.

* **Mój genotyp (WGS):**
  `rs1800858` — **AG** (ensembl+bam, high) — marker_report



**Typ Dziki (Prawidłowa ekspresja RET)**
Pełne unaczynienie i prawidłowe unerwienie splotów jelitowych (Auerbacha i Meissnera). Sprawna motoryka przewodu pokarmowego, prawidłowy rozwój linii dopaminergicznych, doskonała komunikacja na osi jelita-mózg.

**Warianty Polimorficzne o Niskiej Penetracji**
Skłonność do czynnościowych zaburzeń jelitowych (np. IBS o typie zaparciowym) pod wpływem stresu psychofizycznego.

**Mutacja Patogenna (Heterozygota LoF)**
**Choroba Hirschsprunga** (wrodzone neurogenne zaparcie megacolon z powodu agenezji splotów jelitowych). U pacjentów neurorozwojowych mutacje o mniejszej sile ekspresji leżą u podłoża ciężkich, opornych na leczenie dysfunkcji motoryki jelitowej, które drastycznie zaostrzają behawioralny fenotyp autystyczny (ASD) poprzez pętlę prozapalną.

---

---
### RIMS1

* **Główny symbol genu:** RIMS1
* **Pełna nazwa biochemiczna:** Białko regulujące uwalnianie neurotransmiterów 1 (ang. *Regulating synaptic membrane exocytosis 1*)
* **Nazwy potoczne i medialne:** Gen precyzji czasowej synaps, gen białka RIM1
* **Główny rsID / Wariant:** rs11064560 / Rzadkie mutacje punktowe *de novo*
* **Lokalizacja chromosomalna:** Chromosom 6 (6q12-q13)
* **Rola biologiczna genu/białka:** Koduje białko RIM1 – kluczowy komponent strefy aktywnej synapsy presynaptycznej. RIM1 fizycznie koordynuje dokowanie, priming ("uzbrajanie") oraz fuzję pęcherzyków synaptycznych z błoną komórkową w bezpośredniej odpowiedzi na napływ jonów wapnia przez kanały napięciowe. Decyduje o sile, tempie i precyzji sygnału nerwowego.

* **Mój genotyp (WGS):**
  `rs11064560` — **TT** (ensembl+bam, high) — marker_report



**G/G (Typ referencyjny)**
Perfekcyjna synchronizacja neurotransmisji bez opóźnień i szumów tła. Optymalna plastyczność synaptyczna krótko- i długoterminowa, wysoka sprawność przetwarzania bodźców sensorycznych.

**G/T (Wariant heterozygoticzny)**
Subtelne, bezobjawowe różnice w kinetyce wyrzutu neurotransmiterów, w pełni kompensowane homeostatycznie.

**T/T (Homozygota alleliczna T)**
Typowy profil populacyjny tag-SNP rs11064560. Nie oznacza mutacji LoF ani klinicznego ryzyka schizofrenii — rzadkie LoF wymagają sekwencjonowania.

---

---
### SHISA9

* **Główny symbol genu:** SHISA9
* **Pełna nazwa biochemiczna:** Białko Shisa homolog 9 (ang. *Shisa family member 9*)
* **Nazwy potoczne i medialne:** Gen białka CKAMP44, regulator odczulania receptorów AMPA
* **Główny rsID:** rs117039014
* **Lokalizacja chromosomalna:** Chromosom 16 (16q22.1)
* **Rola biologiczna genu/białka:** Koduje CKAMP44 – integralną podjednostkę pomocniczą postsynaptycznych receptorów glutaminianergicznych AMPA. CKAMP44 precyzyjnie kontroluje kinetykę desensytyzacji (odczulenia) oraz deaktywacji receptorów AMPA, bezpośrednio modyfikując siłę, amplitudę i czas trwania pobudzającego prądu synaptycznego.

* **Mój genotyp (WGS):**
  - `rs117039014` — NOT_IN_DBSNP
  - `rs7192086` — **AA** (ensembl+bam, high) — ncbi; depth=55,avg=55.0; gt=1/1; GWAS lead SHISA9






**Typ Dziki (Zbalansowana kinetyka AMPA)**
Optymalny czas trwania impulsu pobudzającego. Sprawny proces desensytyzacji chroni synapsę przed przestymulowaniem, przy jednoczesnym zachowaniu wysokiej wierności i ostrości przekazu sygnału.

**Warianty Polimorficzne (Średnia aktywność)**
Standardowe populacyjne parametry kinetyki synaptycznej i przewodnictwa glutaminianergicznego.

**Homozygota alleliczna A/A (tag-SNP GWAS)**
Profil populacyjny rs7192086 (A/A). W badaniach GWAS skorelowany ze statystyczną podatnością na deficyty uwagi i schizofrenię — nie oznacza zaburzeń klinicznych ani LoF w SHISA9.

---

---
### THSD7A

* **Główny symbol genu:** THSD7A
* **Pełna nazwa biochemiczna:** Domena trombospondyny typu 1 zawierająca białko 7A (ang. *Thrombospondin type 1 domain-containing protein 7A*)
* **Nazwy potoczne i medialne:** Gen adhezji filopodiów synaptycznych
* **Główny rsID / Wariant:** Warianty introniczne eQTL lub mutacje de novo
* **Lokalizacja chromosomalna:** Chromosom 7 (7p21.1)
* **Rola biologiczna genu/białka:** THSD7A to wielko-cząsteczkowe białko błonowe zaangażowane w adhezję komórkową, naprowadzanie aksonów i synaptogenezę. Jest krytyczne dla fizycznego stabilizowania i "kotwiczenia" nowo powstałych połączeń synaptycznych w fazie intensywnej przebudowy kory mózgowej.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Typ Dziki (Ekspresja optymalna)**
Prawidłowa migracja neuronów do odpowiednich warstw kory. Wysoka integralność strukturalna i stabilność połączeń synaptycznych, prawidłowe formowanie sieci neuronowych.

**Heterozygota / Polimorfizmy eQTL**
Subtelne, lokalne wahania w gęstości synaptycznej. Mogą manifestować się drobnymi różnicami w tempie przetwarzania bodźców sensorycznych bez ewidentnych deficytów klinicznych.

**Wariant Patogenny / Silne obniżenie ekspresji**
Utrata integralności mechanicznej i funkcjonalnej synaps. Osłabienie adhezji komórkowej w mózgu prowadzi do przedwczesnej eliminacji połączeń, manifestując się trudnościami poznawczymi, dysfunkcjami sensorycznymi i silną korelacją z ASD.

---

---
### TIE1

* **Główny symbol genu:** TIE1
* **Pełna nazwa biochemiczna:** Receptorowa kinaza tyrozynowa śródbłonka 1 (ang. *Tyrosine kinase with immunoglobulin like and EGF like domains 1*)
* **Nazwy potoczne i medialne:** Strażnik stabilności naczyń mózgowych i bariery krew-mózg
* **Główny rsID:** rs2230026
* **Lokalizacja chromosomalna:** Chromosom 1 (1p34.2)
* **Rola biologiczna genu/białka:** Koduje receptor śródbłonkowy zaangażowany w angiogenezę i remodeling naczyń krwionośnych. Współpracuje z układem angiopoetyn, kontrolując fizyczną szczelność, stabilność strukturalną oraz odporność mechaniczną naczyń włosowatych tworzących barierę krew-mózg (BBB).

* **Mój genotyp (WGS):**
  - `rs2230026` — **AA** (ensembl+bam, high) — marker_report
  - `rs2797285` — **GG** (ensembl, high) — ensembl; depth=39,avg=39.0; gt=1/1



**Typ Dziki (Maksymalna szczelność naczyń)**
Prawidłowo uformowane, stabilne mikrokrążenie mózgowe. Wysoka ochrona przed przeciekiem cytokin obwodowych do miąższu mózgu.

**Warianty Populacyjne (Umiarkowane)**
Standardowa, wydolna homeostaza naczyniowa kompensowana przez inne czynniki angiogenne.

**Wariant Ryzyka / Obniżona Ekspresja TIE1**
Lokalna niestabilność mikronaczyń i skłonność do mikroskopijnych rozszczelnień BBB pod wpływem nadciśnienia lub infekcji. Umożliwia ułatwioną penetrację cząsteczek zapalnych do OUN, napędzając neuroinflammation i zaostrzając objawy neuropsychiatryczne (np. w ChAD i ASD).

---

---

## CZĘŚĆ X. Czynniki transkrypcyjne i cytoarchitektura kory
### GSK3B

* **Główny symbol genu:** GSK3B
* **Pełna nazwa biochemiczna:** Syntaza kinazy glikogenu 3 beta (ang. *Glycogen synthase kinase 3 beta*)
* **Nazwy potoczne i medialne:** Gen przełącznika apoptozy i plastyczności, główny punkt uchwytu litu
* **Główny rsID:** rs3025366 / rs334558
* **Lokalizacja chromosomalna:** Chromosom 3 (3q13.33)
* **Rola biologiczna genu/białka:** Kinaza serynowo-treoninowa zaangażowana w kaskady sygnałowe szlaku Wnt/beta-kateniny oraz PI3K/Akt. Reguluje polaryzację neuronów, stabilność mikrotubul (fosforyluje białko tau) oraz przeżywalność komórek. Nadaktywność GSK3B promuje apoptozę i niszczenie synaps; lit hamuje jej działanie.

* **Mój genotyp (WGS):**
  `rs3025366` — **CC** (ensembl+bam, high) — marker_report


* **Genotyp Niskiej Aktywności (Neuroprotekcyjny):** Stabilnie niska podstawowa aktywność kinazy. Promuje przeżywalność neuronów, chroni przed patologiczną hiperfosforylacją białka tau i zapewnia wysoką rezyliencję na stres psychofizyczny.
* **Genotyp Umiarkowany (Typ dziki):** Zrównoważony metabolizm komórkowy i plastyczność synaptyczna w normie.
* **Genotyp Wysokiej Aktywności (Konstrukcja ryzyka afektywnego):** Chroniczna nadaktywność enzymu. Przyspiesza degradację neuroplastyczną, destabilizuje nastrój i indukuje podatność na ciężkie, nawracające epizody depresyjne oraz afektywne dwubiegunowe. Silny marker zapotrzebowania na terapię jonami litu.

---
### PDE4B

* **Główny symbol genu:** PDE4B
* **Pełna nazwa biochemiczna:** Fosfodiesteraza 4B specyficzna dla cAMP (ang. *Phosphodiesterase 4B*)
* **Nazwy potoczne i medialne:** Gen pamięci długotrwałej, integrator białka DISC1
* **Główny rsID:** rs7412571
* **Lokalizacja chromosomalna:** Chromosom 1 (1q31.2)
* **Rola biologiczna genu/białka:** Hydrolizuje cykliczny AMP (cAMP) – kluczowy wtórny przekaźnik informacji w neuronie. PDE4B tworzy kompleks strukturalny z białkiem DISC1 (Disrupted in Schizophrenia 1). Kontroluje siłę sygnału niezbędnego do aktywacji czynnika CREB, co decyduje o konsolidacji pamięci i plastyczności synaptycznej.

* **Mój genotyp (WGS):**
  `rs7412571` — **TT** (ensembl+bam, high) — ensembl; depth=43,avg=43.0; gt=0/0





* **Genotyp Niskiej Aktywności (Wariant prokognitywny):** Wolniejsze rozkładanie cAMP. Wyższe stężenia drugiego przekaźnika stymulują plastyczność synaptyczną, podnosząc sprawność uczenia się, pamięci długotrwałej i elastyczności poznawczej.
* **Genotyp Umiarkowany (Typ dziki):** Prawidłowa homeostaza nukleotydów cyklicznych w neuronach.
* **Genotyp Wysokiej Aktywności (Wariant ryzyka / Przyspieszona degradacja):** Błyskawiczne niszczenie cAMP. Gwałtowne wygaszanie kaskady CREB uniemożliwia stabilną przebudowę synaps pod wpływem nauki. Blokuje plastyczność kory przedczołowej i hipokampu; potężny marker ryzyka deficytów poznawczych, schizofrenii oraz anhedonii depresyjnej.

---

## CZĘŚĆ XI. Modyfikacje RNA, translacja i epigenetyka enzymatyczna
### LSM6

* **Główny symbol genu:** LSM6
* **Pełna nazwa biochemiczna:** Homolog LSM6 (ang. *LSM6 homolog, U6 small nuclear RNA associated*)
* **Nazwy potoczne i medialne:** Gen obróbki i degradacji mRNA
* **Główny rsID:** rs1051566
* **Lokalizacja chromosomalna:** Chromosom 4 (4q13.3)
* **Rola biologiczna genu/białka:** Koduje składnik pierścienia Lsm (Like-Sm), który wiąże małe jądrowe RNA (snRNA) ułatwiając formowanie spliceosomu (prawidłowy splicing pre-mRNA). Uczestniczy także w procesie decappingu (usuwania czapeczki) i degradacji zużytych nici mRNA w cytoplazmie, kontrolując czas trwania sygnału transkrypcyjnego w synapsie.

* **Mój genotyp (WGS):**
  - `rs1051566` — **CC** (ensembl+bam, high) — marker_report
  - `rs13107325` — **CT** (ensembl, high) — ensembl; depth=53,avg=53.0; gt=0/1



**Typ Dziki (Prawidłowy splicing i klirens RNA)**
Precyzyjna eliminacja starych transkryptów i bezbłędny splicing genów neurorozwojowych. Wysoka sprawność przebudowy postsynaptycznej.

**Warianty Populacyjne (Umiarkowane)**
Standardowa, stabilna dynamika obróbki matrycowej RNA.

**Wariant Ryzyka (Zmieniona ekspresja LSM6)**
Subtelne opóźnienia w degradacji zużytych nici mRNA w kolcach dendrytycznych, wywołujące lokalny chaos translacyjny (szum synaptyczny). Koreluje ze zwiększoną podatnością na zmęczenie kognitywne i cechy neurorozwojowe.

---

---
### NSUN2

* **Główny symbol genu:** NSUN2
* **Pełna nazwa biochemiczna:** Metylotransferaza RNA 2 (ang. *NOP2/Sun RNA methyltransferase 2*)
* **Nazwy potoczne i medialne:** Epigenetyczny kontroler stabilności tRNA, gen zespołu Dubowitza
* **Główny rsID / Wariant:** Warianty patogenne utraty funkcji (LoF) / rs11553556
* **Lokalizacja chromosomalna:** Chromosom 5 (5p15.31)
* **Rola biologiczna genu/białka:** NSUN2 odpowiada za metylację cytozyny do 5-metylocytozyny (m5C) w cząsteczkach tRNA, mRNA i niekodujących RNA. Metylacja ta chroni tRNA przed pękaniem i degradacją pod wpływem stresu komórkowego. Jest to absolutnie krytyczne dla precyzji i tempa translacji białek strukturalnych rozwijającego się mózgu.

* **Mój genotyp (WGS):**
  `rs11553556` — **CC** (ensembl+bam, high) — ensembl; depth=51,avg=51.0; gt=0/0






**Typ Dziki (Pełna ochrona i stabilność RNA)**
Prawidłowa, optymalna synteza białek w komórkach progenitorowych mózgu. Prawidłowy wzrost somatyczny i neurobiologiczny.

**Warianty Polimorficzne (Łagodne modyfikacje)**
Subtelne fluktuacje w tempie syntezy białek synaptycznych pod wpływem skrajnego zmęczenia lub stresu oksydacyjnego.

**Mutacja Patogenna (Homozygota LoF / Zespół Dubowitza-like)**
Brak metylacji m5C skutkuje masowym cięciem tRNA przez endonukleazy. Blokada translacji wstrzymuje neurogenezę i podziały komórkowe. Objawia się ciężką niepełnosprawnością intelektualną, mikrocefalią, dysmorfią twarzy, brakiem mowy oraz głębokim spektrum autyzmu.

---

---
### NSUN6

* **Główny symbol genu:** NSUN6
* **Pełna nazwa biochemiczna:** Metylotransferaza RNA 6 (ang. *NOP2/Sun RNA methyltransferase 6*)
* **Nazwy potoczne i medialne:** Precyzyjny kalibrator translacji synaptycznej
* **Główny rsID:** brak tag-SNP (region bez wspólnych SNV w WGS)
* **Lokalizacja chromosomalna:** Chromosom 10 (10q21.1, ~18.5 Mb hg38)
* **Rola biologiczna genu/białka:** Podobnie jak NSUN2, wprowadza modyfikacje m5C, lecz wykazuje wysoce specyficzne powinowactwo do konkretnych pozycji (C72 i C73) w pętlach akceptorowych tRNA dla cysteiny i treoniny. Decyduje o ostatecznym dopasowaniu przestrzennym tRNA i jego wydajności w rybosomach podczas budowania specyficznych białek receptorowych.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP w ciele genu — ocena wyłącznie sekwencyjna (rs2241604 w panelu mapuje błędnie na chr5).



**Typ Dziki (Precyzyjna translacja)**
Stabilne tempo wbudowywania cysteiny i treoniny do łańcuchów białkowych w neuronach. Prawidłowe formowanie receptorów postsynaptycznych.

**Warianty Populacyjne (Umiarkowane)**
Standardowa, bezobjawowa zmienność w dynamice epigenetycznej obróbki RNA.

**Wariant Ryzyka (Obniżona ekspresja NSUN6)**
Zwolnienie kinetyki translacji specyficznych izoform białek synaptycznych, wywołujące mikro-zaburzenia plastyczności krótko- i długoterminowej. Koreluje ze słabszą elastycznością poznawczą w badaniach neuropsychiatrycznych.

---

---
### QTRT1

* **Główny symbol genu:** QTRT1
* **Pełna nazwa biochemiczna:** Podjednostka katalityczna transferazy queuine-tRNA 1 (ang. *Queuine tRNA-ribosyltransferase catalytic subunit 1*)
* **Nazwy potoczne i medialne:** Gen wbudowywania mikroskładnika queuine, modulator plastyczności synaptycznej
* **Główny rsID:** rs2290432
* **Lokalizacja chromosomalna:** Chromosom 19 (19p13.2)
* **Rola biologiczna genu/białka:** Enzym ten podmienia guaninę w pozycji 34 (antykodon) wybranych tRNA na unikalną zasadę azotową – queuine (pochodzącą wyłącznie z diety i mikrobiomu jelitowego). Modyfikacja ta optymalizuje prędkość translacji oraz precyzyjnie reguluje ekspresję genów odpowiedzialnych za homeostazę neurotransmiterów monoaminowych (dopamina, serotonina).

* **Mój genotyp (WGS):**
  `rs2290432` — **GG** (ensembl+bam, high) — marker_report



**Typ Dziki (Maksymalna integracja mikroskładnika)**
Wydajne wykorzystanie queuine dostarczanej przez mikrobiotę. Optymalna dynamika translacji i stabilna synteza transporterów monoamin.

**Warianty Populacyjne (Średnia wydajność)**
Standardowa zależność sprawności translacyjnej od stanu eubiozy i podaży queuine w diecie.

**Wariant Ryzyka (Niska ekspresja QTRT1)**
Upośledzenie modyfikacji tRNA przy promowaniu niestabilności rybosomalnej. Skorelowany z wyższą podatnością na zaburzenia nastroju (depresja) oraz deficyty uwagi w niesprzyjających warunkach jelitowych (dysbioza).

---

---
### RPS26

* **Główny symbol genu:** RPS26
* **Pełna nazwa biochemiczna:** Białko rybosomalne S26 (ang. *Ribosomal protein S26*)
* **Nazwy potoczne i medialne:** Selektor translacji, top-hit GWAS autoimmunizacji i ChAD
* **Główny rsID:** rs11171739
* **Lokalizacja chromosomalna:** Chromosom 12 (12q13.2)
* **Rola biologiczna genu/białka:** Składnik małej podjednostki 40S rybosomu. RPS26 pełni kluczową funkcję regulacyjną – decyduje o tzw. "preferencjach translacyjnych" rybosomu. Pod wpływem stresu komórkowego rybosomy zawierające RPS26 selektywnie promują translację mRNA kodujących białka punktów kontrolnych i odpowiedzi zapalnej, blokując standardową syntezę białek housekeepingowych.

* **Mój genotyp (WGS):**
  - `rs11171739` — **CT** (ensembl+bam, high) — marker_report
  - `rs3760707` — **TT** (ensembl, high) — ensembl; depth=45,avg=45.0; gt=0/0



**Genotyp Referencyjny (Prawidłowa selekcja stresowa)**
Precyzyjna kontrola nad przełączaniem rybosomów w tryb obrony komórkowej. Wysoka ochrona neuronów przed apoptozą.

**Genotyp Pośredni (Heterozygota)**
Standardowa populacyjna odporność translacyjna na stres komórkowy.

**Genotyp Ryzyka (Niska ekspresja / Sygnał GWAS)**
Zaburzenie dystrybucji rybosomalnej i utrata kontroli nad translacją specyficznych białek odpornościowych. Wariant ten wykazuje potężną, powtarzalną asocjację w badaniach GWAS jednoczesnego ryzyka rozwoju **cukrzycy typu 1, chorób autoimmunologicznych oraz choroby afektywnej dwubiegunowej (ChAD)**.

---

## BLOK 20: Transportery, Enzymy Lizosomalne i pH

---
### TRMT61A

* **Główny symbol genu:** TRMT61A
* **Pełna nazwa biochemiczna:** Katalityczna podjednostka tRNA metylotransferazy 61A (ang. *tRNA methyltransferase 61 subunit A*)
* **Nazwy potoczne i medialne:** Gen modyfikacji m1A RNA, strażnik przeżywalności progenitorów neuronalnych
* **Główny rsID:** rs2275608
* **Lokalizacja chromosomalna:** Chromosom 14 (14q32.31)
* **Rola biologiczna genu/białka:** Wspólnie z TRMT6 tworzy kompleks odpowiedzialny za metylację adeniny do 1-metyloadeniny (m1A) w pozycji 58 cząsteczek tRNA. Modyfikacja m1A wprowadza silny ładunek dodatni, niezbędny do strukturalnej stabilizacji struktury trzeciorzędowej tRNA. Bez niej dochodzi do destabilizacji rybosomów i zahamowania syntezy kluczowych enzymów metabolicznych w mózgu.

* **Mój genotyp (WGS):**
  `rs2275608` — **TT** (ensembl+bam, high) — marker_report



**Typ Dziki (Maksymalna stabilność translacji)**
Wydajna praca aparatu translacyjnego, prawidłowy przebieg podziałów mitotycznych w neurogenezie.

**Warianty Populacyjne (Średnia aktywność)**
Standardowa populacyjna dynamika obróbki tRNA.

**Wariant Ryzyka (Niska ekspresja / Zaburzenie m1A)**
Wzrost niestabilności translacyjnej, prowadzący do aktywacji komórkowego szlaku odpowiedzi na niesfałdowane białka (UPR) i apoptozy neuronów w fazie rozwoju. Skorelowany w badaniach wielkoskalowych z podatnością na zaburzenia neurorozwojowe i deficyty intelektualne.

---

---
### TYW5

* **Główny symbol genu:** TYW5
* **Pełna nazwa biochemiczna:** Białko 5 syntetyzujące wybutozynę tRNA (ang. *tRNA wybutosine-synthesizing protein 5*)
* **Nazwy potoczne i medialne:** Hydroksylaza tRNA, gen kontroli wierności odczytu kodonów
* **Główny rsID / Wariant:** rzadkie mutacje punktowe missens / rs12140417
* **Lokalizacja chromosomalna:** Chromosom 2 (2q33.1)
* **Rola biologiczna genu/białka:** Koduje zależną od Fe²⁺ i 2-oksoglutaranu hydroksylazę, która katalizuje ostatni etap biosyntezy wybutozyny (yW) – unikalnej, wysoce zmodyfikowanej zasady obecnej wyłącznie w pozycji 37 tRNA dla fenyloalaniny. Modyfikacja ta fizycznie zapobiega przesunięciom ramki odczytu (frameshifting) w rybosomie, gwarantując bezbłędny odczyt kodu genetycznego podczas syntezy wielkich białek strukturalnych mózgu.

* **Mój genotyp (WGS):**
  `rs12140417` — **CC** (ensembl+bam, high) — marker_report



**Typ Dziki (Bezbłędny odczyt kodu)**
Maksymalna wierność translacji. Zero patologicznych przesunięć ramki odczytu, stabilna struktura syntezy białek osi synaptycznej.

**Warianty Populacyjne (Łagodne)**
Standardowa zmienność kinetyki hydroksylacji bez wpływu na ogólną wierność translacyjną.

**Mutacja Patogenna (Heterozygota/Homozygota LoF lub rzadki wariant missens)**
Utrata wybutozyny w tRNA-Phe powoduje masowe błędy i powstawanie uciętych, niefunkcjonalnych białek w neuronach. Manifestuje się klinicznie jako postępująca encefalopatia, mikrocefalia, głębokie opóźnienie psychoruchowe i cechy ze spektrum autyzmu.

---

---

## CZĘŚĆ XII. Foliany, metylacja i szlaki metaboliczne
### CBS

* **Główny symbol genu:** CBS
* **Pełna nazwa biochemiczna:** Beta-syntaza cystationiny (ang. *Cystathionine beta-synthase*)
* **Nazwy potoczne i medialne:** Gen szlaku transsulfuracji, gen homocystynurii
* **Główny rsID / Polimorfizm:** CBS C699T (rs234706), CBS T833C lub rzadkie mutacje patogenne
* **Lokalizacja chromosomalna:** Chromosom 21 (21q22.3)
* **Rola biologiczna genu/białka:** Przekształca homocysteinę w cystationinę przy użyciu witaminy B6 (PLP) jako kofaktora. Przekierowuje metabolizm z cyklu metylacji do szlaku transsulfuracji, co prowadzi do produkcji najsilniejszego endogennego antyoksydantu – glutationu – oraz tauryny.

* **Mój genotyp (WGS):**
  `rs234706` — **GG** (ensembl+bam, high) — marker_report



**Wariant C/C (Aktywność zrównoważona / Typ dziki)**
Optymalne poziomy homocysteiny i metioniny. Stabilny przebieg procesów metylacji przy jednoczesnej wydajnej produkcji glutationu na potrzeby ochrony antyoksydacyjnej neuronów.

**Wariant C699T - Genotyp T/T (Gain-of-Function / Nadreaktywność)**
"Ucieczka" grup metylowych ze szlaku remetylacji do transsulfuracji. Powoduje szybkie zużywanie kofaktora B6, chronicznie niskie poziomy homocysteiny, ale też potencjalny deficyt SAMe i obciążenie układu nerwowego nadmiarem produktów ubocznych (siarczyny, amoniak).

**Mutacje Utraty Funkcji (LoF / Klasyczna Homocystynuria)**
Całkowity blok enzymatyczny. Ekstremalna akumulacja homocysteiny i metioniny. Objawia się uszkodzeniami tkanki łącznej, incydentami zakrzepowymi, głęboką niepełnosprawnością intelektualną oraz fenotypem autystycznym z ekstremalną nadwrażliwością sensoryczną.

---

---
### DHFR

* **Główny symbol genu:** DHFR
* **Pełna nazwa biochemiczna:** Reduktaza dihydrofolianu (ang. *Dihydrofolate reductase*)
* **Nazwy potoczne i medialne:** Główny enzym szlaku foliowego, gen remetylacji
* **Główny rsID / Polimorfizm:** Polimorfizm delecyjno-insercyjny 19-bp w intronicie 1 (wariant Del/Ins)
* **Lokalizacja chromosomalna:** Chromosom 5 (5q14.1)
* **Rola biologiczna genu/białka:** DHFR redukuje dihydrofolian do tetrahydrofolianu (THF). Jest to absolutnie kluczowy etap cyklu folianowego, niezbędny do syntezy puryn, tymidylanów oraz remetylacji homocysteiny do metioniny (która generuje SAMe – główny kofaktor dla COMT). Wpływa bezpośrednio na syntezę neurotransmiterów oraz kofaktora BH4.

* **Mój genotyp (WGS):**
  `rs1650699` — **TT** (ensembl+bam, high) — ncbi; depth=49,avg=49.0; gt=0/0






**Genotyp Ins/Ins (Wysoka wydajność enzymatyczna)**
Maksymalna sprawność cyklu folianowego. Optymalna pula THF, stabilna produkcja neurotransmiterów (dopamina, serotonina) i wysoki, sprawny potencjał metylacyjny organizmu.

**Genotyp Ins/Del (Wydajność umiarkowana)**
Pośrednia aktywność enzymu. W warunkach zbalansowanej diety i braku silnych stresorów metabolicznych w pełni kompensuje zapotrzebowanie organizmu na aktywne foliany.

**Genotyp Del/Del (Niska wydajność enzymatyczna)**
Wyraźny spadek sprawności szlaku folianowego. Skutkuje zaburzeniami remetylacji, niedoborem SAMe i wtórnym blokowaniem procesów metylacyjnych w OUN. Prowadzi do akumulacji nieuzdatnionego kwasu foliowego (UMFA), zaburzeń syntezy neuroprzekaźników i jest silnie powiązany z objawami regresu neurorozwojowego w spektrum autyzmu.

---

---
### MC4R

* **Główny symbol genu:** MC4R
* **Pełna nazwa biochemiczna:** Receptor melanokortyny 4 (ang. *Melanocortin 4 receptor*)
* **Nazwy potoczne i medialne:** Gen podwzgórzowej kontroli sytości, gen otyłości monogenowej
* **Główny rsID / Wariant:** rs17782344 / Rzadkie mutacje typu utraty funkcji (LoF)
* **Lokalizacja chromosomalna:** Chromosom 18 (18q21.32)
* **Rola biologiczna genu/białka:** Receptor sprzężony z białkiem Gs, zlokalizowany w jądrze przykomorowym podwzgórza. Jest kluczowym elementem osi leptynowo-melanokortynowej. Aktywacja MC4R przez alfa-MSH wysyła do kory mózgowej potężny, nadrzędny sygnał sytości oraz stymuluje obwodowy wydatek energetyczny.

* **Mój genotyp (WGS):**
  `rs17782344` — **TT** (ensembl+bam, high) — marker_report
* **Dopasowany profil:** Genotyp T/T (Typ dziki / Prawidłowa kontrola łaknienia)



**Genotyp T/T (Typ dziki / Prawidłowa kontrola łaknienia)**
Precyzyjna podwzgórzowa sygnalizacja sytości. Prawidłowy bilans energetyczny organizmu, niska skłonność do podjadania emocjonalnego, nawykowego lub kompulsywnego.

**Genotyp C/T (Umiarkowane osłabienie sygnału)**
Zwiększona tendencja do preferowania pokarmów wysokokalorycznych i bogatych w węglowodany; nieznacznie podwyższone ryzyko nadwagi w niesprzyjającym środowisku dietetycznym.

**Genotyp C/C lub Mutacja Patogenna (LoF)**
Drastyczne upośledzenie sygnału sytości w podwzgórzu (mózg stale interpretuje stan organizmu jako głód/głodzenie – hiperfagia). Najważniejsza genetyczna przyczyna otyłości monogenowej o wczesnym początku; silnie skorelowany z kompulsywnym objadaniem się (binge eating) oraz zaburzeniami metabolicznymi w fenotypach neurorozwojowych (ASD/ADHD), gdzie samokontrola behawioralna jest dodatkowo osłabiona.

---

## CZĘŚĆ XIII. Farmakogenetyka, detoksykacja i bariera krew–mózg
### ABCB1

* **Główny symbol genu:** ABCB1
* **Pełna nazwa biochemiczna:** Glikoproteina P (ang. *ATP-binding cassette sub-family B member 1*)
* **Nazwy potoczne i medialne:** Gen szczelności bariery krew-mózg, gen oporności lekowej
* **Główny rsID:** rs1045642 (słynny polimorfizm synonimiczny C3435T)
* **Lokalizacja chromosomalna:** Chromosom 7 (7q21.12)
* **Rola biologiczna genu/białka:** Koduje glikoproteinę P (P-gp) – energochłonną pompę effluxową osadzoną w komórkach śródbłonka naczyń włosowatych mózgu. Odpowiada za aktywne "wyrzucanie" leków (w tym neuroleptyków, antydepresantów, stymulantów) oraz ksenobiotyków i toksyn z tkanki mózgowej z powrotem do krwiobiegu.

* **Mój genotyp (WGS):**
  `rs1045642` — **AA** (ensembl+bam, high) — marker_report


* **Genotyp C/C (Wysoka ekspresja P-gp):** "Bariera pancerna". Wyjątkowo wydajne oczyszczanie mózgu z toksyn i substancji obcych. Skutek kliniczny: wysoka oporność na farmakoterapię psychiatryczną (leki nie mogą osiągnąć stężenia terapeutycznego w OUN, wymagane wyższe dawki).
* **Genotyp C/T (Ekspresja umiarkowana):** Standardowa, prawidłowa przepuszczalność bariery krew-mózg dla większości cząsteczek leczniczych.
* **Genotyp T/T (Niska ekspresja P-gp):** "Bariera nieszczelna". Pompa działa słabo, pozwalając lekom i toksynom na swobodną penetrację i akumulację w tkance mózgowej. Skutek kliniczny: **ekstremalna podatność na toksyczność i skutki uboczne leków** nawet przy minimalnych dawkach (np. natychmiastowe objawy pozapiramidowe po neuroleptykach lub silna sedacja).

---
### ABCB6

* **Główny symbol genu:** ABCB6
* **Pełna nazwa biochemiczna:** Transporter kasetowy wiążący ATP, podrodzina B, składowa 6 (ang. *ATP-binding cassette sub-family B member 6*)
* **Nazwy potoczne i medialne:** Mitochondrialny gen neuroprotekcji przed metalami ciężkimi
* **Główny rsID:** rs1482110 / rs2240176
* **Lokalizacja chromosomalna:** Chromosom 2 (2q35)
* **Rola biologiczna genu/białka:** Koduje transporter ATP-zależny zlokalizowany głównie w zewnętrznej błonie mitochondrialnej oraz w endosomach. Odpowiada za transport porfiryn niezbędnych do biosyntezy hemu oraz odgrywa fundamentalną rolę w ochronie neuronów i astrocytów przed stresem oksydacyjnym indukowanym przez akumulację metali ciężkich (np. arsen, ołów, kadm).

* **Mój genotyp (WGS):**
  `rs1482110` — **TT** (ensembl+bam, high) — marker_report



**Genotyp Referencyjny (Wysoka neuroprotekcja)**
Sprawny transport mitochondrialny, prawidłowa synteza hemu i wysoki potencjał detoksykacyjny OUN wobec ksenobiotyków i metali ciężkich.

**Genotyp Pośredni (Wariant populacyjny)**
Standardowa, wydolna w warunkach fizjologicznych komórkowa odporność na toksyny środowiskowe.

**Genotyp Ryzyka (Niska ekspresja / Obniżona aktywność)**
Zwiększona wrażliwość mitochondriów na stres oksydacyjny i uszkodzenia wywołane metalami. Może nasilać podatność na neurotoksyczność w warunkach niekorzystnej ekspozycji środowiskowej, wykazując korelacje z przyspieszonym starzeniem poznawczym oraz podatnością na zaburzenia spektrum autyzmu.

---

---
### CYP2B6

* **Główny symbol genu:** CYP2B6
* **Pełna nazwa biochemiczna:** Cytochrom P450, podrodzina 2, podgrupa B, polipeptyd 6 (ang. *Cytochrome P450 family 2 subfamily B member 6*)
* **Nazwy potoczne i medialne:** Gen metabolizmu bupropionu i ketaminy
* **Główny rsID / Allel:** rs3745274 / Allel CYP2B6*6 (zmiana Q172H i K262R)
* **Lokalizacja chromosomalna:** Chromosom 19 (19q13.2)
* **Rola biologiczna genu/białka:** Enzym mikrosomalny wątroby (ale wykazujący też ekspresję lokalną w mózgu). Odpowiada za główny szlak metaboliczny (hydroksylację) bupropionu (stosowanego w ADHD i rzucaniu palenia), efawirenzu oraz anestetyków/dysocjantów (ketamina, propofol).

* **Mój genotyp (WGS):**
  `rs3745274` — **GG** (ensembl+bam, high) — ensembl; depth=51,avg=51.0; gt=0/0





* Allel *1/*1 (Szybki metabolizator - EM):** Prawidłowa prędkość rozkładu leków. Stabilne, przewidywalne okno terapeutyczne dla bupropionu i ketaminy.
* Allel *1/*6 (Pośredni metabolizator - IM):** Umiarkowane zwolnienie klirensu lekowego; standardowe dawkowanie jest zazwyczaj dobrze tolerowane.
* Allel *6/*6 (Wolny metabolizator - PM):** Katastrofalny spadek aktywności enzymatycznej. Bupropion nie jest przekształcany do aktywnego metabolitu (hydroksybupropionu), co znosi jego skuteczność w ADHD, wywołując jednocześnie gigantyczną akumulację formy macierzystej leku. Skutkuje to drastycznym obniżeniem progu drgawkowego, ciężkimi stanami lękowymi, bezsennością i objawami neurotoksycznymi przy standardowych dawkach.

---
### CYP3A4

* **Główny symbol genu:** CYP3A4
* **Pełna nazwa biochemiczna:** Cytochrom P450, podrodzina 3, podgrupa A, polipeptyd 4 (ang. *Cytochrome P450 family 3 subfamily A member 4*)
* **Nazwy potoczne i medialne:** Główny enzym metabolizmu leków psychiatrycznych i kortyzolu
* **Główny rsID / Allel:** CYP3A4*22 (rs11568824 - niska aktywność) lub CYP3A4*1B (rs2740574 - wysoka aktywność)
* **Lokalizacja chromosomalna:** Chromosom 7 (7q22.1)
* **Rola biologiczna genu/białka:** Najważniejszy enzym metabolizmu ksenobiotyków w organizmie ludzkim (wątroba, jelita). Odpowiada za biotransformację ponad 50% leków dostępnych na rynku, w tym neuroleptyków (kwetiapina, arypiprazol), antydepresantów (sertralina, citalopram), benzodiazepin oraz endogennych hormonów sterydowych (kortyzol, testosteron).

* **Mój genotyp (WGS):**
  `rs11568824` — **CC** (ensembl+bam, high) — marker_report



Allel *1/*1 (Szybki metabolizator - EM)**
Standardowa, wysoka aktywność enzymatyczna. Leki oraz kortyzol są metabolizowane w optymalnym tempie; stabilne i przewidywalne okno terapeutyczne farmakoterapii.

**Allel *1B (Ultraszybki metabolizator - UM)**
Ekstremalnie przyspieszony rozkład substancji. Standardowe dawki neuroleptyków lub antydepresantów mogą być całkowicie nieskuteczne z powodu zbyt szybkiego usuwania leku z krwioobiegu (wymaga wyższych dawek).

**Allel *22 (Wolny / Pośredni metabolizator - PM/IM)**
Wyraźny spadek aktywności enzymatycznej o 40-50%. Drastyczne spowolnienie eliminacji leków i kortyzolu. Skutkuje to gwałtownym wzrostem stężenia leków psychotropowych we krwi, wywołując ciężkie objawy toksyczności i uderzające skutki uboczne (np. silną sedację) przy standardowym dawkowaniu.

---

---

## CZĘŚĆ XIV. Transportery, lizosomy, naprawa DNA i markery strukturalne
### ALMS1

* **Główny symbol genu:** ALMS1
* **Pełna nazwa biochemiczna:** Białko zespołu Alströma 1 (ang. *Alström syndrome protein 1*)
* **Nazwy potoczne i medialne:** Gen rzęsek komórkowych, integrator osi łaknienia i neurorozwoju
* **Główny rsID / Wariant:** Rzadkie mutacje nonsensowne / przesunięcia ramki odczytu (LoF)
* **Lokalizacja chromosomalna:** Chromosom 2 (2p13.1)
* **Rola biologiczna genu/białka:** Białko zlokalizowane w centrosomach i ciałkach podstawowych rzęsek komórkowych (cilia). Rzęski pierwotne na neuronach działają jak "antenki" odbierające sygnały neurotroficzne oraz metaboliczne (np. sygnał leptyny w podwzgórzu). ALMS1 kontroluje transport wewnątrzrzęskowy (IFT) oraz endocytozę receptorów.

* **Mój genotyp (WGS):**
  Brak populacyjnego tag-SNP — ocena wyłącznie sekwencyjna/CNV (mutacje de novo / LoF nie wykrywalne w panelu SNP).



**Typ Dziki (Prawidłowa funkcja rzęsek)**
Wydajna, precyzyjna detekcja sygnałów neurotroficznych (BDNF) i metabolicznych (leptyna) przez neurony. Prawidłowa masa ciała, ostrość zmysłów i rozwój intelektualny.

**Warianty Populacyjne (Łagodne modyfikacje)**
Drobne wahania w tempie metabolizmu obwodowego i zmienna wrażliwość na sygnały sytości.

**Mutacja Patogenna (Homozygota/Heterozygota złożona LoF)**
**Zespół Alströma**. Całkowita dysfunkcja rzęsek (ciliopatia). Skutkuje ślepotą (dystrofia siatkówki), głuchotą czuciową, ekstremalną otyłością z hiperfagią, insulinoopornością oraz niepełnosprawnością intelektualną z nasilonymi cechami spektrum autyzmu.

---

## BLOK 21: Sygnały GWAS, Naprawa DNA i Markery Strukturalne

---
### CYREN

* **Główny symbol genu:** CYREN (stara nazwa: *MRI / MRI1*)
* **Pełna nazwa biochemiczna:** Regulator cyklu komórkowego naprawy NHEJ (ang. *Cell cycle regulator of NHEJ*)
* **Nazwy potoczne i medialne:** Gen obrony genomu przed pękaniem nici DNA
* **Główny rsID:** rs2240901
* **Lokalizacja chromosomalna:** Chromosom 7 (7p22.1)
* **Rola biologiczna genu/białka:** CYREN działa jako nadrzędny przełącznik w mechanizmach naprawy pęknięć obydwu nici DNA (DSB). W fazie G1 cyklu komórkowego promuje szybką naprawę metodą niehomologicznego łączenia końców (NHEJ), chroniąc komórki progenitorowe mózgu przed niestabilnością genomową podczas intensywnych podziałów.

* **Mój genotyp (WGS):**
  `rs2240901` — **AA** (ensembl+bam, high) — ensembl; depth=26,avg=26.0; gt=0/0






**Typ Dziki (Wysoka wierność naprawy DNA)**
Błyskawiczne i bezbłędne scalanie pęknięć DNA w neuronach. Maksymalna ochrona przed mutacjami somatycznymi, prawidłowe tempo neurogenezy.

**Warianty Populacyjne (Średnia wydajność)**
Standardowa, wydolna w warunkach fizjologicznych homeostaza genomowa.

**Wariant Ryzyka (Niska ekspresja CYREN)**
Opóźnienie w naprawie uszkodzeń DNA, prowadzące do akumulacji mikromutacji w komórkach macierzystych mózgu lub przedwczesnej apoptozy neuronów. Koreluje w sygnałach GWAS z podwyższonym ryzykiem globalnych opóźnień rozwojowych i neuroatypowości.

---

---
### GOLPH3L

* **Główny symbol genu:** GOLPH3L
* **Pełna nazwa biochemiczna:** Białko fosfoproteinowe aparatu Golgiego 3-podobne (ang. *Golgi phosphoprotein 3 like*)
* **Nazwy potoczne i medialne:** Gen sortowania makrocząsteczek, regulator sekrecji neurotrofin
* **Główny rsID:** rs11660233
* **Lokalizacja chromosomalna:** Chromosom 1 (1q21.3)
* **Rola biologiczna genu/białka:** Rezyduje w trans-sieci aparatu Golgiego (TGN). Odpowiada za fizyczne formowanie pęcherzyków transportowych i sortowanie glikoproteidów kierowanych na powierzchnię neuronu – w tym cząsteczek adhezyjnych oraz receptorów synaptycznych.

* **Mój genotyp (WGS):**
  `rs11660233` — **GG** (ensembl+bam, high) — marker_report



**Typ Dziki (Wydajny transport z aparatu Golgiego)**
Prawidłowa gęstość i bezbłędny eksport nowych receptorów na błonę postsynaptyczną. Wysoki potencjał neuroplastyczny.

**Warianty Populacyjne (Średnia aktywność)**
Standardowa dynamika sekrecji pęcherzykowej.

**Wariant Ryzyka (Niska ekspresja / Niestabilność GOLPH3L)**
Subtelne opóźnienia w dostarczaniu świeżo zsyntetyzowanych receptorów do synaps, co może skutkować osłabieniem procesów długofalowego wzmocnienia synaptycznego (LTP). Powiązany w GWAS z zaburzeniami funkcji wykonawczych i neuroatypowością.

---

---
### NAGA

* **Główny symbol genu:** NAGA
* **Pełna nazwa biochemiczna:** Alfa-N-acetylgalactosaminidaza (ang. *Alpha-N-acetylgalactosaminidase*)
* **Nazwy potoczne i medialne:** Gen choroby Schindlera, enzym lizosomalnego regresu mowy
* **Główny rsID / Wariant:** Mutacje utraty funkcji (LoF) / rs1051513
* **Lokalizacja chromosomalna:** Chromosom 22 (22q13.2)
* **Rola biologiczna genu/białka:** Koduje enzym lizosomalny, który rozszczepia reszty alfa-N-acetylogalaktozaminy z glikoprotein i glikolipidów. Jest niezbędny do stałego klirensu i recyklingu złożonych cząsteczek błonowych w neuronach.

* **Mój genotyp (WGS):**
  `rs1051513` — **CC** (ensembl+bam, high) — marker_report



**Typ Dziki (Wydajny klirens lizosomalny)**
Pełna czystość metaboliczna wnętrza neuronu. Brak spichrzania toksycznych produktów ubocznych, prawidłowa neuroprotekcja.

**Warianty Populacyjne o Obniżonej Aktywności (Łagodne)**
Subtelne spowolnienie recyklingu glikolipidów, mogące obniżać próg podatności na zmęczenie kognitywne w niesprzyjających warunkach metabolicznych.

**Mutacja Patogenna (Heterozygota/Homozygota LoF / Choroba Schindlera typ II)**
Toksyczna akumulacja glikokoniugatów w neuronach i aksonach. Skutkuje postępującą neurodegeneracją, drastycznym, nagłym regresem rozwoju mowy i zdolności poznawczych u dzieci, ataksją ruchową oraz ciężkim fenotypem autystycznym (ASD) o charakterze spichrzeniowym.

---

---
### PROZ

* **Główny symbol genu:** PROZ
* **Pełna nazwa biochemiczna:** Białko Z (ang. *Protein Z, vitamin K-dependent plasma glycoprotein*)
* **Nazwy potoczne i medialne:** Gen neuroprotekcji naczyniowej, inhibitor układu krzepnięcia
* **Główny rsID:** rs4253243
* **Lokalizacja chromosomalna:** Chromosom 13 (13q34)
* **Rola biologiczna genu/białka:** Białko Z działa jako kluczowy kofaktor dla inhibitora proteazy zależnego od białka Z (ZPI), wygaszając kaskadę krzepnięcia (hamuje czynnik Xa). W mózgu wykazuje bezpośrednie działanie neuroprotekcyjne: chroni śródbłonek BBB oraz neurony przed apoptozą wywołaną niedotlenieniem lub lokalnym odczynem zakrzepowo-zapalnym.

* **Mój genotyp (WGS):**
  `rs4253243` — **TT** (ensembl+bam, high) — ensembl; depth=55,avg=55.0; gt=0/0






**Genotyp Referencyjny (Wysoki poziom Białka Z / Neuroprotekcja)**
Doskonała kontrola nad mikrokrążeniem mózgowym. Wysoka odporność naczyń na stany zapalne, optymalne dotlenienie tkanki nerwowej pod wpływem stresu.

**Genotyp Pośredni (Heterozygota)**
Standardowa populacyjna homeostaza przeciwzakrzepowa.

**Genotyp Ryzyka (Niski poziom Białka Z)**
Zwiększona podatność na lokalne mikro-pętle zakrzepowe i subkliniczne niedotlenienie struktur podkorowych w stanach zapalnych. W badaniach psychiatrycznych wariant ten luźno koreluje z przyspieszonym starzeniem poznawczym oraz zaostrzeniem objawów afektywnych.

---

---
### SLC4A1

* **Główny symbol genu:** SLC4A1
* **Pełna nazwa biochemiczna:** Wymiennik anionowy 1 (ang. *Solute carrier family 4 member 1*)
* **Nazwy potoczne i medialne:** Gen AE1, regulator pH i homeostazy kwasowo-zasadowej mózgu
* **Główny rsID:** rs2235081
* **Lokalizacja chromosomalna:** Chromosom 17 (17q21.31)
* **Rola biologiczna genu/białka:** Odpowiada za sprzężoną wymianę jonów chlorkowych (Cl⁻) na wodorowęglanowe (HCO₃⁻) przez błony komórkowe. W układzie nerwowym precyzyjnie kalibruje pH wewnątrzkomórkowe i zewnątrzkomórkowe. Zmiany pH rzędu ułamków jednostki drastycznie modyfikują przewodnictwo postsynaptycznych receptorów NMDA oraz czułość kanałów potasowych.

* **Mój genotyp (WGS):**
  `rs2235081` — **GG** (ensembl+bam, high) — marker_report



**Typ Dziki (Perfekcyjna homeostaza pH)**
Stabilne, optymalne pH tkanki nerwowej. Prawidłowe funkcjonowanie receptorów NMDA i stabilna pobudliwość synaptyczna bez fluktuacji elektrofizjologicznych.

**Warianty Populacyjne (Umiarkowane przesunięcia kinetyki)**
Nieznaczne fluktuacje w progu zmęczenia neuroelektrycznego podczas intensywnego wysiłku fizycznego lub hiperwentylacji.

**Wariant Ryzyka (Obniżona wydajność AE1)**
Lokalne mikro-wahania pH w szczelinach synaptycznych, prowadzące do niestabilności elektrofizjologicznej (np. okresowa hiperreaktywność receptorów glutaminianu). Koreluje ze skłonnością do migren, stanów lękowych i nadwrażliwości sensorycznej.

---

---
### TMEM140

* **Główny symbol genu:** TMEM140
* **Pełna nazwa biochemiczna:** Białko transbłonowe 140 (ang. *Transmembrane protein 140*)
* **Nazwy potoczne i medialne:** Gen morfologii astrocytów i neurostanu zapalnego
* **Główny rsID:** rs2228074
* **Lokalizacja chromosomalna:** Chromosom 22 (22q13.1)
* **Rola biologiczna genu/białka:** Koduje białko transbłonowe o wyjątkowo silnej ekspresji w astrocytach. TMEM140 reguluje morfologię wypustek astrocytarnych, które owijają synapsy (tworząc tzw. synapsę trójdzielną) oraz kontroluje wydzielanie prozapalnych cytokin i chemokin przez glej w odpowiedzi na stres metaboliczny.

* **Mój genotyp (WGS):**
  `rs2228074` — **GG** (ensembl+bam, high) — ensembl; depth=32,avg=32.0; gt=0/0






**Typ Dziki (Zbalansowana odpowiedź glejowa)**
Prawidłowa architektura wypustek astrocytarnych, wydajny wychwyt glutaminianu z synaps, niska spoczynkowa produkcja mediatorów zapalnych.

**Warianty Populacyjne (Średnia ekspresja)**
Standardowa, populacyjna plastyczność glejowa.

**Wariant Ryzyka (Nadekspresja lub niestabilność białka)**
Astrocyty łatwiej wchodzą w tryb reaktywny (astrogliosis), kurcząc swoje wypustki (co upośledza oczyszczanie synaps z glutaminianu) i wydzielając nadmiar czynników zapalnych. Skorelowany w badaniach GWAS z nasileniem przewlekłego neurostanu zapalnego i fenotypami neurorozwojowymi.

---

---
### USP35

* **Główny symbol genu:** USP35
* **Pełna nazwa biochemiczna:** Peptydaza ubikwityno-specyficzna 35 (ang. *Ubiquitin specific peptidase 35*)
* **Nazwy potoczne i medialne:** Gen deubikwitynacji, strażnik stabilności receptorów synaptycznych
* **Główny rsID:** rs1014166
* **Lokalizacja chromosomalna:** Chromosom 11 (11q22.1)
* **Rola biologiczna genu/białka:** USP35 to enzym deubikwitynujący (DUB), który usuwa łańcuchy ubikwityny z białek docelowych, chroniąc je przed zniszczeniem w proteasomach. W mózgu kontroluje czas życia i stabilność receptorów postsynaptycznych oraz kluczowych białek mitochondrialnych, zapobiegając ich przedwczesnej utylizacji.

* **Mój genotyp (WGS):**
  `rs1014166` — **GG** (ensembl+bam, high) — marker_report



**Typ Dziki (Stabilny proteom synaptyczny)**
Precyzyjna kontrola nad obrotem białkowym. Ochrona ważnych receptorów przed niesłuszną degradacją, stabilna energetyka mitochondrialna.

**Warianty Populacyjne (Umiarkowane)**
Standardowe populacyjne tempo utylizacji i deubikwitynacji białek.

**Homozygota alleliczna G/G (tag-SNP GWAS)**
Profil populacyjny rs1014166 (G/G). W GWAS powiązany ze statystyczną podatnością na zmęczenie, depresję i schizofrenię — nie oznacza patologicznej degradacji białek ani klinicznej diagnozy.

---

---

## CZĘŚĆ XV. Tau, pamięć epizodyczna i starzenie neuronalne
### MAPT

* **Główny symbol genu:** MAPT
* **Pełna nazwa biochemiczna:** Białko tau powiązane z mikrotubulami (ang. *Microtubule-associated protein tau*)
* **Nazwy potoczne i medialne:** Gen białka Tau, region inwersji 17q21.31 (Haplotyp H1/H2)
* **Główny rsID / Haplotyp:** Duża inwersja strukturalna (~900 kb) dzieląca populację na klady H1 i H2
* **Lokalizacja chromosomalna:** Chromosom 17 (17q21.31)
* **Rola biologiczna genu/białka:** MAPT odpowiada za stabilizację mikrotubul w cytoszkielecie aksonalnym neuronów, co umożliwia prawidłowy transport wewnątrzkomórkowy. Blok inwersyjny całkowicie znosi rekombinację genetyczną w tym regionie, tworząc dwa odmienne ewolucyjnie haplotypy.

* **Mój genotyp (WGS):**
  `rs1800547` — **AA** (ensembl+bam, high) — ncbi; depth=32,avg=32.0; gt=0/0






**Diplotyp H1/H1 (Wariant bezpośredni, klasyczny)**
Wysoka podstawowa transkrypcja białka tau. Zapewnia doskonałą plastyczność strukturalną sieci neuronalnych w młodości, jednak w wieku dorosłym drastycznie zwiększa predyspozycje do tauopatii (choroba Alzheimera, postępujące porażenie nadjądrowe, choroba Parkinsona).

**Diplotyp H1/H2 (Heterozygota strukturalna)**
Złoty środek populacyjny. Pośrednia ekspresja izoform tau, optymalna stabilność cytoszkieletu komórkowego, zrównoważony profil neuroprotekcyjny i dobra elastyczność poznawcza.

**Diplotyp H2/H2 (Wariant odwrócony)**
Niższa podstawowa ekspresja tau (silna ochrona przed neurodegeneracją w starości). Jednak specyficzna architektura bloku H2 wykazuje ekstremalną skłonność do pętli niestabilności genomowej (niealleliczna rekombinacja homologiczna). Drastycznie zwiększa to ryzyko mikrodelecji 17q21.31, co bezpośrednio indukuje fenotyp neurorozwojowy (opóźnienie mowy, dysmorfia, ASD).

---

---
